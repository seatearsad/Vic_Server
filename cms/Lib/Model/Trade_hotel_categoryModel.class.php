		...
		foreach($tmp_category_list as $value){
			if($value['cat_fid'] == '0'){
				...
				if($has_stock){
					...
					$value['discount_price_txt'] = $stock_list[$value['cat_id']]['discount_price'];
				}
	...
	function get_refund_txt($has_refund,$refund_hour,$enter_time,$dep_time=''){
		if($has_refund == 0){
			...
			// if($dep_time){
				...
				// }
			// }else{
				$has_refund_txt = '入住时间('.str_replace(':00','点',$enter_time).')'.$refund_hour.'小时前内能退';
			// }
		}
		...
		foreach($hotel_stock_list as $value){
			if(!in_array($value['cat_id'],$no_stock_day)){
				if(empty($value['stock'])){
					...
					$stock_list[$value['cat_id']]['price'] = intval($value['price']);
					$stock_list[$value['cat_id']]['discount_price'] = intval($value['discount_price']);
