...
class Card_newModel extends Model{
    ...
    public function get_card_user_list_by_mer_id($mer_id){
        $where['mer_id']=$mer_id;
        $card_user_list = M('Card_userlist')->field('c.id,c.card_money,c.card_money_give,c.card_score,c.physical_id,c.add_time,c.status,u.nickname,u.phone')->join('as c left join '.C('DB_PREFIX').'user as u ON c.uid=u.uid')->where($where)->select();
    ...
    public function get_cardinfo_by_id($id){
        $where['c.id']=$id;
        return M('Card_userlist')->field('c.id,c.uid,c.card_money,c.card_money_give,c.card_score,c.physical_id,c.add_time,c.status,c.gid,u.nickname,u.phone')->join('as c left join '.C('DB_PREFIX').'user as u ON c.uid=u.uid')->where($where)->find();
    }

    

    //会员卡金额记录
    public function card_use_record($id,$type){
        // $where['uid']=$uid;
        $where['card_id']=$id;
    ...
    //自动领卡
    public function auto_get($uid,$mer_id){
        if($card = M('Card_new')->where(array('mer_id'=>$mer_id))->find()){
            ...
            if($card_id = M('Card_userlist')->add($data)){
                ...
                if($now_user = M('User')->where(array('uid'=>$uid))->find()){
                    ...
                    $model->sendTempMsg('OPENTM200964573', array('href' => C('config.site_url').'/wap.php?g=Wap&c=My_card&a=merchant_card&mer_id='.$mer_id, 'wecha_id' =>$now_user['openid'], 'first' =>  '您成功领取了商家【'.$now_merchant['name'].'】的会员卡', 'keyword1' =>$card_id, 'keyword2' => $now_user['nickname'],'keyword3'=>$now_user['phone'],'keyword4'=>date("Y年m月d日 H:i"), 'remark' => '感谢您的关注！'));
                }
    ...
    public function get_card_by_uid_and_mer_id($uid,$mer_id){
        ...
        $card_info = array_merge($merchant_card,$now_user_card);
    ...
    public function auto_reg_or_bind($mer_id,$openid,$card_id){
        ...
        if(!empty($now_card)&&$now_card['auto_get']){
            if(empty($now_user)){
                        ...
                        if($card_id){
                            if($card_id&&!M('Card_userlist')->where(array('uid'=>$now_user['uid'],'mer_id'=>$mer_id))->find()){
                                ...
                                if($now_user['openid']){
                                    $model = new templateNews(C('config.wechat_appid'), C('config.wechat_appsecret'));
                                    $now_merchant = M('Merchant')->where(array('mer_id'=>$mer_id))->find();
                                    $model->sendTempMsg('OPENTM200964573', array('href' => C('config.site_url').'/wap.php?g=Wap&c=My_card&a=merchant_card&mer_id='.$mer_id, 'wecha_id' =>$openid, 'first' => '您成功领取了商家【'.$now_merchant['name'].'】的会员卡', 'keyword1' =>$card_id, 'keyword2' => $now_user['nickname'],'keyword3'=>$now_user['phone'],'keyword4'=>date("Y年m月d日 H:i"), 'remark' => '感谢您的关注！'));
                                }
    ...
    public function get_user_all_card($uid){
         $sql = 'SELECT c.card_id,c.bg,c.diybg,c.numbercolor,m.name,c.discount,cl.id as cardid,cl.card_money,cl.card_money_give,'
            .'m.pic_info,m.mer_id,c.status as card_status ,cl.status as usercard_status FROM '
            .C('DB_PREFIX').'card_userlist `cl`  left join '
            .C('DB_PREFIX').'merchant m on m.mer_id  = cl.mer_id left join '
            .C('DB_PREFIX').'card_new `c` on m.mer_id = c.mer_id   where ( cl.uid = '.$uid.' ) ';
        $res =  M('')->query($sql);
