...
class Group_orderModel extends Model{
	public $status_list = array('-1' => '全部', 0 => '未消费', 1 => '已消费', 2 => '已完成', 3 => '已退款', 4 => '已取消', 6 => '部分退款');
	...
	public function save_post_form($group,$uid,$order_id){
		...
		if($_SESSION['now_village_bind']['village_id']>0&&M('House_village_group')->where(array('village_id'=>$_SESSION['now_village_bind']['village_id'],'group_id'=>$group['group_id']))->find()){
		...
		if($_POST['group_type']==1){
			$data_group_order['price'] = $group['old_price'];
			$data_group_order['single_buy']=1;

		}elseif($_POST['group_type']==3){
			$data_group_order['price'] = $group['price']*$group['start_discount']/100; //团长按团长折扣计算
		...
		$data_group_order['total_money'] = $data_group_order['price'] * $data_group_order['num'] * 1.05;
		
		//不四舍五入
		$data_group_order['total_money'] = floor($data_group_order['total_money'] * 100)/100;		
		...
		if($_POST['delivery_type'] && $group['tuan_type']==2){
			if(!empty($_POST['pick_in_store'])){
				if(!empty($_POST['pick_address'])){
					$data_group_order['is_pick_in_store'] = 1;
					$data_group_order['adress'] = $_POST['pick_address'];
					$data_group_order['pick_lng'] = $_POST['pick_lng'];
					$data_group_order['pick_lat'] = $_POST['pick_lat'];
		...
		if($order_id){
			...
			$data_group_order['remark'] = $_POST['remark'];
...
// 					}
				...
				if ($_SESSION['user']['openid']) {
					...
					$model->sendTempMsg('OPENTM201682460', array('href' => $href, 'wecha_id' => $_SESSION['user']['openid'], 'first' => '您好，您的订单已生成', 'keyword3' => $orderid, 'keyword1' => date('Y-m-d H:i:s'), 'keyword2' => $data_group_order['order_name'], 'remark' => '您的该次'.C('config.group_alias_name').'下单成功，感谢您的使用！'));
				}
...
//		$data_group_order['score_used_count'] = 0;
//		$data_group_order['score_deducte'] = 0;
//		$data_group_order['card_id'] = 0;
//		$data_group_order['card_price'] = 0;
//		$data_group_order['coupon_id'] = 0;
//		$data_group_order['coupon_price'] = 0;
					...
					if(!empty($now_user_spread)){
						...
						if($spread_user && $user_spread_rate&&!in_array($spread_user['uid'],$spread_users)){
							...
							if($spread_money>0){
								...
								$model->sendTempMsg('OPENTM201812627', array('href' => $href, 'wecha_id' => $now_user_spread['spread_openid'], 'first' => $buy_user['nickname'] . '通过您的分享购买了【' . $now_order['order_name'] . '】，验证消费后您将获得'.$money_name.'。', 'keyword1' => $spread_money, 'keyword2' => date("Y年m月d日 H:i"), 'remark' => '点击查看详情！'));
							}
...
//							$sub_user_spread_rate = $update_group['sub_spread_rate'] > 0 ? $update_group['sub_spread_rate'] : C('config.user_first_spread_rate');
							...
							if ($second_user && $sub_user_spread_rate&&!in_array($second_user['uid'],$spread_users)) {
								...
								if($spread_money>0) {
									$model->sendTempMsg('OPENTM201812627', array('href' => $href, 'wecha_id' => $second_user_spread['spread_openid'], 'first' => $sec_user['nickname'] .'的子用户'.$buy_user['nickname'] . '通过您的分享购买了【' . $now_order['order_name'] . '】，验证消费后您将获得佣金。', 'keyword1' => $spread_money, 'keyword2' => date("Y年m月d日 H:i"), 'remark' => '点击查看详情！'));
								}
...
//								$sub_user_spread_rate = $update_group['third_spread_rate'] > 0 ? $update_group['third_spread_rate'] : C('config.user_second_spread_rate');
								...
								if ($first_spread_user && $sub_user_spread_rate&&!in_array($first_spread_user['uid'],$spread_users)) {
									...
									if($spread_money>0) {
										$model->sendTempMsg('OPENTM201812627', array('href' => $href, 'wecha_id' => $first_user_spread['spread_openid'], 'first' =>$fir_user['nickname'].'的子用户的子用户'.$buy_user['nickname'] . '通过您的分享购买了【' . $now_order['order_name'] . '】，验证消费后您将获得佣金。', 'keyword1' => $spread_money, 'keyword2' => date("Y年m月d日 H:i"), 'remark' => '点击查看详情！'));
									}
									// D('User')->add_money($first_spread_user['uid'],$spread_money,'子用户推广用户 '.$now_user['nickname'].' 购买ID为 '.$now_order['group_id'].' 的商品 获得的佣金');
								}
							}
				...
				if($now_group['pin_num']>0&&!$now_order['single_buy']){
					...
					}elseif(!empty($_SESSION['gid'])){
						D('Group_start')->add_group($now_order['group_id'],$now_order['uid'],$now_group,$now_order['order_id'],$_SESSION['gid']);
					}else{
				...
				if ($now_user['openid'] && $order_param['is_mobile']) {
					...
					if($now_order['tuan_type'] < 2&&$now_group['pin_num']==0){
						...
						$model->sendTempMsg('OPENTM201752540', array('href' => $href, 'wecha_id' => $now_user['openid'], 'first' => C('config.group_alias_name').'提醒', 'keyword1' => $now_order['order_name'], 'keyword2' => $now_order['real_orderid'], 'keyword3' => $now_order['total_money'], 'keyword4' => date('Y-m-d H:i:s'), 'remark' => $remark));
					} else {
						$model->sendTempMsg('OPENTM201752540', array('href' => $href, 'wecha_id' => $now_user['openid'], 'first' => C('config.group_alias_name').'提醒', 'keyword1' => $now_order['order_name'], 'keyword2' => $now_order['real_orderid'], 'keyword3' => $now_order['total_money'], 'keyword4' => date('Y-m-d H:i:s'), 'remark' => C('config.group_alias_name').'成功，感谢您的使用'));
					}
				}
					...
					if ($data_group_order['group_pass']&&$now_group['pin_num']==0) {
						if($now_group['open_num']==0&&$now_group['open_now_num']==0&&$now_group['group_share_num']==0&&$now_group['pin_num']==0){
							...
							if($now_group['group_share_num']!=0&&$now_group['group_share_num']>$group_share_num){
								...
								$remark = '还差'.($now_group['open_num']-$now_group['sale_count']).'份才能成团,快分享给好友吧,成团后消费码将在订单详情中显示';
							}else{
								$remark = C('config.group_alias_name').'成功，您的消费码：'.$data_group_order['group_pass'];
							}
						}
