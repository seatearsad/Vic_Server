...
class GroupModel extends Model{
	...
	public function get_group_list($type,$limit=12,$is_wap=false){
		...
		$group_list = D('Group')->join('AS g INNER JOIN '.C('DB_PREFIX').'merchant m ON g.mer_id = m.mer_id ')->field('`g`.`name` AS `group_name`,`m`.`name` AS `merchant_name`,`g`.*,`m`.*')
				...
				->where("`m`.`city_id`='".C('config.now_city')."'  AND `g`.`status`='1' AND `m`.`status`='1' AND `g`.`type`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time'")->order($order)->limit($limit)->select();
...
// 				$group_list[$key]['wx_cheap'] = floatval($value['wx_cheap']);
	...
	public function get_single_store_group_list($store_id,$limit=6,$is_wap=false){
		$now_time = $_SERVER['REQUEST_TIME'];
		$group_list = D('')->field('`g`.`group_id`,`g`.`cat_id`,`g`.`cat_fid`,`g`.`name` AS `group_name`,`g`.`price`,`g`.`pic`,`g`.`old_price`,`g`.`wx_cheap`,`g`.`sale_count`,`g`.`virtual_num`,`g`.`end_time`,`g`.`begin_time`,`g`.`pin_num`,`g`.`extra_pay_price`')->table(array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'group_store'=>'gc'))->where("`gc`.`city_id`='".C('config.now_city')."' AND `g`.`group_id`=`gc`.`group_id` AND `gc`.`store_id`='$store_id' AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`end_time`>'$now_time'")->order('`g`.`sort` DESC,`g`.`group_id` DESC')->limit($limit)->select();// AND `g`.`begin_time`<'$now_time'
		if($group_list){
			$group_image_class = new group_image();
	...
	public function get_group_list_by_keywords($keywords,$order,$is_wap=false){
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`m`.`city_id`='".C('config.now_city')."' AND `g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `m`.`status`='1' AND `g`.`type`='1' AND `g`.`begin_time`<'$now_time' AND `g`.`end_time`>'$now_time' AND (`g`.`name` LIKE '%$keywords%' OR `m`.`name` like '%$keywords%') ";
		$condition_table = array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'merchant'=>'m');
		$condition_field  = '`g`.`name` AS `group_name`,`m`.`name` AS `merchant_name`,`g`.*,`m`.*';
...
// 				$order = 'juli asc,`g`.`group_id` DESC';
	...
	public function wap_get_storeList_by_catid($get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0,$store_name=false){
		$now_time = $_SERVER['REQUEST_TIME'];
		$condition_where = "`gc`.`city_id`='".C('config.now_city')."' AND `g`.`status`='1' AND `g`.`type`='1' AND `s`.`status`='1' AND `g`.`end_time`>'$now_time' AND `gc`.`group_id`=`g`.`group_id` AND `gc`.`store_id`=`s`.`store_id` AND `m`.`status`='1' AND `m`.`mer_id`=`s`.`mer_id` AND `s`.`have_group`=1";// AND `g`.`begin_time`<'$now_time'
		//分类相关
		if(!empty($get_grouplist_catfid)){
			$condition_where .= " AND `g`.`cat_fid`='$get_grouplist_catfid'";
		}else if(!empty($get_grouplist_catid)){
	...
	/*手机版得到订单列表*/
	public function wap_get_order_list($uid,$status = '0',$last_order_id=false,$is_app=false){
		//$condition_where = "`o`.`uid`='$uid' AND `o`.`group_id`=`g`.`group_id` AND `o`.`order_id` = `l`.`order_id` AND `l`.`fid` = `s`.`id`";
		$condition_where ='';
		if($status == '0'){
			$condition_where .= "  `o`.`status`<=3";
		}else if($status == '-1'){
		...
		//$condition_table = array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'group_order'=>'o',C('DB_PREFIX').'group_buyer_list'=>'l',C('DB_PREFIX').'group_start'=>'s');
		if($is_app){
			if($last_order_id){
			...
			$order_list = $this
					...
					->field('`o`.order_id,o.real_orderid,o.status,o.paid,o.pay_time,o.num,o.total_money,o.add_time,o.is_head,o.is_pick_in_store,o.third_id,o.pay_type,`g`.`s_name`,`g`.`pic`,`g`.`end_time`,g.pin_num,s.status as pin_status')
		...
		foreach ($order_list as $value) {
			...
			if (M('Group_order')->where($condition_group_order)->data($data_group_order)->save()) {
				...
				$model->sendTempMsg('TM00017', array('href' => $href,
						...
						'remark' =>'快递号：'.$value['order_id'].'('.$express_nmae['name'].'),请尽快确认'));
