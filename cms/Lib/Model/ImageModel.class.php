...
// 		array('size' => 5, 'path', 'thumbMaxWidth', 'thumbMaxHeight', 'thumb' => true, 'imageClassPath' => 'ORG.Util.Image', 'thumbPrefix' => 'm_,s_', 'thumbRemoveOrigin' => false);
		...
		$upload->allowTypes = array('image/png', 'image/x-png', 'image/jpg', 'image/jpeg', 'image/pjpeg', 'image/gif', 'audio/mp3', 'image/x-icon');
		...
		if (!$upload->upload()) {// 上传错误提示错误信息
			...
			foreach ($files as $file) {
				...
				$this->add(array('oid' => $oid, 'otype' => $otype, 'ip' => get_client_ip(), 'dateline' => time(), 'pic' => $images['url'][$file['key']], 'pic_md5' => md5($images['url'][$file['key']]),'img_remark'=>$file['name'],'img_width'=>intval($imageSizeArr[0]),'img_height'=>intval($imageSizeArr[1])));
				if ($flag) {
