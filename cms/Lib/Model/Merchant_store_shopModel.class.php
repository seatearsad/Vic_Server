...
// 			$v['delivery_fee'] = $v['deliver_type'] == 0 || $v['deliver_type'] == 3 ? C('config.delivery_fee') : $v['delivery_fee'];
     ...
     * @param array $where
     * @param int $is_wap
     * @param int $type 1 链接get_list_by_option 2链接 getRecommendList
     * @param $limit
     * @param int $page
    ...
    public function get_list_arrange($where = array(), $is_wap = 1,$type = 1,$limit,$page = 1,$lat=0,$long=0,$city_id = -1,$menu_version = -1)
    {
        ...
        switch ($type){
            ...
            case 2:
                $t_list = $this->getRecommendList($city_id,$lat,$long,$where['cat_id'],$where['cat_fid'],$menu_version);
                break;
            default:
                $t_list = array();
                break;
...
//            }
            ...
            $temp['event'] = array("use_price"=>"0","discount"=>"0","miles"=>0,"desc"=>"");
            //garfunkel获取减免配送费的活动
    ...
    public function getRecommendList($city_id,$lat,$lng,$cat_id,$cat_fid,$menu_version = -1){
        //获取推荐列表
        $closeArr = array();
        $openArr = array();
        if($cat_id == 0)
            $storeList = D('Shop_category_relation')->where(array('cat_fid'=>$cat_fid))->order('store_sort desc')->select();
        else
