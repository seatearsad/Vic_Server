...
class House_village_visitorModel extends Model{
    ...
    protected function chkownerPhone(){
        ...
        return true;
        if((!preg_match($phone_match, $owner_phone))&&(!preg_match($mobile_match, $owner_phone))){
            return false;
        }else{
            return true;
        }
    ...
    protected function visitorphonePhone(){
        ...
        $phone_match='/^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/';
        $mobile_match='/^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/';
        if((!preg_match($phone_match, $visitor_phone))&&(!preg_match($mobile_match, $visitor_phone))){
            return false;
        }else{
            return true;
        }
    ...
    public function house_village_visitor_add($data,$status){
        if(!$data){
            return false;
        }
        ...
        if($status == 1){
			...
			if($data['visitor_name']){
	                ...
	                if($userInfo['openid'] && !$data['status']){
	        ...
	        if(!$this->create()){
	            ...
	            if($insert_id = $this->add()){
	               ...
	               if(!$insert_info['status'] && C('config.village_sms') == 1){
    ...
    public function ajax_house_village_visitor_search($where,$page,$page_coun){
        if(!$where){
            return false;
        }
