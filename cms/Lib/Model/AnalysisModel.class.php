...
class AnalysisModel extends Model {
	public function get_consumer($areaid, $_type, $tid, $year, $month, $period) {
		...
		if (!empty ($period) && $period != 'undefined') {
			...
			$start_time = strtotime($period[0]);
			$end_time = strtotime($period[1]);
			if ($start_time == $end_time) {
				$time = ' AND UNIX_TIMESTAMP(FROM_UNIXTIME(b.pay_time,"%Y-%m-%d"))=' . $start_time;
			} else {
				$time = ' AND b.pay_time>=' . $start_time . ' AND b.pay_time<=' . $end_time;
			}
		}
	...
	public function check_area_id($area_id, $type) {
		$many_city = D('Config')->field('value')->where(array('name' => 'many_city'))->select();
		if (!$many_city[0]['value']) { // 单城市
			if ($type < 2) {
				$city_id = D('Config')->field ('value')->where(array('name' => 'now_city'))->select();
		...
		if ($type > 0) {
			if ($type == 4) {
				$r = D('House_village')->field('village_name')->where (array('village_id' => $area_id))->select ();
				$area_panme = $r[0]['village_name'];
			} else {
				$r = D('Area')->field ('area_name')->where (array('area_id' => $area_id))->select ();
	...
	public function get_village_consumer($areaid, $type, $year, $month, $period) {
		...
		if (!empty($period)) {
			...
			$end_time = strtotime($period[1]);
			if ($start_time == $end_time) {
				$condition = ' AND UNIX_TIMESTAMP(FROM_UNIXTIME(pay_time,"%Y-%m-%d"))=' . $start_time;
			} else {
				$condition = ' AND pay_time>=' . $start_time . ' AND pay_time<=' . $end_time;
			}
		}
		...
		if ($area_id == '') {
			...
			$sql2 = "select order_name, month(FROM_UNIXTIME(pay_time)) as month,sum(money) as money from " . C('DB_PREFIX') . "house_village_pay_order a " . $condition . " group by order_type";
		} elseif ($type == 1) {
			...
			$sql2 = "select order_name, month(FROM_UNIXTIME(pay_time)) as month,sum(money) as money from " . C ( 'DB_PREFIX' ) . "house_village_pay_order a " . "LEFT JOIN " . C ( 'DB_PREFIX' ) . "house_village b ON a.village_id=b.village_id " . $condition . " AND b.province_id= " . $area_id . " group by order_type";
		} elseif ($type == 2) {
			...
			$sql2 = "select order_name, month(FROM_UNIXTIME(pay_time)) as month,sum(money) as money from " . C('DB_PREFIX') . "house_village_pay_order a " . "LEFT JOIN " . C('DB_PREFIX') . "house_village b ON a.village_id=b.village_id " . $condition . " AND b.city_id= " . $area_id . " group by order_type";
		} elseif ($type == 3) {
			...
			$sql2 = "select order_name, month(FROM_UNIXTIME(pay_time)) as month,sum(money) as money from " . C('DB_PREFIX') . "house_village_pay_order a " . "LEFT JOIN " . C('DB_PREFIX') . "house_village b ON a.village_id=b.village_id " . $condition . " AND b.area_id= " . $area_id . " group by order_type";
		} elseif ($type == 4) {
			if ($_POST ['month'] != '') {
				$condition2 = 'month ,';
			}
