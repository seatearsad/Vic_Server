			...
			if ($status == 0) {
				if ($village['handle_type']) {
					...
					foreach ($workers as $worker) {
						$model->sendTempMsg('TM00356', array('href' => $href, 'wecha_id' => $worker['openid'], 'first' => $first, 'work' => '您有一个' . $first . '需处理', 'remark' => date('Y-m-d H:i:s')));
					}
				}
			} else {
				$href = C('config.site_url').'/wap.php?c=Worker&a=detail&wxscan=1&pigcms_id=' . $order['pigcms_id'];
		...
		if ($village = D('House_village')->field(true)->where(array('village_id' => $order['village_id']))->find()) {
			...
			if ($status == 0) {
				if ($village['handle_type'] == 0) {
					...
					$model->sendTempMsg('TM00356', array('href' => $href, 'wecha_id' => $village['openid'], 'first' => $first, 'work' => '您有一个' . $first . '需处理', 'remark' => date('Y-m-d H:i:s')));
				}
			} else {
				$status_all = array('下单成功', '物业已受理', '处理人员已受理', '处理人员完成', '业主已评价');
				$href = C('config.site_url').'/wap.php?c=Customer&a=detail&pigcms_id=' . $order['pigcms_id'];
				$model->sendTempMsg('TM00356', array('href' => $href, 'wecha_id' => $village['openid'], 'first' => $first, 'work' => $first . '工单现在的状态已经被' . $status_all[$status], 'remark' => date('Y-m-d H:i:s')));
			}
		}
	}
