...
class Meal_store_categoryModel extends Model{
	...
	public function get_son_category_list_byfid($cat_fid){
		...
		$condition_group_category['cat_status'] = '1';
		return $this->field(true)->where($condition_group_category)->order('`cat_sort` DESC,`cat_id` ASC')->select();
	}
	/*根据子分类ID获得同级分类列表*/
	...
	public function get_all_category(){
		$condition_group_category['cat_status'] = '1';
		$tmp_group_category = $this->field(true)->where($condition_group_category)->order('`cat_sort` DESC,`cat_id` ASC')->select();
