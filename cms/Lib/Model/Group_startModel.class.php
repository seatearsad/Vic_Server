<?php
    class Group_startModel extends Model{
        ...
        //参加团购 按人数算
        public function add_group($fid,$uid,$now_group,$order_id,$gid){
            $where['group_id']=$fid;
            //$where['status']='0';
            ...
            $already_buy = M('Group_buyer_list')->where(array('id'=>$the_fastest['id'],'uid'=>$uid))->find();
                    ...
                    foreach($buyer as $v){
                        ...
                        $model->sendTempMsg('TM00017',
                            array('href' => $href,
                                ...
                                'remark' => C('config.group_alias_name').'成功，您的消费码：'.$v['group_pass']));
								//fdump($v,'vv',1);
                        $sms_data['uid'] = $uid;
        ...
        //获取拼团小组成员 包括机器人
        public function get_buyerer_by_order_id($order_id,$gid){
            if(!empty($order_id)){
                $res = $this->get_group_start_by_order_id($order_id);
            }else{
                $res['id']=$gid;
            }
            return M('Group_buyer_list')->field('b.*,o.is_head,o.pay_time,u.phone,u.nickname,u.avatar')->join('as b left join '.C('DB_PREFIX').'user u ON b.uid = u.uid')->join(C('DB_PREFIX').'group_order o ON b.order_id = o.order_id')->where(array('b.fid'=>$res['id']))->group('b.uid')->order('is_head DESC')->select();
        }
        ...
        public function get_group_start_user_by_gid($gid){
            $share_group_info = M('Group_start')->join('as s left join '.C('DB_PREFIX').'user u ON s.uid = u.uid')->where(array('id'=>$_GET['gid']))->find();
...
//            dump($this);
        ...
        public function check_join_pin($group_id,$uid,$effective_time){
            $effective_time = time()-$effective_time*3600;
            $where=array(
                'group_id' =>$group_id,
                'status' => 0,
                'start_time'=>array('gt',$effective_time),
            );
            if(!empty($uid)){
                $where['uid']= array('neq',$uid);
            ...
            $start = $this->where($where)->order('num DESC,start_time ASC')->find();
            if($start&&!$start_head){
                return $start['complete_num']-$start['num'];
            }else{
                return false;
            }
        }
