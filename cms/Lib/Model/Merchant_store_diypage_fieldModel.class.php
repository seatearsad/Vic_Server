					...
					if (!$_SESSION['sync_user']) { //非对接用户可看：优惠券
						...
						foreach ($fieldContent as $v) {
							foreach ($v as $val) {
								...
								if ($coupons) {
									$fieldHtml['html'] .= '<li style="margin-top:5px; width:31%;">  <a href="' . U('My_card/merchant_coupon',array('mer_id'=>C('now_store.mer_id'))) . '"><div class="custom-coupon-price"><span>$</span>' . $coupons['discount'] . '</div><div class="custom-coupon-desc">' . $coupons['name'] . '</div>  </a> </li>';
								}
							}
						}
...
function richTextFormat($matches, $store_id) {
	if (strpos($matches,option('config.wap_site_url')) !== false) {
		if (strpos($matches,'store_id=')  !== false) {
			$url    = preg_replace('/store_id\=\d+/i', 'store_id=' . $store_id, $matches);
		} else {
			if (strpos($matches,'?')  === false) {
				$url    = $matches . '?store_id=' . $store_id;
			} else {
				$url    = $matches . '&store_id=' . $store_id;
			}
		}
