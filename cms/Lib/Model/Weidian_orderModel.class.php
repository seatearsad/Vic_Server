...
//			$data_weidian_order['card_price'] = $now_coupon['price'];
...
//		}
		if ($now_coupon['card_price'] >0) {
			$data_weidian_order['card_id'] = $now_coupon['merc_id'];
			$data_weidian_order['card_price'] = round($now_coupon['card_price'] * 100)/100;
		...
		if(!empty($merchant_balance['card_money'])&&$order_info['use_merchant_balance']){
			if($merchant_balance['card_money'] >= $pay_money){
				...
				$data_weidian_order['merchant_balance'] = $merchant_balance['card_money'];
			}
		...
		if(!empty($merchant_balance['card_give_money'])&&$order_info['use_merchant_balance']){
			if($merchant_balance['card_give_money'] >= $pay_money){
				$data_weidian_order['card_give_money'] = $pay_money;
				$order_result = $this->wap_pay_save_order($order_info,$data_weidian_order);
				if($order_result['error_code']){
					return $order_result;
				}
				...
				$data_weidian_order['card_give_money'] = $merchant_balance['card_give_money'];
			}
	...
	public function wap_pay_save_order($order_info,$data_weidian_order){
		...
		$data_weidian_order['card_give_money'] 	= !empty($data_weidian_order['card_give_money']) ? $data_weidian_order['card_give_money'] : 0;
		$data_weidian_order['card_discount'] 	= !empty($data_weidian_order['card_discount']) ? $data_weidian_order['card_discount'] : 0;
		...
		$data_weidian_order['card_price']	 	= !empty($data_weidian_order['card_price']) ? $data_weidian_order['card_price'] : 0;
...
//			}
			...
			if($this->where($where)->save($data_weidian_order)){
				//支付成功增加商家余额
				$now_order = $this->field(true)->where($where)->find();
				$now_order['order_type']='weidian';
				D('Merchant_money_list')->add_money($now_order['mer_id'],'微店支付计入收入',$now_order);
				...
				D('Scroll_msg')->add_msg('weidian',$now_user['uid'],'用户'.$now_user['nickname'].'于'.date('Y-m-d H:i',$_SERVER['REQUEST_TIME']).'购买微店成功');

				D('User')->add_score($now_order['uid'],round($now_order['money']*$score_get),'购买 '.$now_order['order_id'].' 消费'.floatval($now_order['money']).'元 获得积分');
				D('Scroll_msg')->add_msg('weidian',$now_user['uid'],'用户'.$now_user['nickname'].'于'.date('Y-m-d H:i',$_SERVER['REQUEST_TIME']).'购买微店成功获得'.C('config.score_name'));
	...
	public function get_weidian_url($data){
		...
		return C('config.weidian_url').'/api/pay_callback.php?'.http_build_query($data);
	}
