...
class Card_new_couponModel extends Model{
    ...
    public function get_coupon_list(){
        ...
        $res = $this->where($where)->order('status ASC,allow_new DESC,discount DESC')
            ->getField('coupon_id,name,img,had_pull,num,des,cate_name,cate_id,discount,order_money,start_time,end_time,status,allow_new');
        return $res;
    }

    //根据商家id获取可领取的优惠券
    public function get_coupon_list_by_merid($mer_id,$num){
        $where['end_time'] = array('gt',time());
        $where['start_time'] = array('lt',time());
        $where['status'] = array('neq',0);
        $where['mer_id'] = $mer_id;
        ...
        $res = $this->where($where)->order('status ASC,allow_new DESC,discount DESC,add_time DESC')
            ->getField('coupon_id,name,platform,img,had_pull,num,des,cate_name,cate_id,discount,order_money,start_time,end_time,last_time,status,allow_new,wx_cardid,wx_ticket_addtime,cardsign');
        return $res;
    }
	//根据商家id获取可领取的优惠券
    ...
    public function get_coupon_list_by_merid_and_auto_get($mer_id,$num){
        $where['end_time'] = array('gt',time());
        $where['start_time'] = array('lt',time());
        $where['status'] = array('neq',0);
        $where['auto_get'] = 1;
    ...
    public function get_user_coupon_list($uid,$mer_id,$type,$order_id ){
        //$where['c.end_time'] = array('gt',time());
        $where['c.status'] = array('neq',0);  //状态正常
        $where['h.uid'] = $uid;
        $where['c.mer_id'] = $mer_id;
    ...
    public function get_noworder_coupon_list($now_order,$order_type,$platform,$business_type){
       ...
       // $where['_string'] = '(h.phone='.$phone.')  OR ( h.uid = '.$uid.') ';
...
//        $cate_platform = $this->cate_platform();
        $res = M('Card_new_coupon_hadpull')->join('as h left join '.C('DB_PREFIX').'card_new_coupon c ON h.coupon_id=c.coupon_id')->join(C('DB_PREFIX').'card_userlist l ON h.uid = l.uid ')->join( C('DB_PREFIX').'card_new as cn ON cn.mer_id = l.mer_id')->field('h.id,c.*')->order('h.is_use ASC,c.discount DESC ,c.add_time DESC')->where($where)->select();
