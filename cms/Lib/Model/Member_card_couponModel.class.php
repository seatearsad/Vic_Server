...
class Member_card_couponModel extends Model
{
	...
	public function get_all_coupon($uid, $is_use = '0')
	{
		...
		if ($cardids) {
			...
			if ($ids) {
				...
				$records = D("Member_card_coupon_record")->where(array('wecha_id' => $uid, 'coupon_id' => array('in', $ids)))->order('`is_use` ASC')->select();
				foreach ($records as $r) {
					if (isset($temp[$r['coupon_id']])) {
						$temp[$r['coupon_id']]['record_id'] = $r['id'];
						$temp[$r['coupon_id']]['is_use'] = $r['is_use'];
						if($r['is_use']==1&&$n==1){
							$temp[$r['coupon_id']]['line']=1;
							$n++;
						}
						$result[] = $temp[$r['coupon_id']];
						$merids[] = $r['token'];
	...
	public function add_card($record_id, $mer_id, $uid)
	{
		...
		if ($now_coupon_record['use_time'] > 0 || $now_coupon_record['is_use'] == '1') {
			...
			if (empty($result)) {
				...
				return array('error_code' => 0, 'msg' => '退款，返还优惠券成功');
			}
		}
		return array('error_code' => 1, 'msg' => '退款，返还优惠券失败');
	}
	...
	public function get_coupon_info($id){
		$record = D('Member_card_coupon_record')->where(array('id'=>$id))->find();
		$now_coupon = $this->where(array("id"=>$record['coupon_id']))->find();
		return $now_coupon;
	}
}
?>