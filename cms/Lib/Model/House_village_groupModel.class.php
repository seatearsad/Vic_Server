	...
	public function get_order_list($type,$village_id='',$time_condition='',$is_system = 0){
		...
		switch($type){
			...
			case 'shop':
				...
				$field ='o.real_orderid as order_id,s.name as des,o.userphone as phone,o.pay_time,o.total_price as total_money ,(balance_pay+payment_money+merchant_balance-no_bill_money) as pay_in_fact,o.pay_time,o.status,m.name as mer_name';
				$count  =D($order_table)->join('as o left join '.C('DB_PREFIX').'merchant as m ON o.mer_id = m.mer_id ')->where($condition)->count();
				$p = new Page($count,20);
				$order_list = D($order_table)->field($field)->join('as o left join '.C('DB_PREFIX').'merchant as m ON o.mer_id = m.mer_id left join '.C('DB_PREFIX').'merchant_store s ON s.store_id = o.store_id')->where($condition)->order('o.pay_time DESC')->select();
				break;
			case 'meal':
				...
				$ids = implode(',', $tmp);
				$condition = array( 'o.store_id' => array('in', $ids));
				//$condition['o.paid']= 1;
