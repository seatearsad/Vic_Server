...
class UserinfoAction extends BaseAction{
	...
	public function index()
	{
		...
		if(IS_POST){
			...
			$info['tel'] = isset($_POST['phone']) ? htmlspecialchars($_POST['phone']) : '';
 				...
 				if ($cardinfo) { //如果Member_card_create 不为空，说明领过卡，但是可以修改会员信息
 					...
 					if (!$card) {
 						...
 						if (intval($thisCard['miniscore']) == 0 || $userScore >= intval($thisCard['minscore'])) {
							...
							$data['cardid'] = $cardid;
							$records = M('Member_card_coupon_record')->where(array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'cradid' => $cardid))->select();
