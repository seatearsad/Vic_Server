...
//                }
                if ($_POST['order_type']=='recharge'){
                    $this->error(L("V3_ORDER_RESULT_PAYMENT_FAIL"),$result_url,true);  //这里已经弹出对话框，所以就不用在myinfo里弹了
                }else{
    ...
    public function secure3d(){
        ...
        if($_GET['TermUrl'] && $_GET['ACSUrl']){
            if($_GET['MD'] != '') {//3D V1
                ...
                $inLineForm = '<form name="downloadForm" method="POST" action="'.urldecode($_GET['TermUrl']).'"><input type="hidden" name="creq" value="'.urldecode($_GET['ACSUrl']).'"></form>';
