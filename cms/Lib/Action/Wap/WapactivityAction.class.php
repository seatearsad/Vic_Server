	...
	public function index(){
		...
		if($activity_area == 'all'){
			...
			if($tp_count){
				...
				$activity_list = D('')->field('`eal`.`name` AS `product_name`,`m`.`name` AS `merchant_name`,`eal`.`money` as `eal_money`,`eal`.*,`m`.*')->table(array(C('DB_PREFIX').'extension_activity_list'=>'eal',C('DB_PREFIX').'merchant'=>'m'))->where("`eal`.`status`='1' AND `eal`.`activity_term`='$term_id' AND `eal`.`mer_id`=`m`.`mer_id` AND `m`.`city_id`='{$this->config['now_city']}'".$where_activity)->order('`eal`.`is_finish` ASC,`eal`.`pigcms_id` DESC')->limit($P->firstRow.','.$P->listRows)->select();
			}
		}else{
