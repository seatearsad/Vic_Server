		...
		if(empty($this->_accessListAction[$modle])){
			...
			if($resultArr['status'] != 0 || $resultArr['message'] != 'success'){
				...
				if(empty($wxappCon)){
					...
					D('Wxapp_list')->data(array('pigcms_id'=>$wxappCon['pigcms_id'],'mer_id'=>$this->mer_id,'type'=>$modle,'id'=>$wxappInfo['modelId'],'title'=>$wxappInfo['title'],'info'=>$wxappInfo['info'],'image'=>$wxappInfo['image'],'time'=>$wxappInfo['time'],'status'=>'1'))->save();
