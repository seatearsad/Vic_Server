...
class HouseAction extends BaseAction{
	...
	public function bind_village(){
		...
		$condition['village_id'] =  $_GET['village_id'] + 0;
		if($_GET['type'] == 'go'){
			$condition['status'] =  1;
...
// 				),
	...
	public function village_my(){
		...
		$this->check_village_session($now_village['village_id']);
		if(empty($this->village_bind)){
			//$this->error_tips('',U('bind_village',array('village_id'=>$_GET['village_id'])));
			redirect(U('bind_village',array('village_id'=>$_GET['village_id'])));
		}
	...
	public function empty_bind_relatives(){
		...
		if(IS_POST){
			...
			$bind_condition_not['parent_id'] = $pigcms_id;
	...
	public function village_my_paylists(){
		...
		$order_where['village_id'] = $now_village['village_id'];
		$order_where['paid'] = 1;
		if($_GET['order_type']){
	...
	public function ajax_village_my_paylists(){
		...
		foreach($order_list as $Key=>$order){
			...
			foreach($pay_list as $pay_info){
				if($order['order_id'] == $pay_info['order_id']){
					$order_list[$Key]['start_time'] = date('Y年/m月/d日',$pay_info['start_time']);
					$order_list[$Key]['end_time'] = date('Y年/m月/d日',$pay_info['end_time']);
				}
			}
		}
		if($order_list){
				...
				foreach($property_list['list']['list'] as $property){
					if($diy_propertyt_month_num / $property['property_month_num'] > 1){
						if($property['diy_type'] == 0){
							exit(json_encode(array('status'=>1,'max_presented_property_month'=>$property['presented_property_month_num'] * intval($diy_propertyt_month_num / $property['property_month_num']),'total_price'=>$total_price,'diy_type'=>0,'property_id'=>$property['id'])));
						}else{
							exit(json_encode(array('status'=>1,'diy_content'=>$property['diy_content'],'total_price'=>$total_price,'diy_type'=>1,'property_id'=>$property['id'])));
						}
					}elseif($diy_propertyt_month_num >= $property['property_month_num']){
						if($property['diy_type'] == 0){
							exit(json_encode(array('status'=>1,'max_presented_property_month'=>$property['presented_property_month_num'],'total_price'=>$total_price,'diy_type'=>0,'property_id'=>$property['id'])));
						}else{
							exit(json_encode(array('status'=>1,'diy_content'=>$property['diy_content'],'total_price'=>$total_price,'diy_type'=>1,'property_id'=>$property['id'])));
						}
					}
				}
				exit(json_encode(array('status'=>1,'max_presented_property_month'=>0,'total_price'=>$total_price,'diy_type'=>0)));
			}else{
				exit(json_encode(array('status'=>1,'max_presented_property_month'=>0,'total_price'=>$total_price,'diy_type'=>0)));
			}
		}else{
		...
		if(IS_POST){
			...
			if($type == 0){
				...
				if($insert_id){
					...
					$bind_data['add_time'] = time();
					$bind_data['usernum'] = rand(0,99999) . '-' . time();
					$info_parentid = $database_house_village_user_bind->field('pigcms_id')->where(array('vacancy_id'=>$pigcms_id,'type'=>array("in","0,3"),'status'=>1))->find();
					$bind_data['parent_id'] =  $info_parentid['pigcms_id'];
					$bind_data['memo'] = $_POST['memo'];
					$insert_id = $database_house_village_user_bind->data($bind_data)->add();
					//$insert_id = $database_house_village_user_bind->data($bind_data)->where($bind_where)->save();
