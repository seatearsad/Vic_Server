...
// 		dump($homeInfo);die;
		...
		$cateMenuFileName = 'tpl/Wap/default/Index/menuStyle'.$radiogroup.'.php';
		...
		$this->homeInfo['info'] = str_replace(array("\n","\r\n","\"","&quot;"), '',$this->homeInfo['info']);
		$this->homeInfo['info'] = str_replace(PHP_EOL, '', $this->homeInfo['info']);
		$this->homeInfo['info'] = explode('<br>', nl2br($this->homeInfo['info'], false));
...
// 		$img->where("token = '$token' AND pigcms_id = ".intval($id))->setInc('click');
		...
		if ($weixin_bind = D('Weixin_bind')->where(array('mer_id' => $this->token))->find()) {
			$tplinfo['headerpic'] = $weixin_bind['alias'] ? "http://open.weixin.qq.com/qr/code/?username={$weixin_bind['alias']}" : $weixin_bind['qrcode_url'];
			$tplinfo['wxname'] = $weixin_bind['nick_name'];
			$tplinfo['weixin'] = $weixin_bind['alias'];
		}
