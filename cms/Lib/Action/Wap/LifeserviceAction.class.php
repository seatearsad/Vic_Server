...
class LifeserviceAction extends BaseAction{
	...
	public function query(){
		if(!in_array($_GET['type'],$this->liveServiceTypeArr)){
		...
		if($this->user_session){
			...
			if($last_order){
				// dump(unserialize($last_order['info']));
	...
	protected function recharge($id,$balance){
		...
		if($balance > $now_user['now_money']) return json_encode(array('err_code'=>98,'recharge_money'=>$balance-$now_user['now_money'],'err_msg'=>'您的帐户余额为 <span>'. $now_user['now_money'].'</span> 元，请先充值帐户余额！需要充值 '.($balance-$now_user['now_money']).' 元'));
	...
	protected function check($id){
		...
		$now_user = D('User')->get_user($this->user_session['uid']);
		$now_order = D('Service_order')->field(true)->where(array('order_id'=>$id))->find();
		if($now_order['status'] == 2) return json_encode(array('err_code'=>1006,'err_msg'=>'该订单已经缴费到帐'));
		if($now_order['status'] == 3) return json_encode(array('err_code'=>1007,'err_msg'=>'该订单充值失败，钱已经回到您的帐户余额中'));
		...
		}else if($returnArr['err_code'] == 10001){	//失败
