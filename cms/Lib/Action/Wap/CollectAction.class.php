...
class CollectAction extends BaseAction{
	public function collect(){
		...
		if(!in_array($_POST['type'],array('group_detail','group_shop','appoint_detail','worker_detail'))){
			$this->error('参数错误！请重试。');
		}
		...
		if($_POST['action'] == 'add'){
			...
			if($database_user_collect->data($data_user_collect)->add()){
				...
				}else if($_POST['type'] == 'worker_detail'){
					$condition_group['merchant_worker_id'] = $_POST['id'];
					D('Merchant_workers')->where($condition_group)->setInc('collect_num');
				}else if($_POST['type'] == 'appoint_detail'){
					$condition_group['appoint_id'] = $_POST['id'];
					D('Appoint')->where($condition_group)->setInc('collect_num');
				}
			...
			if($database_user_collect->where($condition_user_collect)->delete()){
				if($_POST['type'] == 'group_detail'){
					...
					$condition_group['merchant_worker_id'] = $_POST['id'];
					D('Merchant_workers')->where($condition_group)->setDec('collect_num');
				}else if($_POST['type'] == 'appoint_detail'){
					$condition_group['appoint_id'] = $_POST['id'];
					D('Appoint')->where($condition_group)->setDec('collect_num');
				}
				$this->success('取消收藏成功');
			}else{
