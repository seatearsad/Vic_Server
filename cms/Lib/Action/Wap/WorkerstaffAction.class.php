...
class WorkerstaffAction extends BaseAction{
    ...
    public function index(){
        ...
        $appoint_num = D('Appoint_order')->get_worker_appoint_num(false, 1);
...
//                }
    ...
    public function grab(){
        if (IS_POST){
            $database_merchant_workers = D('Merchant_workers');
            if ($user = $database_merchant_workers->where(array('uid' => $this->worker_session['merchant_worker_id']))->find()) {
