...
class IndexAction extends BaseAction{	
    public function index(){
		...
		if($this->config['many_city'] && $this->config['many_city_main_domain'].'.'.$this->config['many_city_top_domain'] == $_SERVER['HTTP_HOST'] && $this->config['now_site_url']){
			...
			header('Location: '.$this->config['now_site_url'].$_SERVER['REQUEST_URI']);
			exit();
		}
...
// 			$this->assign('son_category_list',$son_category_list);
		...
		$t = D('Merchant_store')->get_list_by_option_pc($area_id,$now_circle['area_id'], $order, $cat_id, $cat_fid, array(0, 1, 3));
// 		echo "<pre/>";
// 		print_r($t);die;
		$this->assign($t);
