 ...
 *
 */
class KuaidianAction extends BaseAction
{
    ...
    public function index()
    {
		...
		//所有分类 包含2级分类
		$all_category_list_meal = D('Meal_store_category')->get_category();
		$this->assign('all_category_list_meal',$all_category_list_meal);
...
// 		echo $_GET['cat_url'];die;
		if ($area_url != 'all') {
			$tmp_area = D('Area')->get_area_by_areaUrl($area_url, $cat_url, 'kd');
			if(empty($tmp_area)){
				$this->error('当前区域不存在！');
			}
			if($tmp_area['area_type'] == 3){
				...
				$now_area = D('Area')->get_area_by_areaId($tmp_area['area_pid'],true, $cat_url, 'kd');
				if(empty($tmp_area)){
					$this->error('当前区域不存在！');
				}
			...
			$circle_list = D('Area')->get_arealist_by_areaPid($now_area['area_id'],true, $cat_url, 'kd');
			...
			$now_area = D('Area')->get_area_by_areaUrl($area_url, $cat_url, 'kd');
			if(empty($now_area)){
				$this->error('当前区域不存在！');
			}
			$area_id = $now_area['area_id'];
...
// 			}
		...
		//所有区域
		$all_area_list = D('Area')->get_area_list('', '', 'kd');
		...
		//得到区域下的店铺列表
		$this->assign('default_url', C('config.site_url').'/kd/all/all');
		$t = D('Merchant_store')->get_list_by_option_pc($area_id,$now_circle['area_id'], $order, $cat_id, $cat_fid, 2);
// 		echo "<pre/>";
// 		print_r($t);die;
		$this->assign($t);
	...
	protected function get_cat_option_url($cat_url,$area_url,$order,$attrs){
		if($order){
			if($attrs){
				return C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/'.$order.'?attrs='.urlencode($attrs);
			}else{
				return C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/'.$order;
			}
		}else{
			if($attrs){
				return C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'?attrs='.urlencode($attrs);
			}else{
				return C('config.site_url').'/kd/'.$cat_url.'/'.$area_url;
			}
		}
	}
	protected function get_cat_sort_url($cat_url,$area_url,$attrs){
		if($attrs){
			$return['default_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'?attrs='.urlencode($attrs);
			$return['hot_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/hot?attrs='.urlencode($attrs);
			$return['price_asc_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/price-asc?attrs='.urlencode($attrs);
			$return['price_desc_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/price-desc?attrs='.urlencode($attrs);
			$return['rating_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/rating?attrs='.urlencode($attrs);
			$return['time_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/time?attrs='.urlencode($attrs);
		}else{
			$return['default_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url;
			$return['hot_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/hot';
			$return['price_asc_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/price-asc';
			$return['price_desc_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/price-desc';
			$return['rating_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/rating';
			$return['time_sort_url'] = C('config.site_url').'/kd/'.$cat_url.'/'.$area_url.'/time';
		}
		return $return;
	}
}