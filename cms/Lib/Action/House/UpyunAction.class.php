...
class UpyunAction extends BaseAction
{
	...
	public function _initialize()
	{
		...
		$this->siteUrl = $this->siteUrl ? $this->siteUrl : $this->config['site_url'];
	}
	...
	public function upload()
	{
		if ($this->upload_type == 'upyun') {
			...
			$options['return-url'] = $this->siteUrl.'/shequ.php?g=House&c=Upyun&a=uploadReturn'; /// 页面跳转型回调地址
			...
			if (IS_POST) {
				...
				echo '<script>location.href="/shequ.php?g=House&c=Upyun&a=upload&error='.$return['error'].'&msg='.$return['msg'].'";</script>';
			} else {
				if (!isset($_GET['from'])) {
					$this->display('local');
				} else {
...
//导入excel会员卡
	...
	function localUpload($path = '', $filetypes='')
	{
		$path = $path ? $path : 'card';
// 		$image = D('Image')->handle($this->house_session['village_id'], $path, 1, array('size' => 5));
// 		if ($image['error']) {
// 			return $image;
// 		} else {
// 			$this->siteUrl = $this->siteUrl ? $this->siteUrl : $this->config['site_url'];
// 			$msg = array_shift($image['url']);
