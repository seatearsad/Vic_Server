...
// 		$this->assign('defaultCity', $city_list[0]);
	...
	public function suggestion()
	{
		...
		$url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=48.43016873926502,-123.34303379055086&radius=50000&type=restaurant&keyword='.urlencode($query).'&key=AIzaSyBt3Lu-FlQE5LgusybLzqGr8lIXmvTsLZU&language=zh-CN';
		import('ORG.Net.Http');
		$http = new Http();
		...
		if($result){
			...
			if($result['status'] == 'OK' && count($result['results']) > 0){
				...
				foreach($result['results'] as $v) {
					$return[] = [
						'name' => $v['name'],
						'light-name'=>str_replace($query, "<b>$query</b>", $v['name']),
						'lat' => $v['geometry']['location']['lat'],
						'long' => $v['geometry']['location']['lng'],
						'address' => $v['vicinity']
					];
				}
				exit(json_encode(array('error_code'=>0,'data'=>$return)));	
			}else{
				exit(json_encode(array('error_code'=>1,'error_msg'=>'没有搜到相应的地址')));
			}
		}else{
			$return = array();
			$return['error_code'] = 1;
			$return['error_msg'] = 'Google地图接口错误';
