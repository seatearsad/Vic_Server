...
//团购 AJAX服务
class Home_likeAction extends BaseAction{
	public function index(){
		...
		if($content_type=='group'){
			if($page == $page_max){
				...
				$page_count = $limit-$page_count>0?$limit-$page_count:$limit;
			}else if($page>$page_max){
				$tmp_page_count  =0;
				$page_count = 0;
			}
			$new_group_list = D('Group')->get_group_list('index_sort',$page*$tmp_page_count.','.$page_count,true);
			$page_all = $page_max;
			//判断是否微信浏览器，
			$group_list = array();
			...
			$lists = D('Merchant_store_shop')->get_list_by_option($where,1);
			$page_all = $lists['total']>$page_max?$page_max:$lists['total'];
			$return = array();
			$now_time = date('H:i:s');
			$n= 1;
