...
class ChangecityAction extends BaseAction{
    public function index(){
        ...
        $arr['now_city'][]  =   isset($now_city)?$now_city:null;
        ...
        $arr['all_province']  =   isset($all_province)?$all_province:null;
        //得到推荐的城市
        $hot_city = S('hot_city');
        if(empty($hot_city)){
        ...
        $arr['hot_city']  =   isset($hot_city)?$hot_city:null;
        //得到所有城市并以城市首拼排序
        $all_city = S('all_city');
        if(empty($all_city)){
        ...
        $arr['aCity']  =    isset($tmp)?$tmp:null;
        $arr['all_city']  =   isset($all_city)?$all_city:null;;
