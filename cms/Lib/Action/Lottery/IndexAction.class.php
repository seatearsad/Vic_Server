...
class IndexAction extends BaseAction
{
    ...
    public function index()
    {
		...
		$order = isset($_GET['order']) && $_GET['order'] ? htmlspecialchars($_GET['order']) : 'id';
		$attrs = $_GET['attrs'];
...
// 		echo $_GET['cat_url'];die;
		...
		//判断分类信息
		$cat_id = 0;
		if($cat_url != 'all' && isset($category_list[$cat_url])){
...
// 			$return['hot_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/hot?attrs='.urlencode($attrs);
// 			$return['price_asc_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/price-asc?attrs='.urlencode($attrs);
// 			$return['price_desc_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/price-desc?attrs='.urlencode($attrs);
// 			$return['rating_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/rating?attrs='.urlencode($attrs);
...
// 			$return['hot_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/hot';
// 			$return['price_asc_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/price-asc';
// 			$return['price_desc_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/price-desc';
// 			$return['rating_sort_url'] = C('config.site_url').'/lottery/'.$cat_url.'/'.$area_url.'/rating';
