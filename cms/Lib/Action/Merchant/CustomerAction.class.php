...
// 		$fans_list = D('')->table($table)->where($condition)->limit($p->firstRow.','.$p->listRows)->select();
	...
	public function send()
	{
		...
		$fans_list[3] = array('id' => 0, 'name' => '全部粉丝', 'value' => 0);
		$fans_list[1] = array('id' => 1, 'name' => '商家推广获得粉丝', 'value' => 0);
		$fans_list[0] = array('id' => 2, 'name' => '扫描产品获得的粉丝', 'value' => 0);
		$fans_list[2] = array('id' => 3, 'name' => '平台赠送的粉丝', 'value' => 0);
		$total = 0;
		foreach ($count_list as $rr) {
			$total += $rr['count'];
			$fans_list[$rr['from_merchant']]['value'] = $rr['count'];
		}
		$fans_list[3]['value'] = $total;
		$this->assign('fans_list', $fans_list);
...
// 			if (empty($_POST['openids'])) {
// 				$this->error('发送对象不能为空。');
// 			}
// 			$openids = $openid_arr = $_POST['openids'];
...
// 		$condition = "`s`.`openid`=`u`.`openid` AND `s`.`log_id`='$id'";
		...
		$sql = "SELECT u.*, s.status as st FROM " . C('DB_PREFIX') . "send_user AS s LEFT JOIN  " . C('DB_PREFIX') . "user AS u ON `s`.`openid`=`u`.`openid` WHERE `s`.`log_id`={$id} LIMIT {$p->firstRow}, {$p->listRows}";
		$mode = new Model();
		$fans_list = $mode->query($sql);
