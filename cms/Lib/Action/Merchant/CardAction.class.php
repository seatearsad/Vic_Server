...
// 			M('Wxuser')->where(array('token' => $this->token))->save(array('yetcardnum'=>$cardTotal));
	...
	public function design()
	{
		...
		if (IS_POST) {
			...
			$_POST['logo']=str_replace($this->config['site_url'],'',$_POST['logo']);
			if ($data==false) {
			...
			if ($data==false) {
				...
				'logo' => '/static/images/cart_info/logo-card.png',
				'bg' => './static/images/card/card_bg15.png',
				'diybg' => '/static/images/card/card_bg17.png',
				'msg' => '微信会员卡，方便携带收藏，永不挂失',
				'numbercolor' => '#000000',
				'vipnamecolor' => '#121212',
				'Lastmsg' => '/static/images/cart_info/news.jpg',
				'vip' => '/static/images/cart_info/vippower.jpg',
				'qiandao' => '/static/images/cart_info/qiandao.jpg',
				'shopping' => '/static/images/cart_info/shopping.jpg',
				'memberinfo' => '/static/images/cart_info/user.jpg',
				'membermsg' => '/static/images/cart_info/vippower.jpg',
				'contact' => '/static/images/cart_info/addr.jpg',
				'recharge' => '/static/images/cart_info/recharge.jpg',
				'payrecord' => '/static/images/cart_info/payrecord.jpg',
				);
			}
...
// 		$can_cr_num = $markcard['allcardnum'] - $markcard['yetcardnum'];
		if (IS_POST) {
			...
			if ($num > 1000) $this->error('一次最多创建1000张');
	...
	*/
	public function exchange()
	{
		...
		if (IS_POST) {
			...
			$_POST['cardinfo2'] = fulltext_filter($_POST['cardinfo2']);
			if ($data==false) {
	...
	function noticeSet()
	{
		...
		if (IS_POST) {
			...
			if (empty($_POST['title'])) $this->error('特权名称不能为空');
			$_POST['cardid'] = $this->thisCard['id'];
			$_POST['time'] = time();
	...
	public function integral_edit()
	{
		...
		if (IS_POST) {
			$_POST['title'] = htmlspecialchars($_POST['title']);
			if (empty($_POST['title'])) $this->error('礼品名称称不能为空');
			$_POST['cardid'] = $this->thisCard['id'];
			$_POST['token'] = $this->token;
			$_POST['statdate'] = strtotime($_POST['statdate']);
	...
	public function useRecords()
	{
		...
		if ($list) {
			foreach ($list as $l) {
				...
				if ($l['staffid'] && !in_array($l['staffid'], $staffids)){
					array_push($staffids, $l['staffid']);
				}
			}
			//
			$userinfo_where['uid'] = array('in', $wecha_ids);
			$users = M('User')->where($userinfo_where)->select();
			$usersArr = array();
			if ($users) {
				foreach ($users as $u) {
					$usersArr[$u['uid']] = $u;
			...
			if ($staffids) {
				$staffWhere['id'] = array('in', $staffids);
				$staffs = M('Merchant_store_staff')->where($staffWhere)->select();
				$staffsArr = array();
				if ($staffs){
					foreach ($staffs as $s){
						$staffsArr[$s['id']]=$s;
		...
		if ($list){
			foreach ($list as &$l){
				$l['userName'] = $usersArr[$l['wecha_id']]['nickname'];
				$l['userTel'] = $usersArr[$l['wecha_id']]['phone'];
				$l['cardNum'] = $cardsArr[$l['wecha_id']]['number'];
				$l['operName'] = $staffsArr[$l['staffid']]['name'];
			}
		}
		$this->assign('page', $show);
		$this->assign('list', $list);
	...
	public function members()
	{
		...
		if ($members){
			foreach ($members as $member){
				...
				$wechat_idstr .= $pre . "'" .  $member['wecha_id'] . "'";
				$pre = ',';
			}
// 			$userinfo_db	= M('Userinfo');
...
// 			$users = $userinfo_db->where($userinfo_where)->select();
			...
			$Model = new Model();
			$sql = "SELECT u.*, i.* FROM ". C('DB_PREFIX') . "user AS u INNER JOIN ". C('DB_PREFIX') . "userinfo AS i ON u.uid=i.wecha_id WHERE i.token='{$this->token}' AND i.wecha_id IN ($wechat_idstr)";
...
// 				$members[$i]['bornday'] = $thisUser['bornday'];
				$members[$i]['tel'] = isset($thisUser['tel']) && $thisUser['tel'] ? $thisUser['tel'] : $thisUser['phone'];
	...
	public function member()
	{
		...
		//
		$thisUser = M('User')->where(array('uid'=>$thisMember['wecha_id']))->find();
		$thisUserinfo = M('Userinfo')->where(array('token'=>$thisMember['token'],'wecha_id'=>$thisMember['wecha_id']))->find();
		$thisUser = array_merge($thisUser, $thisUserinfo);
	...
	public function recharge()
	{
		$db = M('Userinfo');
		$uid = (int)$_GET['uid'];
		$uinfo = $db->where(array('token' => $this->token, 'id' => $uid))->field('wechaname, wecha_id, id')->find();
		$thisUser=M('User')->where(array('uid'=>$uinfo['wecha_id']))->find();
		$uinfo = array_merge($uinfo, $thisUser);
		if (IS_POST) {
	...
	public function signrecords(){
		...
		$user=M('User')->where(array('uid'=>$thisMember['wecha_id']))->find();
		$thisUser = array_merge($user, $thisUser);
		$this->assign('thisUser',$thisUser);
		$members[0]=$thisMember;
		$members[0]['truename']=$thisUser['truename'];
		$members[0]['wechaname']=$thisUser['wechaname'];
	...
	public function getuserinfo()
	{
		...
		$user=M('User')->where(array('uid'=>$wecha_id))->find();
		$uinfo = array_merge($user, $uinfo);
...
//删除记录
	...
	public function focusAdd()
	{
		...
		if (IS_POST) {
			...
			$image = D('Image')->handle($this->merchant_session['mer_id'], 'member_card_focus', 1);
			if (!$image['error']) {
				$data = array_merge($data, $image['url']);
			}
			if(!empty($_POST['image_select'])){
				$data['img']=$_POST['image_select'];
			}

			if ($fid) {
				$res = M('Member_card_focus')->where(array('id' => $fid, 'token' => $this->token))->save($data);
			} else {
				$fid = $res = M('Member_card_focus')->add($data);
			}
			$fid && D('Image')->update_table_id($data['img'], $fid, 'member_card_focus');
			if ($res) {
