...
// 		$this->assign('count',M('Lottery')->where($where)->count());
	...
	public function add($type)
	{
		...
		if (IS_POST) {
			...
			$image = D('Image')->handle($this->merchant_session['mer_id'], 'lottery', 1);
			if (!$image['error']) {
			...
			if ($error['error']) $this->error($error['msg']);
			if ($_POST['enddate'] < $_POST['statdate']) {
				...
				if ($data->create() != false) {
					if ($id = $data->add()) {
						...
						/*****************更新全局的图片记录**********************/
						D('Image')->update_table_id($_POST['starpicurl'], $id, 'lottery');
						D('Image')->update_table_id($_POST['endpicurl'], $id, 'lottery');
						/*****************更新全局的图片记录**********************/
						
						
						$data1['third_id'] = $id;
						$data1['third_type'] = 'lottery';
// 						$data1['token']=$this->merchant_session['mer_id'];
...
// 				}
				...
				if ($data->where($where)->save($_POST)) {
					/*****************更新全局的图片记录**********************/
					D('Image')->update_table_id($_POST['starpicurl'], $_POST['id'], 'lottery');
					D('Image')->update_table_id($_POST['endpicurl'], $_POST['id'], 'lottery');
					/*****************更新全局的图片记录**********************/
	...
	public function del()
	{
		...
		if ($back == true) {
			...
			//$type == 6 && M('Research')->where(array('lid' => $id,'token' => $this->merchant_session['mer_id']))->save(array('lid' => 0));
