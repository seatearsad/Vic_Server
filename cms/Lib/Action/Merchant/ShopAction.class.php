            ...
            if($val['uid'] == 0){//当用户ID(uid)为0时 -- 代客下单
                ...
                if($curr_order != $record_id){
                    ...
                    if($val['menu_version'] != 1) {
                        $deliverectTax = $val['price'] - $val['goods_price'] - $val['freight_charge'] - $val['freight_charge']*$val['store_tax']/100 - $val['packing_charge'] - $val['packing_charge']*$val['store_tax']/100 - $val['service_price'] - $val['coupon_price'] - $val['delivery_discount'] - $val['merchant_reduce'];
