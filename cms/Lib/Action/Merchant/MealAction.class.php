...
class MealAction extends BaseAction{
	...
	/* public function __construct() {
		...
		$sql = "SELECT s.store_id FROM " . C('DB_PREFIX') . "merchant_store AS s INNER JOIN " . C('DB_PREFIX') . "merchant_store_foodshop AS fs ON s.store_id=fs.store_id WHERE s.mer_id='{$this->merchant_session['mer_id']}' AND s.have_meal=1 AND s.status=1";
		$stores = D()->query($sql);
		if (ACTION_NAME != 'order_detail') {
			if ($stores) {
				redirect(U('Foodshop/index'));
			} else {
				$this->error('餐饮已经改版了，请您去新版去体验吧', U('Foodshop/index'));
			}
			exit;
		}
	}/*
