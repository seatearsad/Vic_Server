...
// 			array_push($items,array('id'=>$item['id'],'name'=>$item['title'],'linkcode'=> str_replace('merchant.php', 'wap.php', '{siteUrl}'. U('Wap/GoldenEgg/index', array('token' => $this->token, 'wecha_id' => '{wechat_id}', 'id' => $item['id']))),'linkurl'=>'','keyword'=>$item['keyword']));
	...
	public function Shop(){
		...
		foreach ($list as $item){
// 			array_push($items,array('id' => $item['store_id'], 'name' => $item['name'], 'linkcode'=> str_replace('merchant.php', 'wap.php', '{siteUrl}'. U('Wap/Meal/index', array('token' => $this->token, 'wecha_id' => '{wechat_id}', 'mer_id' => $item['mer_id'], 'store_id' => $item['store_id']))),'linkurl'=>'','keyword' => $item['name']));
			array_push($items,array('id' => $item['store_id'], 'name' => $item['name'], 'linkcode'=> $this->config['site_url'] . '/wap.php?g=Wap&c=Shop&a=index#shop-' . $item['store_id'],'linkurl'=>'','keyword' => $item['name']));//str_replace('merchant.php', 'wap.php', U('Wap/Article/index', array('imid' => $item['pigcms_id']), true, false, true))
		}
		$this->assign('list', $items);
		$this->assign('page', $show);
		$this->display('detail');
	...
	public function Shop_content(){
		...
		foreach ($list as $item){
			$images = $store_image_class->get_allImage_by_path($item['pic_info']);
			$img_url = $images[0];
			array_push($items,array('id' => $item['store_id'], 'name' => $item['name'], 'linkcode'=> $this->config['site_url'] . '/wap.php?g=Wap&c=Shop&a=index#shop-' . $item['store_id'],'linkurl'=>'','keyword' => $item['name'],'img_url'=>$img_url));
		}
		$this->assign('list', $items);
		$this->assign('page', $show);
		$this->display('content_detail');
	...
	public function Appoint_content(){
		...
		foreach ($list as $item){
			...
			array_push($items,array('id' => $item['store_id'], 'name' => $item['appoint_name'], 'linkcode'=> $this->config['site_url'] . '/wap.php?g=Wap&c=Shop&a=index#shop-' . $item['store_id'],'linkurl'=>'','keyword' => $item['appoint_content'],'img_url'=>$img_url));
		}
		$this->assign('list', $items);
		$this->assign('page', $show);
		$this->display('content_detail');
