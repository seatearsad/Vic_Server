...
//        $order_list = $this->formatOrderArray($order_info, $user_info, $appoint_info, $store_info);
    ...
    public function worker_edit(){
        ...
        if (IS_POST) {
            ...
            if (!$data['mobile']) {
                $this->error('联系号码不能为空！');
            } else if ((!preg_match($phone_match, $data['mobile'])) && (!preg_match($mobile_match, $data['mobile']))) {
            ...
            if ($worker_detail['office_time']) {
                $worker_detail['office_time'] = unserialize($worker_detail['office_time']);
            }
                ...
                if ($staff_arr) {
                    foreach ($staff_arr as $v) {
                        ...
                        if ($v['openid']) {
                            ...
                            $model->sendTempMsg('OPENTM201682460', array('href' => $href, 'wecha_id' => $v['openid'], 'first' => '您好，您的订单已生成', 'keyword3' => $order_id, 'keyword1' => date('Y-m-d H:i:s'), 'keyword2' => $appoint_info['appoint_name'], 'remark' => '您有新的预约，您注意查看并处理'));
                        }
                    }
                }
            }
