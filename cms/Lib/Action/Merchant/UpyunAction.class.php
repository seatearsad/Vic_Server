...
class UpyunAction extends BaseAction
{
	...
	public function upload()
	{
		if ($this->upload_type == 'upyun') {
			...
			$options['return-url'] = $this->siteUrl.'/merchant.php?g=Merchant&c=Upyun&a=uploadReturn'; /// 页面跳转型回调地址
...
//导入excel会员卡
	...
	{
		$path = $path ? $path : 'card';
// 		$image = D('Image')->handle($this->merchant_session['mer_id'], $path, 1, array('size' => 5));
// 		if ($image['error']) {
// 			return $image;
// 		} else {
// 			$this->siteUrl = $this->siteUrl ? $this->siteUrl : $this->config['site_url'];
