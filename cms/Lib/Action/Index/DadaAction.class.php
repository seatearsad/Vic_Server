            ...
            if (isset($result['signature']) && md5($str) == $result['signature']) {
                ...
                switch ($order_status) {
                    ...
                    case 7://已过期
                        ...
                        $shopOrderDB->where(array('order_id' => $row['order_id']))->save(array('status' => 5, 'last_time' => $result['update_time'], 'cancel_reason' => $cancel_reason, 'cancel_type' => 6, 'is_rollback' => 1));//取消类型（0:pc店员，1:wap店员，2:andriod店员,3:ios店员，4：打包app店员，5：用户，6：配送员, 7:超时取消）
                        
                        $orderLogDB->add_log(array('order_id' => $row['order_id'], 'status' => 10, 'name' => $dm_name, 'phone' => $dm_mobile));
                        if (($order['paid'] == 1 || $order['reduce_stock_type'] == 1) && $order['is_rollback'] == 0) {
                            $details = $orderDetailDB->field(true)->where(array('order_id' => $order['order_id']))->select();
