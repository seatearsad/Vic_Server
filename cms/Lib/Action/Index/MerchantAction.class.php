...
class MerchantAction extends BaseAction {
    ...
    protected function _initialize() {
        ...
        foreach ($navmanag as $nmvv) {
            if (($nmvv['url'] == 'mergoods') && ($merchant_mstore['have_shop'] != 1))
                continue; /*                 * 没有开通订餐去掉导航* */
        ...
        $merchantarr['imgs'] = !empty($merchantarr['pic_info']) ? $this->get_allImage_by_path($merchantarr['pic_info'],true) : '';
		...
		if($this->config['many_city'] && $this->config['many_city_main_domain'].'.'.$this->config['many_city_top_domain'] == $_SERVER['HTTP_HOST'] && $this->config['now_site_url']){
			$now_city = D('Area')->field('`area_id`,`area_name`,`area_url`')->where(array('area_id'=>$merchantarr['city_id']))->find();
		...
		$mstore_meal = D('Merchant_store_shop')->where(array('store_id' =>$merchant_mstore['store_id']))->field('store_id,send_time')->find();
...
//         $meal_Db = D('Meal');
    ...
    private function get_allImage_by_path($path,$is_array = false) {
		if(!strpos($path,';') && $is_array == false){
