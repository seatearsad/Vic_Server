...
class ActivityAction extends BaseAction{
	public function index() {
		...
		if($now_activity['begin_time'] > $_SERVER['REQUEST_TIME']){
			...
			$time = $now_activity['begin_time'] - $_SERVER['REQUEST_TIME'];
		...
		if($activity_area == 'all'){
			$tp_count = D('')->table(array(C('DB_PREFIX').'extension_activity_list'=>'eal',C('DB_PREFIX').'merchant'=>'m'))->where("`eal`.`status`='1' AND `eal`.`activity_term`='$term_id' AND `eal`.`mer_id`=`m`.`mer_id` AND `m`.`city_id`='{$this->config['now_city']}'".$where_activity)->count();
			if($tp_count){
				import('@.ORG.activity_page');
				$P = new Page($tp_count,20,'page');
				$activity_list = D('')->field('`eal`.`name` AS `product_name`,`eal`.`money` AS `eal_money`,`m`.`name` AS `merchant_name`,`eal`.*,`m`.*')->table(array(C('DB_PREFIX').'extension_activity_list'=>'eal',C('DB_PREFIX').'merchant'=>'m'))->where("`eal`.`status`='1' AND `eal`.`activity_term`='$term_id' AND `eal`.`mer_id`=`m`.`mer_id` AND `m`.`city_id`='{$this->config['now_city']}'".$where_activity)->order('`eal`.`is_finish` ASC,`eal`.`pigcms_id` DESC')->limit($P->firstRow.','.$P->listRows)->select();
			}
		}else{
			...
			if($tp_count){
				...
				$activity_list = D('')->field('`eal`.`name` AS `product_name`,`eal`.`money` AS `eal_money`,`m`.`name` AS `merchant_name`,`eal`.*,`m`.*')->table(array(C('DB_PREFIX').'extension_activity_list'=>'eal',C('DB_PREFIX').'merchant'=>'m',C('DB_PREFIX').'merchant_store'=>'ms'))->where("`eal`.`status`='1' AND `eal`.`activity_term`='$term_id' AND `eal`.`mer_id`=`m`.`mer_id` AND `ms`.`area_id`='$area_id' AND `ms`.`mer_id`=`m`.`mer_id`".$where_activity)->group('`eal`.`pigcms_id`')->order('`eal`.`is_finish` ASC,`eal`.`pigcms_id` DESC')->limit($P->firstRow.','.$P->listRows)->select();
			}
		}
		if($activity_list){
	...
	public function submit(){
		...
		if(!empty($now_activity['mer_score']) && !in_array($_POST['score_type'],array('1','2'))){
			exit(json_encode(array('status'=>0,'info'=>'该活动是以'.$this->config['score_name'].'兑换，但是您提交的'.$this->config['score_name'].'类别有误')));
		}
		...
		if($now_activity['type'] == 1 && !M('User_adress')->where(array('uid'=>$this->user_session['uid']))->find()){
			exit(json_encode(array('status'=>-5,'info'=>'您必须添加一个收货地址才能进行一元夺宝')));
		}
		...
		if($now_activity['money'] != 0){
			...
			if($_POST['score_type'] == '2'){
				...
				if($user_mer_score < $use_score){
					exit(json_encode(array('status'=>0,'info'=>'您的商家会员卡'.$this->config['score_name'].'为 '. $user_mer_score.' ，不足以兑换此奖品')));
				}
		...
		if($save_ok){
			...
			switch($now_activity['type']){
				case '1':
					$result = $this->_yiyuanduobao($now_activity,$quantity,$record_id,$now_user);
					if(I('r')){
						redirect(U('Wap/Wapactivity/detail',array('id'=>$now_activity['pigcms_id'],'r'=>1)));die;
					}else{
						exit(json_encode(array('status'=>1,'info'=>'参与成功')));
					}
					break;
				case '2':
					$result = $this->_coupon($now_activity,$quantity,$record_id,$now_user['uid']);
					exit(json_encode(array('status'=>1,'info'=>'参与成功')));
					break;
			}
		}else{
	...
	private function _coupon($now_activity,$quantity,$record_id,$uid){
		...
		D('User')->add_score($uid, round($quantity*$now_activity['money']* $this->config['score_get']), '在购买优惠券消费' .$quantity*$now_activity['money']. '元 获得'.$this->config['score_name']);
		D('Scroll_msg')->add_msg('yydb',$uid,'用户'.$now_user['nickname'].'于'.date('Y-m-d H:i',$_SERVER['REQUEST_TIME']).  '在购买优惠券获得'.$this->config['score_name']);
	...
	private function _yiyuanduobao($now_activity,$quantity,$record_id,$now_user){
		//此处有三种方法，采用最简单的一种。
		//第一种 按上一次的号码累加
		...
		for($i=0;$i<$quantity;$i++){
			...
			$numbers[]=$data_extension_yiyuanduobao_record['number']+10000000;
		...
		if($quantity>3){
			$sms_data['content'] = '您好，您本次参与的一元夺宝【'.$now_activity['name'].'】已成功购买，本次的夺宝号码为:（'.$numbers[0].','.$numbers[1].','.$numbers[2].'等），其他号码登录个人中心查看，欢迎下次选购。';
		}else{
			$sms_data['content'] = '您好，您本次参与的一元夺宝【'.$now_activity['name'].'】已成功购买，本次的夺宝号码为:（'.implode(',',$numbers).'），请您耐心等待开奖，欢迎下次选购。';
		}
//单独升级
//			$sms_data['content'] = '您好，您本次参与的一元夺宝【'.$now_activity['name'].'】已成功购买，本次的夺宝号码为:（'.implode(',',$numbers).'），请您耐心等待开奖！';
//
		if($this->config['yydb_sms_enable']==1){
			Sms::sendSms($sms_data);
		}
		D('User')->add_score($now_user['uid'], round($quantity* $this->config['score_get']), '在一元夺宝中消费' .$quantity. '元 获得'.$this->config['score_name']);
		D('Scroll_msg')->add_msg('yydb',$now_user['uid'],'用户'.$now_user['nickname'].'于'.date('Y-m-d H:i',$_SERVER['REQUEST_TIME']).  '在一元夺宝中消费获得'.$this->config['score_name']);
		//抽奖
		//取50条最新购买记录进行判断抽奖
		if($now_activity['all_count'] - $now_activity['part_count'] > $quantity){
			return false;
		...
		$sms_data['sendto'] = 'user';
		$sms_data['content'] = '恭喜您成为一元夺宝【'.$now_activity['name'].'】的幸运者，幸运号码为（'.$lottery_number.'），我们会尽快和您联系，把宝贝送到您的手中。';
		Sms::sendSms($sms_data);
		...
		//售出记入收入
		if($lottery_user['phone']){
			$now_order['order_type'] = 'yydb';
			$now_order['order_id'] = $lottery_user['phone'];
			$now_order['name']        = $now_activity['all_count'];
			$now_order['mer_id'] =$now_activity['mer_id'];
			$now_order['money'] = $now_activity['price'];
			D('Merchant_money_list')->add_money($now_activity['mer_id'],'一元夺宝出售成功记入收入',$now_order);

			//商家推广分佣
			$now_user = M('User')->where(array('uid'=>$lottery_user['uid']))->find();
	...
	public function yiyuan_number(){
		...
		if($record_list){
			foreach($record_list as $value){
				array_push($returnArr,$value['number']+10000000);
			}
			shuffle($returnArr);
