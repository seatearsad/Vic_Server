...
class IndexAction extends BaseAction{
    public function index(){
		...
		//所有分类 页面上
		$list_all_category_list = D('Appoint_category')->get_all_category();
		...
		if($cat_url != 'all'){
			...
			$son_category_list = D('Appoint_category')->get_son_category_list_byid($now_category['cat_fid'],$now_category['cat_id']);
...
//			$like_group_list = D('Group')->get_grouplist_by_catId(0,0,5);
		...
		$group_category_all = C('config.site_url').'/appoint/category/all/all';
		$this->assign('group_category_all',$group_category_all);
//		$this->assign('like_group_list',$like_group_list);

                
		//得到分类下的团购列表
		$this->assign(D('Appoint')->get_appoint_list_by_catid($get_grouplist_catid,$get_grouplist_catfid,$cat_url,$area_id,$now_circle['area_id'],$order,$attrs,$category_cat_field));
	...
	protected function get_cat_option_url($cat_url,$area_url,$order,$attrs,$p_url=''){
		if($order){
			if($attrs){
				return C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/'.$order.'?attrs='.urlencode($attrs);
			}else{
				return C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/'.$order;
			}
		}else{
			if($attrs){
				return C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'?attrs='.urlencode($attrs);
			}else if(!empty($area_url)){
				return C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url;
			}else{
				return C('config.site_url').'/appoint/category/'.$cat_url.'/'.$p_url;
			}
		}
	}
	protected function get_cat_sort_url($cat_url,$area_url,$attrs){
		if($attrs){
			$return['default_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'?attrs='.urlencode($attrs);
			$return['hot_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/hot?attrs='.urlencode($attrs);
			$return['hit_asc_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/hit-asc?attrs='.urlencode($attrs);
			$return['hit_desc_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/hit-desc?attrs='.urlencode($attrs);
			$return['time_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/time?attrs='.urlencode($attrs);
		}else{
			$return['default_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url;
			$return['hot_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/hot';
			$return['hit_asc_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/hit-asc';
			$return['hit_desc_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/hit-desc';
			$return['time_sort_url'] = C('config.site_url').'/appoint/category/'.$cat_url.'/'.$area_url.'/time';
		}
		return $return;
	}
}