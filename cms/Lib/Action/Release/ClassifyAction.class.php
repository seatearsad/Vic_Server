...
class ClassifyAction extends BaseAction {
    ...
    public function Subdirectory() {
        ...
        if ($cid > 0) {
            ...
            foreach ($Subdirectory2 as $vv) {
                ...
                $tmps = $userinputDb->field('id,uid,cid,fcid,sub3dir,title,imgs')->where(array('cid' => $vv['cid'], 'city_id'=>$this->config['now_city'] ,'status' => 1))->order('imgs DESC,toptime DESC,updatetime DESC')->limit('0,7')->select();
                $vv['userinput'] = $tmps;
                $newtmp[$vv['cid']] = $vv;
            }
            unset($tmps);
    ...
    public function Lists($data = false) {
        ...
        if ($cid > 0) {
            ...
            $mywhere = array();
            $wherestr = $sub3dir > 0 ? 'cid=' . $cid . " AND `city_id`=".$this->config['now_city'].' AND sub3dir=' . $sub3dir . " AND status=1" : 'cid=' . $cid . " AND `city_id`='".$this->config['now_city']."' AND status=1";
            ...
            if ($categorys) {
                ...
                if (!empty($categorys)) {
                    ...
                    foreach ($categorys as $kk => $vv) {
                        $tmp = 0;
                        if ($vv['subdir'] == 2) {
                            $tmp = $Classify_userinputDb->where(array('cid' => $vv['cid'] ,'city_id'=>$this->config['city_id'], 'fcid' => $vv['fcid'], 'status' => 1))->count();
                            //if($tmp>0){
    ...
    public function ShowDetail() {
        ...
        if ($vid > 0) {
            ...
            if (!empty($tmpdata)) {
                if($tmpdata['is_assure'] > 0){
                    $this->error('您好，该信息请到手机端进行操作！');
                }
    ...
    public function ajax_upload_pic() {
        ...
        if ($_FILES['file']['error'] != 4) {
        	...
        	$param = array('size' => $this->config['reply_pic_size'], 'thumbMaxWidth' => $this->config['reply_pic_width'], 'thumbMaxHeight' => $this->config['reply_pic_height'], 'thumb' => false, 'imageClassPath' => 'ORG.Util.Image', 'thumbPrefix' => 'm_,s_', 'thumbRemoveOrigin' => false);
        	
			$image = D('Image')->handle($dom_id, 'classify', 2, $param);
			
			if (!$image['error']) {
				$this->dexit(array('error' => 0, 'id' => $dom_id, 'urlpath' => $this->config['site_url'] . $image['url']['file'], 'imgurl' => $image['url']['file'],'data' => $image['url']));
			} else {
				$this->dexit(array('error' => 1, 'id' => $dom_id, 'urlpath' => $this->config['site_url'] . $image['url']['file'], 'imgurl' => $image['url']['file'],'data' => $image['msg']));
			}
