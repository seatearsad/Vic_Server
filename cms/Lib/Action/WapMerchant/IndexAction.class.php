...
class IndexAction extends BaseAction {
    ...
    private function getallincomecount() {
        $tmp_m_price = D('Foodshop_order')->where('mer_id=' . $this->merid . ' AND (status = 3 OR status = 4)')->field('sum(price) as tprice')->find();
        $tmp_m_price['tprice'] = number_format($tmp_m_price['tprice']);
        $meal_price = $tmp_m_price['tprice'] ;
        $tmp_g_price = $this->group_orderDb->where('mer_id=' . $this->merid . ' AND paid="1" AND status != 3')->field('sum(total_money) as tprice')->find();
        $group_price = $tmp_g_price['tprice'] ;
        $tmp_g_price = $this->appoint_orderDb->where('mer_id=' . $this->merid . ' AND paid="1" AND status != 3')->field('sum(pay_money) as tprice')->find();
