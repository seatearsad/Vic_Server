...
class PackimageAction extends BaseAction{
	...
	public function shop(){
		$new_meal_dir = $this->new_dir.'goods/';
		$goods_list = D('Shop_goods')->field('`image`')->select();
		foreach($goods_list as $value){
			if(!empty($value['image'])){
				$image_tmp = explode(',',$value['image']);
				$this->mknewdir($new_meal_dir.$image_tmp[0]);
				$this->newcopy('./upload/goods/'.$image_tmp[0].'/'.$image_tmp[1],$new_meal_dir.$image_tmp[0].'/'.$image_tmp[1]);
				$this->newcopy('./upload/goods/'.$image_tmp[0].'/m_'.$image_tmp[1],$new_meal_dir.$image_tmp[0].'/m_'.$image_tmp[1]);
				$this->newcopy('./upload/goods/'.$image_tmp[0].'/s_'.$image_tmp[1],$new_meal_dir.$image_tmp[0].'/s_'.$image_tmp[1]);
			}
		}
		// exit;
