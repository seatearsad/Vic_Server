...
class ClassifyAction extends BaseAction {
    ...
    public function cat_field_del(){
        if(IS_POST){
            ...
            if($Classify_categoryDb->data($data_classify_category)->save()){
                $this->success('删除成功！');
            }else{
                $this->error('删除失败！请重试~');
            }
    ...
    public function cat_field_modify() {
        ...
        if (IS_POST) {
            ...
            if (empty($_POST['use_field'])) {
                ...
                $post_data['iswrite'] = $_POST['iswrite'] ? $_POST['iswrite'] : 0;
                $post_data['input'] = $mc;
    ...
    public function checkList() {
		...
		$city_list = D('Area')->field('`area_id`,`area_name`')->where(array('area_type'=>'2','is_open'=>'1'))->select();
    ...
    public function assure_order_list(){
        $condition_table = array(C('DB_PREFIX').'classify_order'=>'o', C('DB_PREFIX').'user'=>'u');
        $condition_where = '`o`.`uid` = `u`.`uid`';
        $order_count = D('')->where($condition_where)->table($condition_table)->count();
        import('@.ORG.system_page');
        $p = new Page($order_count,30);

        $order_list = D('')->field('`u`.`nickname`,`o`.*')->where($condition_where)->table($condition_table)->order('`o`.`order_id` desc')->limit($p->firstRow.','.$p->listRows)->select();
    ...
    public function order_detail(){
        ...
        $condition_table = array(C('DB_PREFIX').'classify_order'=>'`o`',C('DB_PREFIX').'user'=>'`u`');
        $condition = '`o`.`uid` = `u`.`uid` AND `o`.`order_id`='.$order_id;
        $fields = array('`o`.*,`u`.`nickname`,`u`.`phone` as `user_phone`');
        $now_order = D('')->table($condition_table)->field($fields)->where($condition)->find();
        if(empty($now_order)){
            $this->error_tips('该订单不存在！');
        }

        $database_user = D('User');
        if(!$now_order['seller_user_id']){
            $database_classify_userinput = D('Classify_userinput');
            $seller_user_id = $database_classify_userinput->where(array('id'=>$now_order['classify_userinput_id']))->getField('uid');
            $now_order['seller_user_info'] = $database_user->get_user($seller_user_id);
        }else{
            $now_order['seller_user_info'] = $database_user->get_user($now_order['seller_user_id']);
        }
    ...
    public function ajax_chk_order(){
        if(IS_AJAX){
            ...
            $now_order = $database_classify_order->where($where)->find();
            ...
            $insert_id = $database_classify_order->where($where)->data($classify_order_data)->save();

            if($insert_id){
                exit(json_encode(array('status'=>1,'msg'=>'确认成功！')));
            }else{
                exit(json_encode(array('status'=>0,'msg'=>'确认失败！')));
            }
        }else{
