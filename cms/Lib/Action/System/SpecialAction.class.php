...
class SpecialAction extends BaseAction{
	...
	public function choose_shop(){
		...
		$count_store = M()->table(array(C('DB_PREFIX').'merchant_store'=>'s',C('DB_PREFIX').'merchant_store_shop'=>'ss'))->where("`s`.`store_id`=`ss`.`store_id` AND `s`.`status`='1' AND `s`.`have_shop`='1'")->count();

		import('@.ORG.system_page');
		$p = new Page($count_store, 30);

		$store_list = M()->field("`s`.`store_id`, `s`.`pic_info`, `s`.`name`, `s`.`adress`, `s`.`phone`, `s`.`sort`,`ss`.`sale_count`,`ss`.`deliver_type`")->table(array(C('DB_PREFIX').'merchant_store'=>'s',C('DB_PREFIX').'merchant_store_shop'=>'ss'))->where("`s`.`store_id`=`ss`.`store_id` AND `s`.`status`='1' AND `s`.`have_shop`='1'")->order("`s`.`sort` DESC,`s`.`store_id` ASC")->limit($p->firstRow . ',' . $p->listRows)->select();
