	...
	public function class_add()
	{
		if (IS_POST) {
			...
			if($_POST['menu_type']==1 && $_POST['keyword'] != ''){
				$data['url'] = $data['wxsys'] = '';
			}elseif($_POST['menu_type']==2 && $_POST['url'] != ''){
				$data['keyword'] = $data['wxsys'] = '';
			}elseif($_POST['menu_type']==3 && $_POST['wxsys'] != ''){
				$data['keyword'] = $data['url'] = '';
			}
			
			$data['mer_id'] = -1;
			if (D("Diymenu_class")->add($data)) $this->success('添加成功');
		} else {
...
// 			$count = M('Diymenu_class')->where(array('pid' => $set['pid']))->count();
// 			if ($set['pid'] == 0 && $count >= 3) {
// 				$this->error('1级菜单最多只能开启3个');
// 			} elseif ($set['pid'] && $count >= 5) {
// 				$this->error('2级子菜单最多开启5个');
// 			}
	...
	public function class_send()
	{
		if(IS_GET){
			...
			$class=M('Diymenu_class')->where(array('pid'=>0,'is_show'=>1, 'mer_id' => -1))->limit(3)->order('sort desc,id ASC')->select();//dump($class);
