...
class UpdatesysAction extends BaseAction {
	public function comein(){
		redirect('https://o2o-service.pigcms.com/workorder/check.php?domain='.$_SERVER['SERVER_NAME'].'&soft_version='.$this->config['system_version']);
	}
	public function index(){
		...
		import('ORG.Net.Http');
		$http = new Http();	
		$returnArr = Http::curlPost('https://o2o-service.pigcms.com/update.php', $data);
		if(isset($returnArr['error_code'])){
			...
			$this->error_tips('升级发生异常！请联系售后进行协助解决',3,U('Index/main'));
		}
	}
	...
	public function workorder(){
		...
		import('ORG.Net.Http');
		$http = new Http();	
		$returnArr = Http::curlPost('https://o2o-service.pigcms.com/workorder/login.php', $data);
		if(isset($returnArr['error_code'])){
			if($returnArr['error_code']){
				$this->error_tips($returnArr['error_msg'],10,U('Index/main'));
			}else{
				redirect('https://o2o-service.pigcms.com/workorder/login.php?workorder_key='.$returnArr['workorder_key'].'&private_key='.$returnArr['private_key']);
			}
		}else{
