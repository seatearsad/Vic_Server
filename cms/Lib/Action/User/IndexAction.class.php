...
//					}
	...
	public function meal_order_check_refund()
	{
		...
		if($now_order['pay_type'] == 'offline'){
			...
			if(D('Meal_order')->data($data_meal_order)->save()){
				...
				$op->printit($this->mer_id, $now_order['store_id'], $msg, 3);

				$str_format = ArrayToStr::print_format($now_order['order_id']);
				foreach ($str_format as $print_id => $print_msg) {
					$print_id && $op->printit($this->mer_id, $now_order['store_id'], $print_msg, 3, $print_id);
				}
	...
	public function shop_list()
	{
		...
		$orders = D("Shop_order")->get_order_list($where);
	...
	public function gift_list(){
		...
		$orders = $database_gift->get_order_list($this->now_user['uid'] , $status , false);
