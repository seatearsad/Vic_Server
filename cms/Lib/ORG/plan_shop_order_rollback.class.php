...
class plan_shop_order_rollback extends plan_base
{
	...
	{
		$time = time();
		$sql = "SELECT o.* FROM " . C('DB_PREFIX') . "shop_order AS o INNER JOIN " . C('DB_PREFIX') . "merchant_store_shop AS s ON s.store_id=o.store_id WHERE o.paid=0 AND o.reduce_stock_type=1 AND o.is_rollback=0 AND (s.rollback_time>0 AND {$time}-o.create_time>s.rollback_time*60)";
		$res = D()->query($sql);
		...
		foreach ($res as $row) {
			...
			$order_log_db->add_log(array('order_id' => $row['order_id'], 'status' => 10, 'name' => '超时没有买单系统自动取消', 'phone' => ''));
