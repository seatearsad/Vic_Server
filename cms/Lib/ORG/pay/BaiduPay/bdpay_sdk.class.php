...
// 	const SP_KEY_FILE = 'sp.key';
	...
	// 日志文件
	const LOG_FILE = './runtime/sdk.log';	
	...
	public function __construct($sp_no, $key) 
	{
		$this->sp_no = $sp_no;
		$this->key = $key;
	}
	...
	/**
	 * 生成百度钱包H5端即时到账支付接口对应的URL
	 *
	 * @param array $params	生成订单的参数数组，具体参数的取值参见接口文档
	 * @param string $url   百度钱包H5端即时到账支付接口URL
	 * @return string 返回生成的百度钱包H5端即时到账支付接口URL
	 */
	function create_baifubao_pay_order_url($params, $url) {
		//file_put_contents('./t.php', "1111");
		...
		if (!in_array($url, array(self::BFB_PAY_DIRECT_LOGIN_URL, self::BFB_PAY_WAP_DIRECT_NEEDLOGIN_URL, self::BFB_QUERY_ORDER_URL,))) {
			//$this->log(sprintf('invalid url[%s], bfb just provide three kind of pay url', $url));
			return false;
		}
		...
		$this->log(sprintf('the params that create baifubao pay order is [%s]', $params_str));
	...
	function check_bfb_pay_result_notify() {
		...
		//unset($_GET['paytype'], $_GET['c'], $_GET['a'], $_GET['g']);
		if (empty($_GET) || !isset($_GET['sp_no']) || 
				 !isset($_GET['order_no']) || !isset($_GET['bfb_order_no']) ||
		 	...
		 	return array('error' => 1, 'msg' => 'return_url页面的请求的必选参数不足');
			$this->err_msg = 'return_url页面的请求的必选参数不足';
			$this->log(sprintf('missing the params of return_url page, params[%s]', print_r($_GET)));
		}
		$arr_params = $_GET;
		$this->order_no = $arr_params['order_no'];
		// 检查商户ID是否是自己，如果传过来的sp_no不是商户自己的，那么说明这个百度钱包的支付结果通知无效
		if ($this->sp_no != $arr_params['sp_no']) {
			return array('error' => 1, 'msg' => '百度钱包的支付结果通知中商户ID无效，该通知无效');
			$this->err_msg = '百度钱包的支付结果通知中商户ID无效，该通知无效';
			$this->log('the id in baifubao notify is wrong, this notify is invaild');
			return false;
		}
		// 检查支付通知中的支付结果是否为支付成功
		if (self::BFB_PAY_RESULT_SUCCESS != $arr_params['pay_result']) {
			return array('error' => 1, 'msg' => '百度钱包的支付结果通知中商户支付结果异常，该通知无效');
			$this->err_msg = '百度钱包的支付结果通知中商户支付结果异常，该通知无效';
			$this->log('the pay result in baifubao notify is wrong, this notify is invaild');
			return false;
		}
...
/*echo "<Pre/>";
		print_r($arr_params);die;
		$config_file = './bd.php';
		$fp = fopen($config_file, 'a+');
		fwrite($fp, "check_bfb_pay_result_notify: \n");
		fwrite($fp, "\nreturn " . stripslashes(var_export($arr_params, true)) . ";");
		fclose($fp);*/
			return array('error' => 1, 'msg' => '百度钱包后台通知签名校验失败');
			$this->err_msg = '百度钱包后台通知签名校验失败';
			$this->log('baifubao notify sign failed');
			return false;
		}
		return array('error' => 0, 'params' => $arr_params);
...
// 		$this->log('baifubao query pay result by order_no success');
	 ...
	 */
	function notify_bfb() {
		$rep_str = "<html><head>" . self::BFB_NOTIFY_META .  "</head><body><h1>这是一个return_url页面</h1></body></html>";
		echo "$rep_str";
	}
	 ...
	 * @return int 如果订单处于等待支付状态，返回self::SP_PAY_RESULT_WAITING
	 *         其它情况用户也可以自己定义
	 */
	private function query_order_state($order_no) {
		 ...
		 */
		return self::SP_PAY_RESULT_WAITING;
	}
	...
	function query_baifubao_pay_result_by_order_no($order_no) {
		$params = array (
			'service_code' => self::BFB_QUERY_INTERFACE_SERVICE_ID, // 查询接口的服务ID号
			'sp_no' => $this->sp_no,
			'order_no' => $order_no,
			'output_type' => self::BFB_INTERFACE_OUTPUT_FORMAT, // 百度钱包返回XML格式的结果
			'output_charset' => self::BFB_INTERFACE_ENCODING, // 百度钱包返回GBK编码的结果
			'version' => self::BFB_INTERFACE_VERSION,
		...
		if (false === ($sign = $this->make_sign($params))) {
			$this->log('make sign for query baifubao pay result interface failed');
			return false;
		}
		$params ['sign'] = $sign;
		$params_str = http_build_query($params);
		
		$query_url = self::BFB_QUERY_ORDER_URL . '?' . $params_str;
		$this->log(sprintf('the url of query baifubao pay result is [%s]', $query_url));
		$content = $this->request($query_url);
		$retry = 0;
		while (empty($content) && $retry < self::BFB_QUERY_RETRY_TIME) {
			$content = $this->request($query_url);
			$retry++;
		}
		if (empty($content)) {
		...
		$this->log(sprintf('the result from baifubao query pay result is [%s]', $content));
		...
		if (empty($response_arr) || !isset($response_arr ['query_status']) ||
			...
			$this->err_msg = sprintf('百度钱包的订单查询接口查询失败，返回数据为[%s]', print_r($response_arr, true));
			return false;
		}
		// 检查订单查询接口的响应数据中查询状态query_status是否为0，0代表查询成功
		if (0 != $response_arr ['query_status']) {
			$this->log(sprintf('query the baifubao pay result interface faild, the query_status is [%s]', $response_arr ['query_status']));
			$this->err_msg = sprintf('百度钱包的订单查询接口查询失败，查询状态为[%s]', $response_arr ['query_status']);
			return false;
		}
		// 检查商户ID是否是自己，如果传过来的sp_no不是商户自己的，那么说明这个百度钱包的订单查询接口的响应数据无效
		if ($this->sp_no != $response_arr['sp_no']) {
			$this->log('the sp_no returned from baifubao pay result interface is invaild');
			$this->err_msg = '百度钱包的订单查询接口的响应数据中商户ID无效，该通知无效';
			return false;
		}
		// 检查订单查询接口的响应数据中的支付结果是否为支付成功
		if (self::BFB_PAY_RESULT_SUCCESS != $response_arr ['pay_result']) {
			$this->log(sprintf('the pay result returned from baifubao pay result interface is invalid, is [%s]', $response_arr ['pay_result']));
		...
		if (false === $this->check_sign($response_arr)) {
			$this->log('sign the result returned from baifubao pay result interface failed');
	...
	function create_baifubao_Refund_url($params, $url) 
	{
		...
		if (!in_array($url, array(self::BFB_REFUND_URL, self::BFB_REFUND_QUERY_URL))) {
			$this->log(sprintf('invalid url[%s], bfb just provide three kind of pay url', $url));
			return false;
		}
	...
	{
		// 检查请求的必选参数，具体的参数参见接口文档
		if (empty($_GET) || !isset($_GET ['bfb_order_no']) || 
				!isset($_GET['cashback_amount']) || !isset($_GET['order_no']) ||
				!isset($_GET['ret_code']) ||
				...
				// 检查商户ID是否是自己，如果传过来的sp_no不是商户自己的，那么说明这个百度钱包的支付结果通知无效
				if ($this->sp_no != $arr_params ['sp_no']) {
					return array('error' => 1, 'msg' => '百度钱包退款通知中商户ID无效，该通知无效');
	...
	function query_baifubao_refund_result_by_order_no($order_no) {
		...
		if (false === ($sign = $this->make_sign($params))) {
			$this->log('make sign for query baifubao refund result interface failed');
			return false;
		}
		$params ['sign'] = $sign;
		$params_str = http_build_query($params);
	
		$query_url = self::BFB_REFUND_QUERY_URL . '?' . $params_str;
		$this->log(sprintf('the url of query baifubao refund result is [%s]', $query_url));
		$content = $this->request($query_url);
		$retry = 0;
		while (empty($content) && $retry < self::BFB_QUERY_RETRY_TIME) {
			$content = $this->request($query_url);
	...
	function query_baifubao_refund_result_by_sprefund_no($order_no,$sp_refund_no) {
		...
		if (isset($response_arr ['ret_details'])) {
			$response_arr ['ret_details'] = iconv("UTF-8", "GBK", $response_arr ['ret_details']);
		}
	...
	private function make_sign($params) {
		if (is_array($params)) {
			...
			if (ksort($params)) {
				...
				// 选择相应的加密算法
				if ($params ['sign_method'] == self::SIGN_METHOD_MD5) {
					return md5($sign_str);
				} else if ($params ['sign_method'] == self::SIGN_METHOD_SHA1) {
	...
	function request($url) {
		...
		if (false === $res || !empty($err)) {
			...
			$this->log(sprintf('curl the baifubao pay result interface failed, err_msg [%s]', $info));
			$this->err_msg = $info;
			return false;
		}
	...
	function log($msg) {
		if(defined(self::LOG_FILE)) {
			//error_log(sprintf("[%s] [order_no: %s] : %s\n", date("Y-m-d H:i:s"), $this->order_no, $msg));
		} else {
			//error_log(sprintf("[%s] [order_no: %s] : %s\n", date("Y-m-d H:i:s"), $this->order_no, $msg), 3, self::LOG_FILE);
		}
	}
}
