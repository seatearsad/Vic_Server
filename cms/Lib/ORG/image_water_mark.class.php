...
// 		$_config['setting']['imagelib'] = 0;
// 		$_config['setting']['imageimpath'] = '';
// 		$_config['setting']['thumbquality'] = 80;
		
// 		$watermarkfile = WEILOON_ROOT . 'data/attachment/watermark/' . $this->_account['bind_username'] . '_' . $this->_type . '.' . $watermark['type'];
// 		$watermarksize = @getimagesize($watermarkfile);
		
// 		$watermark['watermarkstatus'] = $watermark['pos']; 位置
// 		$watermark['watermarktype'] = $watermark['type'];  水印的后缀
// 		$watermark['watermarkfile'] = $watermarkfile;	        水印文件
// 		$watermark['watermarkminwidth'] = $watermarksize[0];水印文件的宽度
// 		$watermark['watermarkminheight'] = $watermarksize[1];水印文件的高度度
...
// 		return $watermark;
	...
	public function Watermark($source, $param, $target = '') {
	...
	public function init($method, $source, $target, $nosuffix = 0) {
		...
		if(isset($parse['host'])) {
			...
			$attachdir  = './upload/';//TODO
			$this->tmpfile = $source = tempnam($attachdir .'./temp/', 'tmpimg_');
		...
		if($method == 'thumb') {
			...
			$target = empty($target) ?  $source : $attachdir .'./'.$target;
		}
		...
		switch($this->param['thumbtype']) {
			...
			case 2:
				if(!($this->imginfo['width'] < $this->param['thumbwidth'] || $this->imginfo['height'] < $this->param['thumbheight'])) {
                    ...
                    $startx = ($this->startX) ? $this->startX : $startx;
                    $starty = ($this->startY) ? $this->startY : $starty;
	...
	public function Watermark_GD() {
		...
		if($wmwidth > 10 && $wmheight > 10 && !isset($this->imginfo['animated'])) {
