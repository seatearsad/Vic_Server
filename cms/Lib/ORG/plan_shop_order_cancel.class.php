...
class plan_shop_order_cancel extends plan_base
{
	...
	public function runTask()
	{
		...
		$cancel_time = isset($config['value']) ? intval($config['value']) : 0;//60 * C('config.shop_order_cancel_time');
		...
		foreach ($res as $row) {
			...
			$order_log_db->add_log(array('order_id' => $row['order_id'], 'status' => 10, 'name' => '店员接单超时系统自动取消', 'phone' => ''));
