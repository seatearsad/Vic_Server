...
class Page{
    ...
    public function show(){
		...
		$str = '<span class="txt"><span id="row_count">'.$this->totalRows.'</span> 条记录 '.$now.' / '.$total.' 页  </span>';
		if($now > 2){
			$str .= '<a class="previous" href="'.$url.'1">首页</a> ';
		}
		if($now > 1){
		...
		for($i=1;$i<=5;$i++){
			...
			if($page != $now  && $page>0){
				...
				if($page == $now) $str.='<a class="pageNow">'.$page.'</a>';
			}
		}
		...
		if ($now != $total){
			...
			$str .= ' <a  class="next" href="'.$url.($total).'"><i class="tri"></i>尾页</a>';
		}
