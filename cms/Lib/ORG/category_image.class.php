...
class category_image{
	...
	public function get_image_by_path($path){
		if(!empty($path)){
			...
			$return = C('config.site_url').'/upload/category/'.$image_tmp[0].'/'.$image_tmp['1'];
			return $return;
		}else{
			return false;
		}
	...
	public function get_allImage_by_path($path){
		if(!empty($path)){
			...
			foreach($tmp_pic_arr as $key=>$value){
				...
				$return[$key] = C('config.site_url').'/upload/category/'.$image_tmp[0].'/'.$image_tmp['1'];
			}
			return $return;
		}else{
			return false;
	...
	public function del_image_by_path($path){
		if(!empty($path)){
			$image_tmp = explode(',',$path);
			unlink('./upload/category/'.$image_tmp[0].'/'.$image_tmp['1']);
			return true;
		}else{
			return false;
		}
