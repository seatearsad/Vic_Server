...
class Page{
    ...
    public function show(){
		...
		for($i=1;$i<=5;$i++){
			...
			if($page != $now  && $page>0){
				if($page<=$total){
					$str .= '<li class="page"><a href="'.$url.$page.'" title="'.$page.'">'.$page.'</a></li>';
				}else{
					break;
				}
			}else{
				if($page == $now) $str .= '<li class="page active"><a href="'.$url.$page.'" title="'.$page.'">'.$page.'</a></li>';
			}
		}
		if($total != $now && $now<$total-5 && $total>10){
			$str .= '<li class="page"> … </li><li><a href="'.$url.$total.'" title="'.$total.'">'.$total.'</a></li>';
		}
		if ($now != $total){
