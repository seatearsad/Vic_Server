	...
	public function sendSmsData($data){	
		...
		foreach ($data as $k => $v) {
			$post .= $k . '=' . $v .'&';
                ...
                //发送短信请求修改  ydhl-liulongxing@20171116
                $result = Sms::sendSmsByInfobip($data['mobile'], $data['content']);
                $status = is_array($result) ? $result["messages"]["status"]["id"] : $result;
                
                //发送短信的接口修改为上面的英比富，故注释下面的代码
...
//                $status = $arr[0];
		...
		//增加到本地数据库
		$row = array('mer_id' => $data['o2o_mer_id'], 'uid' => $data['o2o_uid'], 'store_id' => $data['o2o_store_id'], 'time' => $send_time, 'phone' => $data['mobile'], 'text' => $data['content'], 'status' => $status, 'type' => $data['o2o_type'], 'sendto' => $data['o2o_sendto']);
    ...
    /**
     * 向英比富发送请求，发送短信
     * @author ydhl-liulongxing@20171116
     * @return type
     */    
    public function sendSmsByInfobip($mobile, $content){
        $smsapi_senturl = C('config.sms_api');//英必富接口地址
        $body = array(
            "from" => "【岛居】",
            "to" => $mobile,
            "text" => $content
        ...
        $data = Sms::getResultInfobip($smsapi_senturl, $body_json, 'json', 'post');
        
        return $data;
    }
       
    /**
     * 英必富接口获取包头验证信息,使用 "Basic " + Base64编码（用户名:密码）
     * @return string
     * @author ydhl-liulongxing
     */
    public function getInfobipAuthorization()
    {
        $site_name = trim( C('config.site_name') );//英必富用户名 
        $sms_pwd = trim( C('config.sms_pwd') );//英必富密码
        $data = $site_name . ":" . $sms_pwd;
        
        return trim("Basic ".base64_encode($data));
    }
     ...
     * @return mixed|string
     */
    public function connectionInfobip($url, $body, $type, $method)
    {
        if ($type == 'json') {
        ...
        if (function_exists("curl_init")) {
            $header = array(
                ...
                'Authorization:' . Sms::getInfobipAuthorization(),
            );
            $ch = curl_init($url);
            ...
            $headers['header'][] = "Authorization: ".Sms::getAuthorization();
     ...
     * @return mixed|string
     */
    public function getResultInfobip($url, $body = null, $type = 'json', $method)
    {
        $data = Sms::connectionInfobip($url,$body,$type,$method);
