	...
	public function get_image_by_path($path, $image_type='-1')
	{
		if (!empty($path)) {
			...
			if ($image_type == '-1') {
				...
				$return['m_image'] = C('config.site_url') . '/upload/foodshop_goods/' . $image_tmp[0] . '/m_' . $image_tmp['1'];
				$return['s_image'] = C('config.site_url') . '/upload/foodshop_goods/' . $image_tmp[0] . '/s_' . $image_tmp['1'];
			} else {
				$return = C('config.site_url') . '/upload/foodshop_goods/' . $image_tmp[0] . '/' . $image_type . '_' . $image_tmp['1'];
			}
			return $return;
		} else {
			return false;
	...
	public function get_allImage_by_path($path, $image_type='-1')
	{
		if (!empty($path)) {
			...
			foreach ($tmp_pic_arr as $key => $value) {
				...
				if ($image_type == '-1') {
					$return[$key]['image'] = C('config.site_url') . '/upload/foodshop_goods/' . $image_tmp[0] . '/' . $image_tmp['1'];
					$return[$key]['m_image'] = C('config.site_url') . '/upload/foodshop_goods/' . $image_tmp[0] . '/m_' . $image_tmp['1'];
					$return[$key]['s_image'] = C('config.site_url') . '/upload/foodshop_goods/' . $image_tmp[0] . '/s_' . $image_tmp['1'];
				} else {
					$return[$key] = C('config.site_url') . '/upload/foodshop_goods/' . $image_tmp[0] . '/' . $image_type . '_' . $image_tmp['1'];
				}
			}
			return $return;
		} else {
