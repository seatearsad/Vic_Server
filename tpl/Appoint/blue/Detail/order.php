...
.form-field label em{
	...
	<script>var group_price=0;var finalprice=0;var map_url="{pigcms{:U('Group/Map/frame_select')}";<if condition="$user_session">var is_login=true;<else/>var is_login=false;var login_url="{pigcms{:U('Index/Login/frame_login')}";</if><if condition="$user_session['phone']">var has_phone=true;<else/>var has_phone=false;var phone_url="{pigcms{:U('Index/Login/frame_phone')}";</if>
	var ajaxWorkUrl="{pigcms{:U('ajaxWorker')}";
    var ajaxWorkerTimeUrl="{pigcms{:U('ajaxWorkerTime')}";
	//var appoint_id={pigcms{$Think.get.appoint_id}+0;
        var appoint_id={pigcms{$_GET['appoint_id']}+0;
	var jqueryUrl="http://libs.baidu.com/jquery/1.7.0/jquery.min.js";
	var appointFormLoadUrl="{pigcms{$static_path}js/appoint_load.js";
						...
						<form action="{pigcms{$config.site_url}/appoint/order/{pigcms{$now_group.appoint_id}.html" method="post" id="deal-buy-form" class="common-form J-wwwtracker-form">
							<div class="mainbox cf">
								<div id="deal-buy-delivery" class="blk-item delivery J-deal-buy-delivery">
											...
											<div class="comm-service">
												...
												<div class="con-service">
													<div class="con-service-inner">
														...
														<span class="clearfix">{pigcms{$appoint_product[0]['content']}</span>
                                                        <p><span>平均耗时：</span><span style="color:red">{pigcms{$appoint_product[0]['use_time']}分钟</span></p>
													</div>
												</div>
												...
												<li <if condition="$i eq 1">class="active"</if> data-id="{pigcms{$vo['id']}">
													<label class="pay-type" for="pay-type-{pigcms{$vo['id']}">
														...
														<div class="service-intro">
                                                          ...
                                                          <p><span>平均耗时：</span><span style="color:red" data-role="use_time">{pigcms{$appoint_product[0]['use_time']}分钟</span></p>
														</div>
                                    ...
                                    <if condition='$now_group["is_store"]'>
                                        ...
                                        <if condition="$worker_list">
                                            <div class="form-field" style="display:none">
                                                <label for="address-detail"><em>*</em> 选择工作人员：</label>
                                                <select name="merchant_workers_id" id="merchant_workers_id" class="address-province dropdown--small" style="color:black;" onchange="getWorkerTime($(this))">
                                                    <option value="0">请选择工作人员</option>
                                                    <volist name="worker_list" id="vo">
                                                        <option value="{pigcms{$vo.merchant_worker_id}">{pigcms{$vo.name}</option>
                                                    </volist>
                                                </select>
                                            </div>
                                            <hr/>
                                    ...
                                    <if condition='$now_group["is_store"] eq 0'>
											...
											<div class="form-field" data-name="{pigcms{$vo.name}">
												...
												<switch name="vo['type']">
													...
													<case value="2">
														...
														<p class="cover">
															<input data-role="position" class="ipt-attr" type="text" name="custom_field[{pigcms{$i}][value]" readonly="readonly" style="cursor:pointer;color:#666;" value="请点击标注地图" <if condition="$vo['iswrite']">data-required="required"</if>/>	
														</p>
														<p class="cover" style="margin-top:10px;"><input data-role="position-desc" class="ipt-attr" type="text" name="custom_field[{pigcms{$i}][value-desc]" data-role="text" style="color:#999;" value="请标注地图后填写详细地址"/></p>
													</case>
													<case value="3">
								...
								<div class="form-submit shopping-cart" style="text-align:left;">
