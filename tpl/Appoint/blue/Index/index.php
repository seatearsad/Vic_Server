...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>{pigcms{$now_category.cat_name}预约列表_{pigcms{$config.site_name}</title>
		...
		<![endif]-->
		<style>
			.deal-tile__detail{border-bottom:0px;}
			.deal-tile__detail .sales{float:right;font-size:12px;}
			.deal-tile__detail .sales .num{font-weight:normal;color:#fe5842;}
		</style>
	</head>
	<body>
					...
					<ul class="cf">
						...
						<li>
							<a href="{pigcms{$hot_sort_url}"<if condition="$_GET['order'] eq 'hot'">class="selected"</if>>
								<div class="li_txt">预约数</div>
								<div class="li_img"></div>
							</a>
						</li>
						<li>
							<a <if condition="$_GET['order'] eq 'hit-asc'">href="{pigcms{$hit_desc_sort_url}" class="selected time-asc"<elseif condition="$_GET['order'] eq 'hit-desc'"/>href="{pigcms{$hit_asc_sort_url}" class="selected"<else/>href="{pigcms{$hit_desc_sort_url}"  class="time-asc"</if>>
								<div class="li_txt">点击数</div>
								<div class="li_img"></div>
							</a>
						</li>
						<li>
					...
					<if condition="$group_list">
						<ul>
							<volist name="group_list" id="vo">
								<li <if condition='$i%3 eq 0'>class="last--even"</if>>
									<div class="category_list_img">
										...
										<div class="datal">
											<a href="{pigcms{$vo.url}" target="_blank">
												<div class="category_list_title" style="text-indent:0">{pigcms{$vo.merchant_name}</div>
												<div class="category_list_description">{pigcms{$vo.appoint_name}</div>
											</a>
											<div class="deal-tile__detail cf">
												<if condition="$vo['payment_status'] eq 1">
													<span>定金 :</span><span class="price">$<strong>{pigcms{$vo.payment_money}</strong> </span>
												<else/>
													<span>无需定金</span>
												</if>
												 <div class="cheap"><if condition="$vo['appoint_type'] eq 1">上门<else/>到店</if></div>
												<!--if condition="$vo['appoint_sum']"-->
						...
						{pigcms{$pagebar}
					<else/>
						<div style="text-align:center;height:500px;margin-top:60px;">暂无此类预约，请查看其他分类</div>
					</if>
				</div>
				<div class="activity">
