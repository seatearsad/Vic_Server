...
<body id="index">
        ...
        <form id="form" method="post" action="{pigcms{:U('My/withdraw')}">
		    ...
		    <p class="btn-wrapper">余额：{pigcms{$user_info['now_money']},可提现：{pigcms{$user_info.can_withdraw_money}元</p>
		    <if condition="$user_info.score_recharge_money gt 0"><p class="btn-wrapper"><font color="red">您有积分兑换余额{pigcms{$user_info.score_recharge_money}元，不能用于提现！</font></p></if>
