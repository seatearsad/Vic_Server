			                    	...
			                    	<notempty name="score_count">
                                                <tr>
                                                    <td style="text-align:left;">
                                                        ...
                                                        <span class="inline-block money" style="color:#EA4F01;">
															<strong id="deal-buy-total-t">{pigcms{$now_user.score_count}</strong>
															<input type="hidden" name="score_count" value="{pigcms{$now_user.score_count}">
                                                        </span>
														&nbsp;&nbsp;&nbsp;&nbsp;
                                                        <strong>本单可抵扣{pigcms{$config['score_name']}</strong>：
                                                        <span class="inline-block money" style="color:#EA4F01;">
                                                            <strong id="deal-buy-total-t">{pigcms{$score_can_use_count}</strong>
                                                            <input type="hidden" name="score_used_count" value="{pigcms{$score_can_use_count}">
                                                        </span>
														&nbsp;&nbsp;&nbsp;&nbsp;
                                                        <strong>{pigcms{$config['score_name']}可抵扣现金</strong>：
                                                        <span class="inline-block money" style="color:#EA4F01;">
                                                                <strong id="deal-buy-total-t">${pigcms{$score_deducte|floatval=###}</strong>
                                                    ...
                                                    <td colspan="3" class="extra-fee total-fee rightpadding">
                                                        使用{pigcms{$config['score_name']}抵扣:<input type="checkbox" id ="use_score" name="use_score" value="1" <if condition="$score_checkbox eq 1 "> checked="checked" </if><if condition="$score_can_use_count eq 0 "> disabled="disabled" </if>>
                                                    </td>
                                                </tr>
                                            </notempty>
			                	</table>
							...
							<if condition="$order_info['order_type'] != 'recharge'">
								<div >
									...
									<span class="inline-block money" style="font-size:20px;color:#EA4F01;">
										$<strong id="deal-buy-total-t" class="need_pay">{pigcms{$pay_money}</strong>
									</span>
								</div>
							</if>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$('#J-order-weixin-button').live('click',function(){
				window.location.href="{pigcms{:U('Pay/weixin_back',array('order_type'=>$order_info['order_type']))}&order_id="+orderid;
			});
		});
	</script>
