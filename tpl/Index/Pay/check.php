			                    ...
			                    <table cellspacing="0" class="buy-table">
			                    	...
			                    	<if condition="$score_count gt 0">
										<tr>
											...
											<td colspan="3" class="extra-fee total-fee rightpadding">
												使用积分抵扣:<input type="checkbox" id ="use_score" name="use_score" value="1" <if condition="($score_checkbox eq 1) || (!empty($_GET['type']) && ($_GET['type'] == 'gift'))"> checked="checked" </if><if condition="($score_can_use_count eq 0) ||(($_GET['type'] == 'gift')) "> disabled="disabled" </if>>
											</td>
										</tr>
									</if>
								<if condition="$order_info['order_type'] != 'recharge'">
