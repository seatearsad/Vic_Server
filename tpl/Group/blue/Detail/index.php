								...
								<div class="product_info_list">
									<ul>
										<li class="cf">
											...
											<div class="priduct_price">$<strong>{pigcms{$now_group.price}<if condition="$now_group.extra_pay_price gt 0 AND $config.open_extra_price eq 1">+{pigcms{$now_group.extra_pay_price}{pigcms{$config.extra_price_alias_name}</if></strong><span>${pigcms{$now_group.old_price}<!--<if condition="$now_group.extra_pay_old_price gt 0 AND $config.open_extra_price eq 1">+{pigcms{$now_group.extra_pay_old_price}{pigcms{$config.extra_price_alias_name}</if>--></span></div>
										</li>
										<li class="cf">
										...
										<li class="cf">
											...
											<div class="priduct_data">截止到{pigcms{$now_group.end_time|date='Y.m.d',###} （<span><switch name="now_group['is_general']"><case value="0">周末、法定节假日通用</case><case value="1">周末不能使用</case><case value="2">法定节假日不能使用</case><case value="3">周末、法定节假日不能通用</case></switch></span>）</div>
										</li>
										<if condition="$now_group.group_share_num gt 0">
											<li class="cf">
												<div class="product_info_list_left">组团：</div>
												<div class="priduct_data">您需要购买或者邀请好友购买【{pigcms{$now_group.group_share_num}】份才能成团</div>
											</li>
										<elseif condition="$now_group.open_now_num gt 0" />
											<li class="cf">
											<div class="product_info_list_left">拼团：</div>
											<div class="priduct_data">还差【{pigcms{$now_group.open_now_num}】份成团</div>
											</li>
										<elseif condition="$now_group.open_num gt 0" />
											<li class="cf">
											<div class="product_info_list_left">拼团：</div>
											<div class="priduct_data">还差【{pigcms{$now_group.open_num}】份成团</div>
											</li>
										</if>
									</ul>
								</div>
						...
						<div class="product_list_bottom">
							...
							<if condition="isset($mpackages) AND !empty($mpackages)">
								...
								<if condition="$now_group['end_time'] gt $_SERVER['REQUEST_TIME'] AND $now_group['begin_time'] lt $_SERVER['REQUEST_TIME'] AND $now_group['type'] eq 1">
									...
									<button class="info_but appoint_but" type="button">立即预约</button>
									<elseif condition="$now_group['pin_num'] gt 0 OR ($now_group['trade_type'] neq '' AND $now_group['trade_info'] neq '' AND $now_group['trade_info'] neq 0) " />
									<button class="info_but pin_btn" type="button" style="font-size:14px" data-type="{pigcms{:U('Index/Recognition/see_qrcode',array('type'=>'group','id'=>$now_group['group_id']))}"">请在移动端购买</button>
									<else />
							...
							<if condition="$now_group['begin_time'] gt $_SERVER['REQUEST_TIME']">
								...
								距开始：<span id="time_d" style="color:#9B9B9B;">{pigcms{$time_array['d']}</span><b style="color:#9B9B9B"> 天　</b>
								<span id="time_h" style="color: #3A9407">{pigcms{$time_array['h']}</span><b style="color:#FEB04A"> : </b>
								<span id="time_m" style="color: #3A9407">{pigcms{$time_array['m']}</span><b style="color:#FEB04A"> : </b>
								<span id="time_s" style="color: #3A9407">{pigcms{$time_array['s']}</span><b style="color:#FEB04A"> : </b>
								<span id="time_mm">00</span>
								</div>
							<elseif condition="$now_group['end_time'] lt $_SERVER['REQUEST_TIME'] OR $now_group['type'] eq 3" />
								<div  style="color: red;font-size: 20px">真遗憾！这单{pigcms{$config.group_alias_name}已经结束</div>
							<else />
			...
			<div class="content_left">
				...
				<section class="package cf" id="anchor-detail">
					...
					<style>
						...
						.group_content ul li{list-style-type: disc;margin-left: 15px;}
						...
						.group_content img{max-width:100%;_width:100%;}
					</style>
		...
		<script>
			...
			$(function(){	
				...
				var timer = setInterval(function(){
					var timeD = parseInt(timeDDom.html());
							...
							if(timeM == 0){
								if(timeH == 0){
									if(timeD == 0){
										...
										timeDDom.html(format_time(timeD-1));
									}	
									timeHDom.html('23');
								}else{
			...
			$('.pin_btn').click(function(){
				var qrcode_href = $(this).attr('data-type');
			...
			$('.appoint_but').click(function(){
				var url = "{pigcms{:U('Appoint/Detail/index')}";
				var appoint_id = "{pigcms{$now_group['appoint_id']}";
				url +='&appoint_id='+appoint_id;
				location.href=url;
			});
		</script>
