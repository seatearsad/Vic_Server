...
<!-- 									<th id="shopList_c4" width="50">使用商家会员卡余额</th> -->
									<th id="shopList_c5" width="150" >支付详情</th>
...
<!-- 										<td>{pigcms{$vo.merchant_balance}</td> -->
										...
										<if condition="$vo['third_id'] eq '0' AND $vo['pay_type'] eq 'offline'">
											总金额： ${pigcms{$vo['total_price']}<br>
									 		平台余额支付 ：{pigcms{$vo.balance_pay} <br>
									 		商家会员卡余额支付：{pigcms{$vo.merchant_balance}<br>
											线下需向商家付金额：<font color="red">${pigcms{$vo['total_price']-$vo['merchant_balance']-$vo['balance_pay']}元</font>
										<else/>
											平台余额支付：{pigcms{$vo.balance_pay} <br>
									 		商家会员卡余额支付：{pigcms{$vo.merchant_balance}<br>
									 		在线支付金额：{pigcms{$vo.payment_money}<br> 
										</if>
									 	</td>
										<td>
