	...
	<div class="breadcrumbs" id="breadcrumbs">
		<ul class="breadcrumb">
			<li>
				...
				{pigcms{$config.waimai_alias_name}订单列表
			</li>
		</ul>
	</div>
	<!-- 内容头部 -->
						...
						<table class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th id="shopList_c4" width="80">支付方式</th>
							...
							<tbody>
								...
								<volist name="order_list" id="vo">
									<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
										...
										<td>{pigcms{$vo.discount_price}</td>
										<td>
											<if condition="$vo['pay_type'] eq 'offline'" ><span class="red">线下支付</span>
											<elseif condition="$vo['paid'] eq 0 && $vo['pay_type'] neq 'offline'" /><span class="red">未支付</span>
										...
										<td>
											...
											<elseif condition="$vo['order_status'] eq 4"/>已取餐 
											<elseif condition="$vo['order_status'] eq 5"/>正在配送
										...
										<td class="button-column" width="50">
											...
											<elseif condition=" $vo['order_status'] eq 2 && ( $vo['pay_type'] eq 'offline' || ( $vo['pay_type'] neq 'offline' && $vo['paid'] eq 1) )"  />
										...
										<if condition="( $vo['pay_type'] eq 'offline' || ( $vo['pay_type'] neq 'offline' && $vo['paid'] eq 1) ) " > 
											<if condition="$vo['order_status'] eq 2 || $vo['order_status'] eq 3 || $vo['order_status'] eq 4 || $vo['order_status'] eq 5 " > 
											...
											<span title="订单已经取消" class="gray edit_btn" style="padding-right:8px;">订单已取消</span>
											</if> 
										</if>
										</td>
									</tr>
...
$(function(){
	...
	jQuery(document).on('click','.js-cancel-order',function(){
        if(!confirm('确定要取消这个订单吗?不可恢复。')) return false;
        	...
        	$.post("{pigcms{:U('Store/waimai_cancel')}",{order_id:order_id},function(result){
        		if(result.status == 1){
        			alert('取消成功');
