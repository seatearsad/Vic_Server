...
<div class="main-content">
	...
	<div class="breadcrumbs" id="breadcrumbs">
        ...
        <div id="nav" class="mainnav_title">
			<form id="frmselect" name="frmselect" method="get" action="" style="margin-bottom:0;margin-left:13px;">
				...
				<input type="hidden" name="a" value="storebill"/>
                ...
                <ul>
					<a href="{pigcms{:U('Count/storebill',array('mer_id'=>$mer_id, 'type' => 'meal'))}<if condition="$_GET['store_id']">&store_id={pigcms{$_GET['store_id']}</if>" <if condition="$type eq 'meal'">class="on"</if>>{pigcms{$config.meal_alias_name}账单</a>
					<a href="{pigcms{:U('Count/storebill',array('mer_id'=>$mer_id, 'type' => 'group'))}<if condition="$_GET['store_id']">&store_id={pigcms{$_GET['store_id']}</if>" <if condition="$type eq 'group'">class="on"</if>>{pigcms{$config.group_alias_name}账单</a>
					<if condition="$config['is_open_weidian']">
					<a href="{pigcms{:U('Count/storebill',array('mer_id'=>$mer_id, 'type' => 'weidian'))}<if condition="$_GET['store_id']">&store_id={pigcms{$_GET['store_id']}</if>" <if condition="$type eq 'weidian'">class="on"</if>>微店账单</a>
					</if>
					<if condition="$config['appoint_page_row'] gt 0">
					<a href="{pigcms{:U('Count/storebill',array('mer_id'=>$mer_id, 'type' => 'appoint'))}<if condition="$_GET['store_id']">&store_id={pigcms{$_GET['store_id']}</if>" <if condition="$type eq 'appoint'">class="on"</if>>{pigcms{$config.appoint_alias_name}账单</a>
					</if>
					<if condition="$config['wxapp_url']">
							<a href="{pigcms{:U('Count/storebill',array('mer_id'=>$mer_id, 'type' => 'wxapp'))}<if condition="$_GET['store_id']">&store_id={pigcms{$_GET['store_id']}</if>" <if condition="$type eq 'wxapp'">class="on"</if>>营销账单</a>
					</if>
					<if condition="$config['is_cashier'] OR $config['pay_in_store']">
						<a href="{pigcms{:U('Count/storebill',array('mer_id'=>$mer_id, 'type' => 'store'))}<if condition="$_GET['store_id']">&store_id={pigcms{$_GET['store_id']}</if>" <if condition="$type eq 'store'">class="on"</if>>收银账单</a>
					</if>
					<if condition="$config['waimai_alias_name']">
						<a href="{pigcms{:U('Count/storebill',array('mer_id'=>$mer_id, 'type' => 'waimai'))}" <if condition="$type eq 'waimai'">class="on"</if>>{pigcms{$config.waimai_alias_name}账单</a>
					</if>
				</ul>
        </div>
	<div class="alert alert-info" style="margin:10px;">
        ...
        <div class="alert alert-info" style="margin:10px;">
            ...
            <div class="month"></div>
            <!--时间筛选-->
            <form id="myform" method="post" action="{pigcms{:U('Count/storebill')}" >
			<if condition="$_GET['store_id']"> <input type="hidden" name="store_id" value="{pigcms{$_GET['store_id']}"></if>
                <input type="hidden" name="mer_id" value="{pigcms{$mer_id}">
                <input type="hidden" name="type" value="{pigcms{$type}">
											...
											<table class="table table-striped table-bordered table-hover">
												<thead>
													<tr>
														...
														<th>在线支付金额</th>
														<th>商家会员卡余额支付金额</th>
															<if condition="($type eq 'meal') or ($type eq 'group') or ($type eq 'appoint')"><th>平台优惠金额</th></if>
														<th>商家优惠券</th>
														<th>下单时间</th>
														<th>支付时间</th>
														<th>支付类型</th>
													...
													<if condition="$order_list">
														<volist name="order_list" id="vo">
															<tr>
																...
																<if condition="($type eq 'meal') or ($type eq 'group') or ($type eq 'appoint')"><td>{pigcms{$vo.system_pay}</td></if>
...
function total_price()
{
	...
	if (total > 0) {
		$('#show_count').html('账单总计金额：<strong style=\'color:red\'>$' + total + '</strong>, 平台对该商家的抽成比例是：<strong style=\'color:green\'>' + percent + '%</strong>, 平台抽成金额：<strong style=\'color:green\'>$' + Math.round(system_pay * percent) /100 + '</strong>,平台支付金额：<strong style=\'color:green\'>$' + Math.round(system_pay*100)/100 + '</strong>,商家应得金额:<strong style=\'color:red\'>$' + Math.round((system_pay - Math.round(total * percent) /100) * 100)/100 + '</strong>');
	} else {
		$('#show_count').html('');
	}
}
...
function getyear(){
        ...
        for(var year={pigcms{$start_year};year<=now_year;year++){
            if(selected_year!=''&&year==selected_year){
                year_list+='<a  href="{pigcms{:U(\'Count/storebill\',array(\'mer_id\'=>$mer_id,\'type\'=>$type))}&year='+year<if condition="$_GET['store_id']">+'&store_id={pigcms{$_GET['store_id']}'</if>+'" class="on">'+year+'</a>  ';
            }else{
                year_list+='<a  href="{pigcms{:U(\'Count/storebill\',array(\'mer_id\'=>$mer_id,\'type\'=>$type))}&year='+year<if condition="$_GET['store_id']">+'&store_id={pigcms{$_GET['store_id']}'</if>+'">'+year+'</a>  ';
            }
...
function getmonth(year){
        ...
        for (var m = 1; m <= month_end; m++) {
            if (m==selected_month) {
                month_list +='<a href="{pigcms{:U(\'Count/storebill\',array(\'mer_id\'=>$mer_id,\'type\'=>$type))}&year='+year+'&month='+m<if condition="$_GET['store_id']">+'&store_id={pigcms{$_GET['store_id']}'</if>+'"  class="on">'+m+'月 '+'</a>';
            }else{
                month_list +='<a href="{pigcms{:U(\'Count/storebill\',array(\'mer_id\'=>$mer_id,\'type\'=>$type))}&year='+year+'&month='+m<if condition="$_GET['store_id']">+'&store_id={pigcms{$_GET['store_id']}'</if>+'" >'+m+'月 '+'</a>';
            }
        }
        month_list+='</ul></div>';
        $('.month').empty();
