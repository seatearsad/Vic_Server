...
<div class="main-content">
	...
	<div class="breadcrumbs" id="breadcrumbs">
		<ul class="breadcrumb">
			...
			<li>
				<a href="{pigcms{:U('index')}">景内商品推荐分类</a>
			</li>
			<li>
				<a href="{pigcms{:U('com_list',array('cat_id'=>$_GET['cat_id']))}">景内商品列表</a>
			</li>
				...
				<div class="col-xs-12">
					<div class="tabbable">
						<ul class="nav nav-tabs" id="myTab">
							<li class="active">
								<a data-toggle="tab" href="#basicinfo">修改商品</a>
							</li>
						</ul>
					</div>
						...
						<div class="tab-content">
							<div id="basicinfo" class="tab-pane active">
								<div class="form-group">
									<label class="col-sm-1"><label>商品名</label></label>
									<input type="text" name="com_name" value="{pigcms{$now_order['com_name']}" />
									<span class="form_tips">必填。</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>商品介绍</label></label>
									<input type="text" name="com_title" value="{pigcms{$now_order['com_title']}" />
									<span class="form_tips">必填。</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>价格</label></label>
									<input type="text" name="price" value="{pigcms{$now_order['price']}" />
									<span class="form_tips">必填。</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>排序</label></label>
									<input type="number" name="sort" value="{pigcms{$now_order['sort']}" />
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>跳转URL</label></label>
									<input type="text" name="url" value="{pigcms{$now_order['url']}" />
									<span class="form_tips">必填。</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1">首页推荐</label>
								...
								<div class="form-group">
									...
									<span class="form_tips">必填。只需要上传一张。必须清楚，尺寸建议：150px*150px</span>
								</div>
								<div class="form-group">
...
KindEditor.ready(function(K){
	...
	$('#edit_form').submit(function(){
		$.post("{pigcms{:U('com_edit')}",$('#edit_form').serialize(),function(result){
			if(result.status == 1){
				alert(result.info);
				window.location.href = "{pigcms{:U('com_list',array('cat_id'=>$_GET['cat_id']))}";
			}else{
				alert(result.info);
			}
		})
