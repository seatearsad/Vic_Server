								...
								<if condition="$list">
									<volist name="list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<a class="label label-sm label-info" href="{pigcms{:U('Customer/userdetail',array('logid'=>$vo['pigcms_id']))}">查看发送对象</a>
											<else />
												<if condition="$vo['type'] eq 1" >商家推广获得粉丝
												<elseif condition="$vo['type'] eq 2" />扫描商家产品获得的粉丝
												<elseif condition="$vo['type'] eq 3" />平台的粉丝
												<else />
												全部粉丝
												</if>
											</if>
