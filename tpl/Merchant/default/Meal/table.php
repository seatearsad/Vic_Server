			...
			<div class="row">
				<div class="col-xs-12">
					<div class="alert alert-block alert-success">
						...
						<p>
							<strong>【特别注意如果有客户预定了该餐桌，且已经支付，那么该客户预定餐桌前后2小时都不会被其他人预定了】</strong>
						</p>
					</div>
					<button class="btn btn-success" onclick="CreateShop()">新建桌台</button>
								...
								<if condition="$list">
									<volist name="list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<td>{pigcms{$vo.num}</td>
											<td>
												<a href="{pigcms{:U('Meal/url_qrcode',array('pigcms_id'=>$vo['pigcms_id'], 'store_id' => $vo['store_id']))}" class="see_qrcode">查看二维码</a>
											</td>
											<td class="">
												...
												<a href="{pigcms{:U('Meal/table_order',array('id'=>$vo['pigcms_id'], 'store_id' => $vo['store_id']))}">桌台预定详情</a>
											</td>
										</tr>
									</volist>
								<else/>
...
function updateStatus(dom1, dom2, status1, status2, attribute){
	$(dom1).each(function(){
		...
		$.ajax({
			url:"{pigcms{:U('Meal/table_status')}",
			type:"post",
