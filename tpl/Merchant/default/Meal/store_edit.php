				...
				<div class="col-xs-12">
					<div class="tabbable">
						<ul class="nav nav-tabs" id="myTab">
							...
							<if condition="$now_store['store_type'] eq 0 OR $now_store['store_type'] eq 2">
							<li>
								<a data-toggle="tab" href="#deliver">外送设置</a>
							</li>
							</if>
							<li>
						...
						<div class="tab-content">				
							...
							<div id="deliver" class="tab-pane">
								<div class="form-group">
									...
									<span><label><input id='deliver_type' name="deliver_type" <if condition="$store_meal['deliver_type'] eq 0 ">checked="checked"</if> value="0" type="radio"></label>&nbsp;<span>平台配送</span>&nbsp;</span>
							...
							<div id="label" class="tab-pane">
								<volist name="label_list" id="vo">
									<div class="form-group">
										<div class="radio">
											<label>
												<input class="cat_class" type="checkbox" name="store_labels[]" value="{pigcms{$vo.id}" id="Config_store_label_{pigcms{$vo.id}" <if condition="in_array($vo['id'], $store_meal['store_labels'])">checked="checked"</if>/>
							...
							<div id="promotion" class="tab-pane">
								...
								<div class="form-group">
									...
									<input class="col-sm-1" size="10" maxlength="10" name="store_discount" id="Config_mean_full_money" type="text" value="{pigcms{$store_meal.store_discount}" /><strong style="color:red">请填写0~100之间的整数，0和100都是表示无折扣，98表示9.8折</strong>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>折扣方式</label></label>
									<span><label><input id='discount_type0' name="discount_type" <if condition="$store_meal['discount_type'] eq 0 ">checked="checked"</if> value="0" type="radio"></label>&nbsp;<span>折上折</span>&nbsp;</span>
									<span><label><input id='discount_type1' name="discount_type" <if condition="$store_meal['discount_type'] eq 1 ">checked="checked"</if> value="1" type="radio" ></label>&nbsp;<span>折扣最优</span></span>
									<strong style="color:red">折上折的意思是如果这个用户是有VIP等级，VIP等级有折扣优惠。那么这个用户的优惠计算方式是先用店铺的优惠进行打折后，再用VIP折扣进去打折；<br/>
							...
							<div id="stock" class="tab-pane">
								<div class="form-group">
									...
									<label><input type="radio" name="stock_type" value="0" <if condition="$store_meal['stock_type'] eq 0">checked="checked"</if>>&nbsp;&nbsp;每天自动更新固定量的库存</label>&nbsp;&nbsp;&nbsp;
									<label><input type="radio" name="stock_type" value="1" <if condition="$store_meal['stock_type'] eq 1">checked="checked"</if>>&nbsp;&nbsp;固定的库存，不会每天自动更新</label>&nbsp;&nbsp;&nbsp;
								</div>
								<div style="clear:both;"></div>
							</div>
...
$(function($){
	...
	$('#edit_form').submit(function(){
		$.post("{pigcms{:U('Meal/store_edit',array('store_id'=>$store_meal['store_id']))}",$('#edit_form').serialize(),function(result){
			if(result.status == 1){
				alert(result.info);
				window.location.href = "{pigcms{:U('Meal/index')}";
			}else{
				alert(result.info);
			}
		})
