...
<p>
	<div>
		...
		<input type="file" style="width:80%;border:1px solid #ddd" name="photo"></input>
	</div>
...
<php>}else{</php>
	...
	<img src="{pigcms{$static_path}images/export.png" align="absmiddle" /> 
...
function returnHomepage(url)
{
	...
	var domhrefid = domid + '_href';
	if (origin.document.getElementById(domsrcid)) {
		origin.document.getElementById(domsrcid).src = url;
	}
	
	if (origin.document.getElementById(domhrefid)) {
		origin.document.getElementById(domhrefid).href = url;
	}
	dom.value = url;
	
	if(typeof(parent.upload_func) == 'function'){
		parent.upload_func();
	}
