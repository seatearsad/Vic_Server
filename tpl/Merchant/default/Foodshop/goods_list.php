		...
		<div class="page-content-area">
			...
			<div class="row">
				<div class="col-xs-12">
					...
					<a class="btn btn-success" href="{pigcms{:U('Foodshop/total', array('store_id' => $now_store['store_id']))}">查看商品的销量统计</a>
								...
								<if condition="$goods_list">
									<volist name="goods_list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<td class="button-column">
												<a title="修改" class="green" style="padding-right:8px;" href="{pigcms{:U('Foodshop/goods_edit',array('goods_id'=>$vo['goods_id']))}">
													<i class="ace-icon fa fa-pencil bigger-130"></i>
												</a>
												<a title="删除" class="red" style="padding-right:8px;" href="{pigcms{:U('Foodshop/goods_del',array('goods_id'=>$vo['goods_id']))}">
...
function CreateShop(){
	window.location.href = "{pigcms{:U('Foodshop/goods_add',array('sort_id' => $now_sort['sort_id']))}";
}
function updateStatus(dom1, dom2, status1, status2, attribute){
	$(dom1).each(function(){
		...
		$.ajax({
			url:"{pigcms{:U('Foodshop/goods_status')}",
			type:"post",
