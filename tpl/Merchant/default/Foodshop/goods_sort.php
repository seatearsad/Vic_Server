								...
								<if condition="$sort_list">
									<volist name="sort_list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<!--td><if condition="$vo['see_image']"><img src="{pigcms{$vo['see_image']}" style="width:50px;height:50px" /></if></td-->
											<td>
											...
											<td>
												<a class="green" style="padding-right:8px;" title="商品管理" href="{pigcms{:U('Foodshop/goods_list',array('sort_id' => $vo['sort_id']))}">商品管理<i class="ace-icon fa fa-pencil bigger-130"></i></a>
											</td>
											<td>
												<a title="修改" class="green" style="padding-right:8px;" href="{pigcms{:U('Foodshop/sort_edit',array('sort_id' => $vo['sort_id']))}">
													<i class="ace-icon fa fa-pencil bigger-130"></i>
												</a>　　
												<a title="删除" class="red" style="padding-right:8px;" href="{pigcms{:U('Foodshop/sort_del',array('sort_id' => $vo['sort_id']))}">
...
function CreateShop(){
	window.location.href = "{pigcms{:U('Foodshop/sort_add',array('store_id'=>$now_store['store_id']))}";
}
function updateStatus(dom1, dom2, status1, status2, attribute){
	$(dom1).each(function(){
		...
		$.ajax({
			url:"{pigcms{:U('Foodshop/sort_status')}",
			type:"post",
