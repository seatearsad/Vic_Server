						...
						<div class="grid-view">
							<form enctype="multipart/form-data" class="form-horizontal" method="post" action="">
								<div class="form-group">
									<label class="col-sm-1"><label for="contact_name">订单号</label></label>
									<label class="col-sm-3">{pigcms{$order.order_id}</label>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label class="col-sm-1"><label for="contant_url">应收取现金 </label></label>
								...
								<div class="form-group">
									...
									<label class="col-sm-3"><if condition="$order['note']">{pigcms{$order.note|htmlspecialchars_decode=ENT_QUOTES}<elseif condition="$order['info'][0]['omark']"/>{pigcms{$order.info.0.omark}<else />顾客没有特殊说明</if></label>
								</div>
								...
								<div class="form-group">
									<label class="col-sm-1"><label for="canrqnums">付款状态</label></label>
									<label class="col-sm-3">
										...
										<span class="lbl" style="z-index:1;color:green;">已付${pigcms{$order['balance_pay']+$order['merchant_balance']+$order['coupon_price']+$order['card_price']+$order['score_deducte']|floatval}</span>，
										<span class="lbl" style="z-index:1;color:red;">
											<if condition="$order['total_price'] gt 0">
								...
								<if condition="$order['status'] eq 0" >
									<div class="form-group">
										<label class="col-sm-1"><label for="canrqnums">订单状态</label></label>
										<div class="radio">
											<label>
								...
								<elseif condition="$order['status'] eq 2" />
									<div class="form-group">
										<label class="col-sm-1"><label for="canrqnums">订单状态</label></label>
										<label class="col-sm-3">已评价</label>
									</div>
								<elseif condition="$order['status'] eq 3" />
									<div class="form-group">
										<label class="col-sm-1"><label for="canrqnums">订单状态</label></label>
										<label class="col-sm-3">已退款</label>
									</div>
								<elseif condition="$order['status'] eq 4" />
									<div class="form-group">
										<label class="col-sm-1"><label for="canrqnums">订单状态</label></label>
										<label class="col-sm-3">已取消</label>
									</div>
								</if>
