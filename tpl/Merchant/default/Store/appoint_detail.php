...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>小猪{pigcms{$config.appoint_alias_name}预约系统 - 店铺管理中心</title>
	...
	<body>
		<table>
				...
				<td width="35%">
					<?php if($now_order['paid'] == 0){ ?>
						...
						<?php if($now_order['service_status'] == 0) { ?>
					   		...
					   		<?php if(($now_order['is_del'] == 0) && ($now_order['paid'] == 1)) {?><a href="{pigcms{:U('Store/appoint_verify',array('order_id'=>$now_order['order_id']))}" class="group_verify_btn">验证服务</a><?php }?>
			...
			<tr>
				...
				<th width="15%">总价</th>
				<td width="35%"><if condition='$now_order["product_id"] gt 0'>$ {pigcms{$now_order.product_price}<else />$ {pigcms{$now_order.appoint_price}</if></td>
			</tr>
			...
			<if condition='$now_order["product_detail"]'>
				...
				<tr>
					...
					<td width="35%">
						...
						<else />
							{pigcms{$now_order.pay_money|number_format=###,2}
						</if>
					</td>
				</tr>
				...
				<if condition='($now_order["product_id"]) AND ($now_order["paid"] == 1) AND ($now_order["pay_time"] gt 0)'>
					<tr>
						...
						<td width="35%">{pigcms{$now_order['pay_money'] + $now_order['balance_pay'] + $now_order['score_deducte']|number_format=###,2}</td>
