			...
			<div class="row">
				 <div class="form-group">
					<form action="{pigcms{:U('Appoint/order_list')}" method="get">
						...
						<a href="{pigcms{:U('Appoint/export',$_GET)}" class="button" style="float:right;margin-right: 10px;">导出订单</a>
					</form>
				</div>
									...
									<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
										...
										<td>
											...
											<?php if(empty($vo['paid'])): ?>
												付款时间：无
											<?php else : ?>
												付款时间：<if condition="$vo['pay_time']">{pigcms{$vo['pay_time']|date='Y-m-d H:i:s',###}<else/>无</if>
											<?php endif; ?><br/>
