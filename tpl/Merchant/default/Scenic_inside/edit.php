				...
				<div class="col-xs-12">
					<div class="tabbable">
						<ul class="nav nav-tabs" id="myTab">
							<li class="active">
								<a data-toggle="tab" href="#basicinfo">修改活动</a>
							</li>
						</ul>
					</div>
					<form enctype="multipart/form-data" class="form-horizontal" method="post" id="edit_form">
						<input type="hidden" name="activity_id" value="{pigcms{$now_order.activity_id}"/>
						<div class="tab-content">
							<div id="basicinfo" class="tab-pane active">
								<div class="form-group">
									<label class="col-sm-1">活动标题</label>
									<input class="col-sm-5" size="20" name="activity_title" id="activity_title" value="{pigcms{$now_order.activity_title}" type="text"/>
									<span class="form_tips">必填。</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>短标题</label></label>
									<textarea class="col-sm-5" rows="3" name="activity_stitle">{pigcms{$now_order.activity_stitle}</textarea>
									<span class="form_tips">必填。精准的活动目的！</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>活动时间</label></label>
									<input type="datetime-local" name="start_time" value="{pigcms{$now_order.start_time}" />
									<span class="form_tips">必填。</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1">活动介绍</label>
									<textarea id="scenic_intr" name="activity_content" placeholder="写上一些想要发布的活动内容">{pigcms{$now_order['activity_content']|htmlspecialchars_decode=ENT_QUOTES}</textarea>
								</div>
								<div class="form-group">
