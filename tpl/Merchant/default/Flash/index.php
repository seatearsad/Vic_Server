									...
									<volist name="info" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<td class="button-column" nowrap="nowrap">
												<if condition="$vo['tip'] eq 1">
												<a title="修改" class="green" style="padding-right:8px;" href="{pigcms{:U('Flash/edit',array('id'=>$vo['id'], 'tip'=>$vo['tip']))}">
												<else />
												<a title="修改" class="green" style="padding-right:8px;" href="{pigcms{:U('Flash/editbg',array('id'=>$vo['id'], 'tip'=>$vo['tip']))}">
												</if>
												
													<i class="ace-icon fa fa-pencil bigger-130"></i>
												</a>
												<a title="删除" class="red" style="padding-right:8px;" href="{pigcms{:U('Flash/del',array('id'=>$vo['id'], 'tip' => $vo['tip']))}">
...
<else />
function CreateShop(){
	window.location.href = "{pigcms{:U('Flash/addbg', array('tip' => $tip))}";
}
</if>
