...
<div class="main-content">
	...
	<div class="breadcrumbs" id="breadcrumbs">
		<ul class="breadcrumb">
			...
			<li>
				<a href="{pigcms{:U('one_index',array('map_id'=>$_GET['map_fid']))}">地图二级分类</a>
			</li>
			<li class="active">修改二级分类</li>
		</ul>
	</div>
	<!-- 内容头部 -->
				...
				<div class="col-xs-12">
					<div class="tabbable">
						<ul class="nav nav-tabs" id="myTab">
							<li class="active">
								<a data-toggle="tab" href="#basicinfo">修改二级分类</a>
							</li>
						</ul>
					</div>
					<form enctype="multipart/form-data" class="form-horizontal" method="post" id="edit_form">
						<input type="hidden" name="map_id" value="{pigcms{$_GET['map_id']}"/>
						<input type="hidden" name="map_fid" value="{pigcms{$_GET['map_fid']}"/>
						<div class="tab-content">
							<div id="basicinfo" class="tab-pane active">
								<div class="form-group">
									<label class="col-sm-1"><label>分类名</label></label>
									<input type="text" name="map_name" value="{pigcms{$now_order.map_name}" />
									<span class="form_tips">必填。</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label>排序</label></label>
									<input type="number" name="map_sort" value="{pigcms{$now_order['map_sort']}" />
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<span class="form_tips">必填。只需要上传1张。前台展示的logo，尺寸建议：35px*38px</span>
								</div>
								<div class="form-group">
...
KindEditor.ready(function(K){
	...
	$('#edit_form').submit(function(){
		$.post("{pigcms{:U('one_edit')}",$('#edit_form').serialize(),function(result){
			if(result.status == 1){
				alert(result.info);
				window.location.href = "{pigcms{:U('one_index',array('map_id'=>$_GET['map_fid']))}";
			}else{
				alert(result.info);
			}
		})
