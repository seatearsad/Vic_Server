				...
				<div class="col-xs-12">
					<div class="tabbable">
						<ul class="nav nav-tabs" id="myTab">
							<li class="active">
								<a data-toggle="tab" href="#basicinfo">添加自提点</a>
							</li>
						</ul>
					</div>
					<form enctype="multipart/form-data" class="form-horizontal" method="post" id="edit_form">
					<input type="hidden" name="id" value="{pigcms{$pick_addr['id']}" >
						<div class="tab-content">
							<div class="form-group">
								<label class="col-sm-1"><label for="long_lat">店铺经纬度</label></label>
								<input class="col-sm-2" size="10" name="long_lat" id="long_lat" value="{pigcms{$pick_addr.long},{pigcms{$pick_addr.lat}" type="text" readonly="readonly"/>
								&nbsp;&nbsp;&nbsp;&nbsp;<a href="#modal-table" class="btn btn-sm btn-success" id="show_map_frame" data-toggle="modal">点击选取经纬度</a>
							</div>
							<div class="form-group">
								<label class="col-sm-1"><label>自提点</label></label>
								<fieldset id="choose_cityarea" province_id="{pigcms{$pick_addr.province_id}" city_id="{pigcms{$pick_addr.city_id}" area_id="{pigcms{$pick_addr.area_id}"></fieldset>
							</div>
							<div class="form-group">
								<label class="col-sm-1"><label>详细地址</label></label>
								<input type="text" name="pick_addr" value="{pigcms{$pick_addr.pick_addr}" />
							</div>
							<div class="form-group">
								<label class="col-sm-1"><label>电话号码</label></label>
								<input type="text" name="phone" value="{pigcms{$pick_addr.phone}" />
							</div>
...
var static_public="{pigcms{$static_public}",static_path="{pigcms{$static_path}",merchant_index="{pigcms{:U('Index/index')}",choose_province="{pigcms{:U('Area/ajax_province')}",choose_city="{pigcms{:U('Area/ajax_city')}",choose_area="{pigcms{:U('Area/ajax_area')}",choose_circle="";
</script>
...
$(document).ready(function(){
	$('#edit_form').submit(function(){
		$.post("{pigcms{:U('Config/pick_address_edit')}",$('#edit_form').serialize(),function(result){
			if(result.status == 1){
				alert(result.info);
				window.location.href = "{pigcms{:U('Config/pick_address_edit',array('id'=>$_GET['id']))}";
			}else{
				alert(result.info);
			}
		})
