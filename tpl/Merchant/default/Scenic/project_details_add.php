				...
				<div class="col-xs-12">
					<div class="tabbable">
						<ul class="nav nav-tabs" id="myTab">
							<li class="active">
								<a data-toggle="tab" href="#basicinfo">添加详情</a>
							</li>
					...
					<form enctype="multipart/form-data" class="form-horizontal" method="post" id="edit_form">
						<input type="hidden" name="project_id" value="{pigcms{$_GET['project_id']}"/>
							...
							<div class="form-group">
								...
								<textarea id="project_conter" name="project_conter"  placeholder="写上一些想要发布的内容"></textarea>
							</div>
							<div class="form-group">
								<label class="col-sm-1">项目语音文本</label>
								<textarea class="col-sm-5" rows="5" name="project_conter_audio" id="project_conter_audio"></textarea>
							</div>
							<div class="form-group">
								<label class="col-sm-1"><label>排序</label></label>
								<input type="number" name="project_sort" value="0" />
...
//		uploadJson : "{pigcms{$config.site_url}/index.php?g=Index&c=Upload&a=editor_ajax_upload&upload_dir=merchant/news" , 'link' 'image', , '|', 'emoticons'
	...
	$('#edit_form').submit(function(){
		$.post("{pigcms{:U('project_details_add')}",$('#edit_form').serialize(),function(result){
			if(result.status == 1){
				alert(result.info);
