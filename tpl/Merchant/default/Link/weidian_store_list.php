...
<body style="background:#fff;padding:20px 20px;">
	...
	<table class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
		<thead>
			<tr>
				<th>店铺名称</th>
				<th style=" width:150px;">操作 <span class="tooltips" ><span>
				<p>点击“选中”即可</p>
				</span></span></th>
			</tr>
		</thead>
		<volist name="list" id="m">
		<tr><td>{pigcms{$m.name}</td><td class="norightborder"><a href="###" onclick="returnHomepage('<?php if (!intval($_GET['iskeyword'])){?>{pigcms{$m.linkcode}<?php }else{?>{pigcms{$m.keyword}<?php }?>')">选中</a>&nbsp;&nbsp;<a href="<if condition="$m['wei_page_count']">{pigcms{:U('Link/Weidian_page',array('id'=>$m['id']))}<else/>javascript:alert('该店铺没有添加微页面！');</if>">微页面列表</a>&nbsp;&nbsp;<a href="<if condition="$m['product_count']">{pigcms{:U('Link/Weidian_product',array('id'=>$m['id']))}<else/>javascript:alert('该店铺没有添加商品！');</if>">商品列表</a></td></tr>
		</volist>
	</table>
	<script>
