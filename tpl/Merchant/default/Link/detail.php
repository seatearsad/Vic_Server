...
<tr><td>{pigcms{$m.name}</td><td class="norightborder"><a href="###" onclick="returnHomepage('<?php if (!intval($_GET['iskeyword'])){?>{pigcms{$m.linkcode}<?php }else{?>{pigcms{$m.keyword}<?php }?>')">选中</a> <if condition="$m['sub'] neq NULL"><?php if (!intval($_GET['iskeyword'])){?>&nbsp;<a href="{pigcms{$m.sublink}">详情</a><?php }?></if></td></tr>
</volist>
</table>
