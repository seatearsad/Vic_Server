						...
						<table class="table table-striped table-bordered table-hover">
							<thead>
								<tr>
									...
									<th id="shopList_c0" width="50">所需{pigcms{$config.score_name}</th>
											   ...
											   <if condition="$vo['status'] eq 0">开始活动<else/>结束活动</if>				   
												...
												<if condition="$vo['is_finish']">
													<if condition="$vo['type'] eq 1">
														<a title="查看详情" class="green yiyuan_handle_btn" style="padding-right:8px;" href="{pigcms{:U('Activity/yiyuanduobao',array('id'=>$vo['pigcms_id']))}">
															<i class="ace-icon fa fa-search bigger-130"></i>
														</a>
													</if>
												</if>
												<if condition="$vo.type eq 1">
													<a href="{pigcms{:U('yydb_order_list',array('activity_id'=>$vo['pigcms_id']))}">订单列表</a> 
												</if>
											</td>
										</tr>
									</volist>
