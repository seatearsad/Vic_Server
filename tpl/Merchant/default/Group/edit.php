	...
	<div class="breadcrumbs" id="breadcrumbs">
		<ul class="breadcrumb">
			<li>
				...
				<a href="{pigcms{:U('Group/index')}">商品列表</a>
			</li>
						...
						<ul class="nav nav-tabs" id="myTab">
						  ...
						  <if condition="!empty($levelarr)">
							...
							<if condition="$appoint_list">
								<li>
									<a data-toggle="tab" href="#appointsetting">预约设置</a>
								</li>
							</if>
							<if condition="$now_group['tuan_type'] neq 0">
							<li>
						...
						<div class="tab-content">
							...
							<div id="txtintro" class="tab-pane">
								<div class="form-group">
									...
									<span class="form_tips">如果是{pigcms{$config.group_alias_name}券或代金券，则会生成券密码；如果是实物，则需要填写快递单号(<font color="red">为防止对订单产生影响团购添加后，团购类型不能编辑不能编辑</font>)</span>
								</div>
							...
							<if condition="$appoint_list">
								<div id="appointsetting" class="tab-pane">
									<div class="form-group">
										<label  class="col-sm-1">是否绑定预约：</label>

											<label><input type="radio" name="is_appoint_bind" value="1" class="ace" <if condition='$now_group["is_appoint_bind"] eq 1'>checked="checked"</if>><span class="lbl" style="z-index: 1">开启</span></label>
											<label style=" margin-left:10px;"><input type="radio" name="is_appoint_bind" value="0" class="ace" <if condition='$now_group["is_appoint_bind"] eq 0'>checked="checked"</if>><span class="lbl" style="z-index: 1">关闭</span></label>
											<span class="form_tips" style=" color:red">绑定后，下单将成为预约订单。(设置拼团后改设置无效)</span>
									</div>
									...
									<div class="form-group">
										...
										<select class="col-sm-1" name="appoint_id">
											...
											<volist name="appoint_list" id="vo">
												<option value="{pigcms{$vo.appoint_id}" <if condition='$now_group["appoint_id"] eq $vo["appoint_id"]'>selected="selected"</if>>{pigcms{$vo.appoint_name}</option>
											</volist>
										</select>
									</div>
								</div>
							...
							<else />
								<div id="opengroupnum2" class="tab-pane">
									<div class="form-group">
										...
										<input class="col-sm-1" maxlength="20" name="pin_num" type="num" value="{pigcms{$now_group.pin_num}" disabled="disabled"/><span class="form_tips">拼团人数设置，0表示不设置拼团</span>
									</div>
									<div class="form-group">
										<label  class="col-sm-1">团长优惠比例</label>
										<input class="col-sm-1" maxlength="20" name="start_discount" type="num" value="{pigcms{$now_group.start_discount}" disabled="disabled"/><span class="form_tips">团长优惠 0 为免费，100 为原价</span>
									</div>
									<div class="form-group">
							...
							<if condition="!empty($now_group['trade_info']) ">
								...
								<div class="form-group">
									...
									<select name="bind_trade" id="bind_trade" class="col-sm-2">
										<option value="0">不绑定</option>
											...
											<option value="1" <php>if('appoint'==$now_group['trade_type']){</php>selected="selected"<php>}</php>>预约模块</option>
...
KindEditor.ready(function(K) {
	...
	$('#add_form').submit(function(){
		...
		$.post("{pigcms{:U('Group/edit',array('group_id'=>$now_group['group_id']))}",$('#add_form').serialize(),function(result){
			if(result.status == 1){
				alert(result.info);
