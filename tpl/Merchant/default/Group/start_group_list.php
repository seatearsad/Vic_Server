								...
								<if condition="$start_group_list">
									<volist name="start_group_list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											<td class="start_id">{pigcms{$vo.id}</td>
...
<script type="text/javascript">
	...
	function group_right_now(obj){
		...
		if (r==true){
			$.post("{pigcms{:U('Group/update_start_status')}", {status: 3,id:$(obj).parent('td').parent('tr').find('.start_id').html()}, function(data, textStatus, xhr) {
				if(data){
