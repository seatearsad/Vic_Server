	...
	$.getJSON(getFoodMenuUrl,{order_id:$('#order_id').val()},function(result){
		...
		$('.food_body li').click(function(){
			if($(this).hasClass('hasSpec')){
				...
				cartFunction({productKey:$(this).data('id'),productId:$(this).data('id'),productName:$(this).data('name'),productUnit:$(this).data('unit'),productPrice:$(this).data('price'),productStock:$(this).data('stock_num'),productParam:[],extra_price:$(this).data('extra_price')},'plus');
			}
		});
		...
		if(foodData.tmp_order){
			for(var i in foodData.tmp_order){
				...
				cartFunction({productKey:foodData.tmp_order[i].goods_id+'_'+foodData.tmp_order[i].spec_id+'_tmp_order',productId:foodData.tmp_order[i].goods_id,productName:foodData.tmp_order[i].name,productUnit:foodData.tmp_order[i].unit,productPrice:foodData.tmp_order[i].price,productStock:'-1',productParam:[],productLabel:foodData.tmp_order[i].spec,isTmpOrder:true,tmpOrderId:foodData.tmp_order[i].id,count:parseInt(foodData.tmp_order[i].num),extra_price:foodData.tmp_order[i].extra_price},'plus');
			}
		}
