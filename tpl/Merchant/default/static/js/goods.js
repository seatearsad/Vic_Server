...
KindEditor.ready(function(K) {
	...
	$('#goods_seckill_open_time').click(function(){WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})});
	$('#goods_seckill_close_time').click(function(){WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})});
	/*调整保存按钮的位置*/
...
$(document).ready(function(){
	...
	$(".add_properties").click(function(){
		...
		var t = '<div class="question_box properties"><p class="question_info"><span>'+ getLangStr('ATTRIBUTE_NAME_BKADMIN')+'：</span><input type="text" class="txt properties_name" value="" name="properties[]"/><span>'+getLangStr('QUANTITY_ALLOWED_BKADMIN')+'：</span><input type="text" class="txt properties_num" value="1" name="properties_num[]" style="width:50px"/>'
				+'<input type="hidden" name="properties_id[]" ><a href="javascript:;" class="box_del">'+getLangStr('DELETE_BKADMIN')+'</a></p>'
				+'<ul id="1" class="optionul"><li><u>'+ getLangStr('ATTRIBUTE_VALUE_BKADMIN')+'：</u><input type="text" class="txt properties_val" name="properties_val['+i+'][]"/> <a class="list_del" href="javascript:;" title="'+getLangStr('DELETE_BKADMIN')+'">×</a></li>'
				+'</ul><p class="bot_add"><a href="javascript:;" class="btn btn-sm btn-success">  '+getLangStr('ADD_VALUE_BKADMIN')+'</a></p></div>';
	...
	$(".add_spec").click(function(){
		var i = $('.spec').size();
		var t = '<div class="question_box spec"><p class="question_info"><span>'+getLangStr('SPECIFICATION_NAME_BKADMIN')+'：</span><input type="text" class="txt spec_name" value="" name="specs[]"/>'
				+'<input type="hidden" name="spec_id[]" /><a href="javascript:;" class="box_del">'+getLangStr('DELETE_BKADMIN')+'</a></p>'
				+'<ul id="1" class="optionul"><li><u>'+getLangStr('SPECIFICATION_VALUE_BKADMIN')+'：</u><input type="hidden" class="hide_txt spec_val_id" name="spec_val_id['+i+'][]" value=""> <input type="text" class="txt spec_val" name="spec_val['+i+'][]"/> <a class="list_del" href="javascript:;" title="'+getLangStr('DELETE_BKADMIN')+'">×</a></li>'
				+'</ul><p class="bot_add"><a href="javascript:;" class="btn btn-sm btn-success">  '+getLangStr('ADD_VALVE_BKADMIN')+'</a></p></div>';
		$(".add_spec").before(t);
		if ($('.spec').size() > 0) {
	...
	$('.add_table').click(function(){
		var header_html = '<tr><th style="display:none">商品编码</th>';
		formathtml = new Array(), format_value = new Array();
		var flag = false;
		$('.spec_name').each(function(){
			if ($(this).val().length < 1) {
				alert('Please input '+ getLangStr('SPECIFICATION_NAME_BKADMIN'));
				flag = true;
				return false;
			}
			header_html += '<th>' + $(this).val() + '</th>';
		});
		header_html += '<th style="display:none">原价</th><th style="display:none">进价</th><th>'+ getLangStr('CURR_PRICE_BKADMIN')+'</th><th style="display:none">限时价</th><th>'+ getLangStr('STOCK_BKADMIN')+'</th>';
		$('.properties_name').each(function(){
			if ($(this).val().length < 1) {
				alert('Please input'+ getLangStr('ATTRIBUTE_NAME_BKADMIN'));
				flag = true;
				return false;
			}
			header_html += '<th>' + $(this).val() + '('+getLangStr('QUANTITY_ALLOWED_BKADMIN')+')</th>';
		});
		header_html += '</tr>';
		...
		$('.spec').each(function(i){
			...
			$(this).children('ul').children('li').children('.spec_val').each(function(){
				if ($(this).val().length < 1) {
                    alert('Please input'+ getLangStr('SPECIFICATION_VALUE_BKADMIN'));
					//alert('请输入规格值的名称');
					flag = true;
					return false;
				}
		...
		for (var e = 0; e < formathtml.length; e++) {
			header_html += '<tr id="'+format_value[e]+'">' +
				'<td style="display:none"><input type="text" style="display: none;" class="txt" name="numbers[]" value="" style="width:100%;"></td>'
				+ formathtml[e] +
				'<td style="display:none"><input type="text" class="txt" name="old_prices[]" value="" style="width:80px;"></td>' +
				'<td style="display:none" ><input type="text" class="txt" name="cost_prices[]" value="" style="width:80px;"></td>' +
				'<td><input type="text" class="txt" name="prices[]" value="" style="width:80px;"></td>' +
				'<td style="display:none"><input type="text" class="txt" name="seckill_prices[]" value="" style="width:80px;"></td>' +
				'<td><input type="text" class="txt" name="stock_nums[]" value="-1" style="width:80px;"></td>';
	...
	$('.add_table_foodshop').click(function(){
		...
		header_html += '<th>原价</th><th>现价</th><th style="display:none">限时价</th><th style="display:none">库存</th>';
		$('.properties_name').each(function(){
			if ($(this).val().length < 1) {
				alert('请输入属性名称');
				flag = true;
		...
		format_html(spec_val_arr, 0, '');
		for (var e = 0; e < formathtml.length; e++) {
			header_html += '<tr id="'+format_value[e]+'"><td><input type="text" class="txt" name="numbers[]" value="" style="width:100%;"></td>' + formathtml[e] + '<td><input type="text" class="txt" name="old_prices[]" value="" style="width:80px;"></td><td><input type="text" class="txt" name="prices[]" value="" style="width:80px;"></td><td style="display:none"><input type="text" class="txt" name="seckill_prices[]" value="" style="width:80px;display:none;"></td><td style="display:none"><input type="text" class="txt" name="stock_nums[]" value="-1" style="width:80px;"></td>';
			$('.properties_name').each(function(t){
				header_html += '<td><input type="text" class="txt" name="num' + t + '[]" value="1" style="width:80px;"></td>';
			});
			header_html += '</tr>';
...
function show_property_html(cat_id)
{
	$.post(ajax_goods_properties, {'cat_id':cat_id, 'goods_id':$('#goods_id').val()}, function(response){
		if (response.error_code == false) {
			...
			$.each(data, function(i, item){
				html += '<div class="form-group properties">';
