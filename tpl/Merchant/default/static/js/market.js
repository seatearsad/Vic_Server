...
var shop_mall = {
	...
	'go_pay':function(paymethod, auth_code, offline_pay){
		...
		$.post(arrival_pay,{order_id:order_id, 'auth_code':auth_code, 'auth_type':paymethod, 'change_reason':change_reason, 'price':price, 'auth_type':paymethod, 'card_id':card_id, 'uid':uid, 'discount':discount, 'coupon':coupon, 'card_money':card_money, 'offline_pay':offline_pay},function(result){
				...
				$.post(shop_order_save, {data:left_cart_data}, function(response){
					...
					if (response.error_code) {
						...
						if (response.discount_msg != '') {
							$('.disk').html(response.discount_msg + '之后的价格');
