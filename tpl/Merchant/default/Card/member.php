						...
						<div class="tab-content">
							<div class="tab-pane active">
								<div id="shopList" class="grid-view">
									...
									<table class="table table-striped table-bordered table-hover">
										<thead>
											<tr>
												...
												<th id="shopList_c3" width="100">{pigcms{$config['score_name']}</th>
									...
									<h5 style="margin-top:20px;">{pigcms{$config['score_name']}、线下消费记录</h5>
									<table class="table table-striped table-bordered table-hover">
										<thead>
											<tr>
												<th id="shopList_c1" width="100">日期</th>
												<th id="shopList_c1" width="100">金额(元)</th>
												<th id="shopList_c1" width="100">获取{pigcms{$config['score_name']}</th>
											...
											<if condition="$records">
												<volist name="records" id="r">
													<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
														...
														<td><a href="javascript:drop_confirm('您确定要删除吗?', '/merchant.php?g=Merchant&c=Card&a=useRecord_del&itemid={pigcms{$r.id}&id={pigcms{$thisCard.id}');" class="red"><i class="ace-icon fa fa-trash-o bigger-130"></i></a></td>
													</tr>
												</volist>
											<else/>
												<tr class="odd"><td class="button-column" colspan="8" >{pigcms{:L('NO_CONTENT_BKADMIN')}</td></tr>
