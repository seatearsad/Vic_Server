		...
		<div class="page-content-area">
			<div class="row">
				<form id="frmselect" method="get" action="" style="margin-bottom:0;">
					...
					<input type="hidden" name="a" value="group_reply"/>
					<select id="group_id" name="group_id">
						<option value="all">全部{pigcms{$config.group_alias_name}</option>
								...
								<if condition="$reply_list">
									<volist name="reply_list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<td><a style="width: 60px;" class="label label-sm label-info" title="评论列表" href="{pigcms{:U('Message/group_reply_detail',array('id'=>$vo['pigcms_id']))}">评论详情</a></td>
										</tr>
									</volist>
								<else/>
