		...
		<div class="page-content-area">
			<div class="row">
				<div class="col-xs-12">
					...
					<a class="btn btn-success" href="{pigcms{:U('Card_new/recharge_list')}" >会员卡充值记录</a>
					...
					<div id="shopList" class="grid-view">
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								<if condition="$card_user_list">
									<volist name="card_user_list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											<td>{pigcms{$vo.id}</td>
											<td><if condition="$vo['uid'] neq ''">{pigcms{$vo.nickname}<else /><a href="{pigcms{:U('Card_new/see_qrcode',array('id'=>$vo['id']))}" data-id = "{pigcms{$vo.id}" class="see_qrcode">查看二维码(用户扫描后绑定)</a></if></td>
								...
								<else/>
									<tr class="odd"><td class="button-column" colspan="9" >{pigcms{:L('NO_CONTENT_BKADMIN')}</td></tr>
								</if>
							</tbody>
						</table>
						{pigcms{$pagebar}
