...
<tr><td>{pigcms{$m.name}</td><td class="norightborder"><if condition="$m['sub'] neq NULL && !intval($_GET['iskeyword'])"><a href="{pigcms{$m.sublink}">详情</a>&nbsp;&nbsp;&nbsp;</if><a href="###" onclick="returnHomepage('<?php if (!intval($_GET['iskeyword'])){?>{pigcms{$m.linkcode}<?php }else{?>{pigcms{$m.keyword}<?php }?>')">选中</a></td></tr>
</volist>
</table>
