<include file="Public:header"/>
		...
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					...
					<a href="{pigcms{:U('shop_edit',array('id'=>$now_special['pigcms_id']))}" class="on">编辑专题页面</a>|
				</ul>
			</div>
			<div class="app-design clearfix">
				<div class="app-preview">
					...
					<div class="app-entry">
						<div class="app-config js-config-region" data-name="{pigcms{$now_special.name}" data-desc="{pigcms{$now_special.desc}" data-bgcolor="{pigcms{$now_special.bgcolor}">
							<div class="app-field clearfix">
								<h1><span>{pigcms{$now_special.name}</span></h1>
							</div>
						</div>
						<div class="app-fields js-fields-region">
							<div class="app-fields ui-sortable">
								<div class="app-field clearfix">
									<div class="control-group">
										<div class="custom-image-swiper" title="点击上传图片，不上传则留空" data-image="{pigcms{$now_special.image}">
											<if condition="$now_special['image']">
												<img style="display:block;width:100%;" src="{pigcms{$now_special.image}"/>
											<else/>
								...
								<div class="app-field clearfix">
									<div class="control-group">
										<div class="coupon_list clearfix">
											<volist name="now_special['coupon']" id="vo">
												<a href="javascript:;" class="couponRow" data-id="{pigcms{$vo.id}" data-name="{pigcms{$vo.name}" data-order_money="{pigcms{$vo.order_money}" data-discount="{pigcms{$vo.discount}">
													<p class="coupon_name pull-left"><span>{pigcms{$vo.name}</span></p>
													<p class="coupon_monery pull-left">{pigcms{$vo.order_money}</p>
													<p class="coupon_use pull-left oneline">满{pigcms{$vo.discount}元可用</p>
													<span class="icon2"></span>
													<div class="delete" title="删除">×</div>
												</a>
											</volist>
										</div>
										<div class="addCouponBtn" title="点击添加优惠券" <if condition="count($now_special['coupon']) eq 9">style="display:none;"</if>></div>
									</div>
								</div>
							</div>
							<div class="app-fields ui-sortable addCategoryBox">
								<div class="app-field clearfix">
									<div class="control-group">
										<div class="addCategoryBtn col-{pigcms{:count($now_special['product_list'])}" title="点击添加分类">
											<ul></ul>
											<div class="right">+</div>
										</div>
									</div>
		...
		<script type="text/javascript">
			$(function(){
				var productList = $.parseJSON('{pigcms{:json_encode($now_special['product_list'])}');
				for(var i in productList){
					$('.addCategoryBtn ul').append('<li>'+productList[i].name+'</li>');
					$('.addCategoryBtn ul li:eq(0)').addClass('curr');
					for(var j in productList[i].product){
						var theme_data = {id:productList[i].product[j].id,name:productList[i].product[j].name,image:productList[i].product[j].image,catIndex:i};
						laytpl($('#productTpl').html()).render(theme_data, function(html){
							$('.addProductBtn').closest('.app-fields').before(html);
						});
					}
					$('.productRow').hide();
					$('.productRow.cat-0').show();
					changeAddFixed();
				}
				...
				$('.app-actions .btn-save').click(function(){
					...
					var postData = {};
					postData.pigcms_id = "{pigcms{$now_special.pigcms_id}";
					...
					$.post("{pigcms{:U('amend')}",postData,function(result){
						if(result.status == 1){
			...
			function select_product(id,name,image){
				...
				if($('.productRow.cat-'+catIndex).size() >= 20){
