			...
			<div id="nav" class="mainnav_title">
				<ul>
					<if condition="empty($_GET['mer_id'])">
						<a href="{pigcms{:U('Group/product')}" class="on">商品列表</a>
					<else/>
						<a href="{pigcms{:U('Group/product')}">商品列表</a> |
						<a href="{pigcms{:U('Group/product',array('mer_id'=>$_GET['mer_id']))}" class="on">指定商家的商品列表</a>
					</if>
				</ul>
			</div>
							...
							<if condition="is_array($group_list)">
								<volist name="group_list" id="vo">
									<tr>
										...
										<td class="textcenter"><a href="{pigcms{:U('Group/order_list',array('group_id'=>$vo['group_id']))}">订单列表</a> | <a href="{pigcms{:U('Group/reply_list',array('group_id'=>$vo['group_id']))}">评论列表</a> | <a href="{pigcms{:U('Merchant/merchant_login',array('mer_id'=>$vo['mer_id'],'group_id'=>$vo['group_id']))}">编辑</a> | 
										<a href="/admin.php?g=System&c=Group&a=product&action=del&id={pigcms{$vo.group_id}" onclick="return confirm('确定要删除吗？')">删除</a>
										</td>
									</tr>
								</volist>
