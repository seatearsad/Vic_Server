$
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<if condition="empty($_GET['mer_id'])">
						<a href="{pigcms{:U('Group/product')}" class="on">待审核团购列表</a>
					<else/>
						<a href="{pigcms{:U('Group/product')}">待审核团购列表</a> |
						<a href="{pigcms{:U('Group/product',array('mer_id'=>$_GET['mer_id']))}" class="on">指定商家的团购列表</a>
					</if>
				</ul>
			</div>
			<table class="search_table" width="100%">
				<tr>
					<td>
						<form action="{pigcms{:U('Group/product')}" method="get">
							...
							<if condition="is_array($group_list)">
								<volist name="group_list" id="vo">
									<tr>
										...
										<td><a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{$config.site_url}/index.php?g=Index&c=Recognition&a=see_qrcode&type=group&id={pigcms{$vo.group_id}&img=1','查看二维码',430,433,true,false,false,null,'merchant_qrcode',true);" class="see_qrcode">查看二维码</a></td>
										<td>
										...
										<td><switch name="vo['status']"><case value="0"><font color="red">关闭</font></case><case value="1"><font color="green">正常</font></case><case value="2"><font color="red">审核中</font><a href="javascript:void(0);" class="delete_row" parameter="id=10" url="{pigcms{:U('Group/verify',array('group_id'=>$vo['group_id']))}">通过审核</a></case></switch></td>
										<td class="textcenter"><a href="{pigcms{:U('Group/order_list',array('group_id'=>$vo['group_id']))}">订单列表</a> | <a href="{pigcms{:U('Group/reply_list',array('group_id'=>$vo['group_id']))}">评论列表</a> | <a href="{pigcms{:U('Merchant/merchant_login',array('mer_id'=>$vo['mer_id'],'group_id'=>$vo['group_id']))}">编辑</a></td>
									</tr>
								</volist>
