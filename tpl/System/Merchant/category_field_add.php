<include file="Public:header"/>
	<div id="nav" class="mainnav_title">
		<ul>
			<a href="{pigcms{:U('category_field_add',array('cat_id'=>$_GET['cat_id'],'type'=>'0'))}" <if condition="$_GET['type'] eq 0">class="on"</if>>自定义字段</a>|
			<a href="{pigcms{:U('category_field_add',array('cat_id'=>$_GET['cat_id'],'type'=>'1'))}" <if condition="$_GET['type'] eq 1">class="on"</if>>选择内置字段</a>
		</ul>
	</div>
	<form id="myform" method="post" action="{pigcms{:U('category_field_modify')}" frame="true" refresh="true">
		<input type="hidden" name="cat_id" value="{pigcms{$_GET.cat_id}"/>
		<if condition="$_GET['type'] eq 0">
			<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
				<tr>
					<td colspan="2">请谨慎小心填写，无法编辑删除</td>
				</tr>
				<tr>
					<th width="80">字段名称</th>
