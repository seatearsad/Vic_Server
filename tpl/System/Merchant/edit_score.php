<include file="Public:header"/>
	<form id="myform"  frame="true" refresh="true">
		  ...
		  <table cellpadding="0" cellspacing="0" class="table_form" width="100%" id="tab_mer_pr" style="display: table;">
		   <tbody>
			...
			<tr>
			 ...
			 <input type="text" class="input-text" name="merchant_score_max" id="config_merchant_score_max" value="{pigcms{$mer_pr.merchant_score_max}" size="10" validate="number:true" tips="商家最大{pigcms{$config.score_name}使用数（填写大于等于0的整数，填-1 则跳过当前业务设置,按上级平台的{pigcms{$config.score_name}最大使用数算算）" <php>if($mer_pr['merchant_score_max']<0||$mer_pr['merchant_score_max']==""){</php>style="display:none"<php>}</php> /></td>
			</tr>
			
		
			<tr>
			 <th width="160">团购最大{pigcms{$config.score_name}使用数：</th>
			 <td>
				<span class="cb-enable">
			 ...
			 <input type="text" class="input-text" name="merchant_group_score_max" id="config_group_rate" value="{pigcms{$mer_pr.merchant_group_score_max}" size="10" validate="number:true" tips="团购最大{pigcms{$config.score_name}使用数（填写大于等于0的整数，填-1 则跳过当前业务设置,按上级【商家最大{pigcms{$config.score_name}使用数】算）" <php>if($mer_pr['merchant_group_score_max']<0||$mer_pr['merchant_group_score_max']==""){</php>style="display:none"<php>}</php>/></td>
			</tr>
		
			<tr>
			 <th width="160">快店最大{pigcms{$config.score_name}使用数：</th>
			 <td>
			 ...
			 <input type="text" class="input-text" name="merchant_shop_score_max" id="config_shop_rate" value="{pigcms{$mer_pr.merchant_shop_score_max}" size="10" validate="number:true" tips="快店最大{pigcms{$config.score_name}使用数（填写大于等于0的整数，填-1 则跳过当前业务设置,按上级【商家最大{pigcms{$config.score_name}使用数】算）" <php>if($mer_pr['merchant_shop_score_max']<0||$mer_pr['merchant_shop_score_max']==""){</php>style="display:none"<php>}</php>/></td>
			</tr>
		
			<tr>
			 <th width="160">餐饮最大{pigcms{$config.score_name}使用数：</th>
			 <td>
				<span class="cb-enable">
			 ...
			 <input type="text" class="input-text" name="merchant_meal_score_max" id="config_meal_rate" value="{pigcms{$mer_pr.merchant_meal_score_max}" size="10" validate="number:true" tips="餐饮最大{pigcms{$config.score_name}使用数（填写大于等于0的整数，填-1 则跳过当前业务设置,按上级【商家最大{pigcms{$config.score_name}使用数】算）" 
		...
		<if condition="C('config.appoint_page_row')">
			...
			<tr>
			 <th width="160">预约最大{pigcms{$config.score_name}使用数：</th>
			 <td>
				<span class="cb-enable">
			 ...
			 <input type="text" class="input-text" name="merchant_appoint_score_max" id="config_appoint_rate" value="{pigcms{$mer_pr.merchant_appoint_score_max}" size="10" validate="number:true" tips="预约最大{pigcms{$config.score_name}使用数（填写大于等于0的整数，填-1 则跳过当前业务设置,按上级【商家最大{pigcms{$config.score_name}使用数】算）" <php>if($mer_pr['merchant_appoint_score_max']<0||$mer_pr['merchant_appoint_score_max']==""){</php>style="display:none"<php>}</php>/></td>
			</tr>
			</if>
		   </tbody>
		  </table> 
