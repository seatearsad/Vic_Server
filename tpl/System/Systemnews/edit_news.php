	...
	<form id="myform" method="post" action="{pigcms{:U('Systemnews/edit_news')}" frame="true" refresh="true">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			<tr>
				...
				<td><input type="text" class="input fl" name="title" value="{pigcms{$news.title}" size="75" placeholder="{pigcms{:L('I_TITLE')}" validate="maxlength:50,required:true"/></td>
			</tr>
            <tr>
                <th width="80">{pigcms{:L('I_SUBTITLE')}</th>
                <td><input type="text" class="input fl" name="sub_title" value="{pigcms{$news.sub_title}" size="75" placeholder="{pigcms{:L('I_SUBTITLE')}" validate="maxlength:50,required:true"/></td>
            </tr>
            <tr>
                <th width="80">{pigcms{:L('I_KEYWORDS')}</th>
                <td><input type="text" class="input fl" name="keyword" size="75" value="{pigcms{$news.keyword}" placeholder="{pigcms{:L('I_KEYWORDS')}" validate="maxlength:100,required:true"/></td>
            </tr>
            <tr>
                <th width="80">{pigcms{:L('I_BRIEF_INTRODUCTION')}</th>
                <td><input type="text" class="input fl" name="desc" size="75" value="{pigcms{$news.desc}" placeholder="{pigcms{:L('I_BRIEF_INTRODUCTION')}" validate="maxlength:150,required:true"/></td>
            </tr>
			<tr>
            ...
            <tr>
                ...
                <td>
                    <span class="cb-enable"><label class="cb-enable <if condition="$news['is_commend'] eq 1">selected</if>"><span>{pigcms{:L('I_RECOMMENDED')}</span><input type="radio" name="is_commend" value="1" checked="checked"/></label></span>
                    <span class="cb-disable"><label class="cb-disable <if condition="$news['is_commend'] eq 0">selected</if>"><span>{pigcms{:L('C_CATEGORYDIS')}</span><input type="radio" name="is_commend" value="0" /></label></span>
                </td>
            </tr>
		</table>
