<include file="Public:header"/>
	<form id="myform" method="post" action="{pigcms{:U('Portal/activity_edit')}" enctype="multipart/form-data">
		...
		<input type="hidden" name="a_id" value="{pigcms{$activityInfo.a_id}">
			...
			<tr>
				<th width="80">活动状态</th>
				<td>
					<span class="cb-enable"><label class="cb-enable selected"><span>启用</span><input type="radio" name="status" value="1" <if condition="$activityInfo['status'] eq 1">checked="checked"</if> /></label></span>
					<span class="cb-disable"><label class="cb-disable"><span>关闭</span><input type="radio" name="status" value="0" <if condition="$activityInfo['status'] eq 0">checked="checked"</if> /></label></span>
				</td>
			</tr>
		</table>
