		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<if condition="$now_area['area_type'] gt 1">
				<tr>
					...
					<td><input type="text" class="input fl" name="area_url" value="{pigcms{$now_area.area_url}" size="20" placeholder="" validate="maxlength:20,required:true" tips="{pigcms{:L('GITLIOT')}"/></td>
				</tr>
			</if>
			<if condition="$now_area['area_type'] gt 1 && $now_area['area_type'] lt 4">
				<tr>
					<th width="80">{pigcms{:L('I_IP_SYMPOL')}</th>
					<td><input type="text" class="input fl" name="area_ip_desc" value="{pigcms{$now_area.area_ip_desc}" size="20" placeholder="" validate="maxlength:30,required:true" tips="{pigcms{:L('TGFIXC')}"/></td>
				</tr>
			</if>
			<tr>
				<th width="80">{pigcms{:L('I_LISTING_ORDER')}</th>
				<td><input type="text" class="input fl" name="area_sort" value="{pigcms{$now_area.area_sort}" size="10" value="0" validate="required:true,number:true,maxlength:6" tips="{pigcms{:L('HIGHVAL')}"/></td>
			</tr>
            <!--tr>
