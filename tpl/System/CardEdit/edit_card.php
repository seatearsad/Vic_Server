	...
	<form id="myform" method="post" action="{pigcms{:U('Cardedit/edit_card')}" frame="true" refresh="true">
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<tr>
				<td width="80">余额：</td>
				<td>
				<if condition="$physical_card['balance_money'] gt 0"> {pigcms{$physical_card['balance_money']}<else /> <input type="text" class="input fl" name="balance_money" value="{pigcms{$physical_card['balance_money']}" autocomplete="off" validate="digits:true"  ></if>
				</td>
			</tr>
