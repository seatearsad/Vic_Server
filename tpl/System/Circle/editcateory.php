...
<script src="{pigcms{$static_public}kindeditor/lang/zh_CN.js"></script>
	<form id="myform" method="post" action="{pigcms{:U('Circle/docateoryedit')}" frame="true" refresh="true">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<tr>
				<th width="80">图标</th>
				<td >
				<input type="hidden" class="input fl" name="logo" id="icon_value" value="{pigcms{$now_category.icon_value}" size="25" placeholder="" />
                
				<img src="{pigcms{$config.site_url}{pigcms{$now_category.icon_value}" id="icon" style="height:30px;width:30px;">
                
				<a href="javascript:void(0)" class="button" id="image1">更换图标</a>
				</td>
			</tr>
			
			<if condition="$now_category['fid'] neq 0">
			<tr>
				<th width="80">选择分类</th>
				<td>
				<select name="fid" validate="required:true">
				<volist name="fu" id="vo">
				<if condition="$vo['id'] eq $now_category['fid']">
