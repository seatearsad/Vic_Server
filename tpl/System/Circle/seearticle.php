  ...
  <input type="hidden" name="uid" value="{pigcms{$now_category.uid}"/>
    <table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
        ...
        <tr>
            <th width="800">封面图</th>
            <td id="fengmian">
           <?php if($now_category['image']){ ?>
            <input type="text" value="{pigcms{$now_category.image}" style="width:400px;" name="image"/><img src='./{pigcms{$now_category.image}' style='width:30px;height:30px;'><input type="button" value="×" onclick="mydel(this);">
           <?php } ?>
            </td>
        </tr>
        <tr>
            <th width="80">更换封面图</th>
            <td>
		...
		<?php if($imgurl) { foreach($imgurl as $key=>$list){?>
		<input type="text" value="<?php echo $list ?>" style="width:400px;" name="img_url[]"/><img src='./<?php echo $list ?>' style='width:30px;height:30px;'><input type="button" value="×" onclick="mydel(this);"><br/>
		<?php } }?>
       
            </td>
        </tr>
        <tr>
            <th width="80">增加展示图</th>
            <td>
       ...
       <tr>
				...
				<td>
					<span class="cb-enable">
					<label class="cb-enable <if condition="$now_category['status'] eq 0">selected</if>"><span>显示</span><input type="radio" name="status" value="0"  <if condition="$now_category['status'] eq 1">checked="checked"</if> /></label>
					</span>
					<span class="cb-disable">
                    ...
                    editor.loadPlugin('image', function() {
                        editor.plugin.imageDialog({
                            ...
                            clickFn : function(url, title, width, height, border, align) {
                                ...
                                var html="<input type='text' value='"+url+"' style='width:400px;' name='img_url[]'>";
                                var html2="<input type='button' value='×' onclick='mydel(this);'><br/>";
                                var pic = "<img src='./"+url+"' style='width:30px;height:30px;'>";
                    ...
                    editor.loadPlugin('image', function() {
                        editor.plugin.imageDialog({
                            ...
                            clickFn : function(url, title, width, height, border, align) {
                                ...
                                var html="<input type='text' value='"+url+"' style='width:400px;' name='image'>";
                                var html2="<input type='button' value='×' onclick='mydel(this);'><br/>";
                                var pic = "<img src='./"+url+"' style='width:30px;height:30px;'>";
                                K('#fengmian').html(html);
