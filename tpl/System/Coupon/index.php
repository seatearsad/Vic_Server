					...
					<td>
						<form action="{pigcms{:U('Coupon/index')}" method="get">
							...
							<select name="searchtype">
								...
								<option value="coupon_id" <if condition="$_GET['searchtype'] eq 'coupon_id'">selected="selected"</if>>消息ID</option>
							</select>
							...
							<if condition="is_array($coupon_list)">
								<volist name="coupon_list" id="vo">
									<tr>
										...
										<td class="textcenter"><a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Coupon/edit',array('coupon_id'=>$vo['coupon_id']))}','编辑优惠券信息',800,500,true,false,false,editbtn,'edit',true);">编辑</a></td>
									</tr>
								</volist>
