<include file="Public:header"/>
	<form id="myform" method="post" action="{pigcms{:U('area_amend')}" frame="true" refresh="true">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<if condition="$now_area['area_type'] gt 1">
				<tr>
					<th width="80">热门</th>
					<td>
						<span class="cb-enable"><label class="cb-enable <if condition="$now_area['is_hot'] eq 1">selected</if>"><span>是</span><input type="radio" name="is_hot" value="1" <if condition="$now_area['is_hot'] eq 1">checked="checked"</if> /></label></span>
						<span class="cb-disable"><label class="cb-disable <if condition="$now_area['is_hot'] eq 0">selected</if>"><span>否</span><input type="radio" name="is_hot" value="0" <if condition="$now_area['is_hot'] eq 0">checked="checked"</if>/></label></span>
					</td>
				</tr>
			</if>
			<tr>
				...
				<tr>
					<th width="80">国外</th>
					<td>
						<span class="cb-enable"><label class="cb-enable <if condition="$now_area['is_abroad'] eq 1">selected</if>"><span>国外</span><input type="radio" name="is_abroad" value="1" <if condition="$now_area['is_abroad'] eq 1">checked="checked"</if> /></label></span>
						<span class="cb-disable"><label class="cb-disable <if condition="$now_area['is_abroad'] eq 2">selected</if>"><span>国内</span><input type="radio" name="is_abroad" value="2" <if condition="$now_area['is_abroad'] eq 2">checked="checked"</if> /></label></span>
					</td>
				</tr>
		</table>
