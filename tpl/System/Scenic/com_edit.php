<include file="Public:header"/>
	<form id="myform" method="post" action="{pigcms{:U('com_edit')}" frame="true" refresh="true" autocomplete="off">
		<input type="hidden" name="com_id" value="{pigcms{$_GET['com_id']}" />
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			<tr>
				<th width="80">商品名</th>
				<td><input type="text" class="input fl" value="{pigcms{$list['com_name']}" name="com_name" id="com_name" size="20" placeholder="请输入名称" validate="maxlength:30,required:true"/></td>
			</tr>
			<tr>
				<th width="80">简单描述</th>
				<td><input type="text" class="input fl" value="{pigcms{$list['com_title']}" name="com_title" id="com_title" size="20" placeholder="请输入名称" validate="maxlength:200,required:true"/></td>
			</tr>
			<tr>
				<th width="80">排序</th>
				<td><input type="number" class="input fl" value="{pigcms{$list['sort']}" name="sort" id="sort" size="20" placeholder="" tips="值越大越靠前"/></td>
			</tr>
			<tr>
				<th width="80">价格</th>
				<td><input type="text" class="input fl" value="{pigcms{$list['price']}" name="price" id="price" size="20" placeholder="" tips="值越大越靠前" validate="maxlength:30,required:true"/></td>
			</tr>
			<tr>
				<th width="15%">图片</th>
				<td width="85%" colspan="3">
				    <input type="hidden" name="com_img" value="{pigcms{$list['com_img']}"/>
			...
			<tr>
				<th width="80">链接地址</th>
				<td>
					<input type="text" class="input fl" name="url" id="url" value="{pigcms{$list['url']}" style="width:200px;" placeholder="请填写链接地址" validate="maxlength:200,required:true,url:true"/>
