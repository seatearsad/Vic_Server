<include file="Public:header"/>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<a href="{pigcms{:U('User/authentication_car')}" <if condition="$status neq 1">class="on"</if>>待审核列表</a>
					<a href="{pigcms{:U('User/authentication_car',array('status'=>1))}" <if condition="$status eq 1">class="on"</if>>已审核列表</a>
				</ul>
			</div>
							...
							<if condition="is_array($card_list)">
								<volist name="card_list" id="vo">
									<tr>
										...
										<if condition="$vo['status'] eq 0">
											<td class="textcenter">
												<a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('User/car_check',array('car_id'=>$vo['car_id']))}','审核实名认证',680,560,true,false,false,editbtn,'edit',true);">审核</a>
											</td>
										<else/>
											<td class="textcenter">
												<a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('User/car_check',array('car_id'=>$vo['car_id'],'statuss'=>1))}','查看实名认证',680,560,true,false,false);">查看</a>
											</td>
										</if>
									</tr>
								</volist>
