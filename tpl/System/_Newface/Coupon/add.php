...
<div id="wrapper-singlepage">
                        ...
                        <div>
                            <form id="myform" method="post" action="{pigcms{:U('Coupon/add')}" frame="true" refresh="true">
                                ...
                                <div class="btn tutti_hidden_obj">
                                    <input type="button" name="dosubmit" id="dosubmit" value="{pigcms{:L('_BACK_SUBMIT_')}" class="button" onclick="checkSubmit()" />
	...
	<script type="text/javascript">
        ...
        function checkSubmit(){
            var min_money = parseFloat($('input[name="order_money"]').val());
            var discount = parseFloat($('input[name="discount"]').val());

            if(discount > min_money){
                alert("{pigcms{:L('COUPON_ADD_TIP')}");
            }else{
                $('#myform').submit();
            }
        }
