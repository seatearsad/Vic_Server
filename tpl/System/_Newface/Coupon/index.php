    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-lg-3" style="height 90px;margin-top:40px;">
            <button class="btn btn-white active">
                <a href="{pigcms{:U('Coupon/had_pull')}" style="color: inherit">{pigcms{:L('_BACK_PICK_COU_LIST_')}</a>
            </button>
            <a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Coupon/add')}','{pigcms{:L(\'_BACK_ADD_COUPON_\')}',800,500,true,false,false,addbtn,'edit',true);"><button type="button" class="btn btn-primary btn-sm float-right">{pigcms{:L('_BACK_ADD_COUPON_')}</button></a>
        </div>
    </div>
                            ...
                            <if condition="is_array($coupon_list)">
                                <volist name="coupon_list" id="vo">
                                    <tr>
                                        ...
                                        <td class="textcenter">
                                            <if condition="$vo['allow_new'] eq 1"><font color="green">Yes</font><else /><font color="red">No</font></if>
                                        </td>
                                        <td class="textcenter">{pigcms{$vo.city_name}</td>
                                        <td class="textcenter">
                                            <if condition="$vo['status'] eq 1"><span class="label label-primary">{pigcms{:L('_BACK_ACTIVE_')}</span><elseif condition="$vo['status'] eq 2"/><span class="label label-warning">{pigcms{:L('_EXPIRED_TXT_')}</span><elseif condition="$vo['status'] eq 3" /><span class="label label-warning">领完了</span><else /><span class="label label-warning">{pigcms{:L('_BACK_FORBID_')}</span></if>
                                        </td>
                                        <td class="textcenter">
                                            <a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Coupon/edit',array('coupon_id'=>$vo['coupon_id']))}','{pigcms{:L(\'_BACK_EDIT_COU_INFO_\')}',800,500,true,false,false,editbtn,'edit',true);"><button class="btn btn-white text-grey" type="button">{pigcms{:L('_BACK_EDIT_')}</button></a>
                                        </td>
                                    </tr>
                                </volist>
