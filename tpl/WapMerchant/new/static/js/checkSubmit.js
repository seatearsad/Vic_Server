...
function checkSubmit(){
	...
	if($("input[name='email']")[0] && $("input[name='email']").val() == ''){
		message += '邮 箱<br>';
	}else{
	...
	if($("input[name='phone']")[0] && $("input[name='phone']").val() != '' ){
	...
	if(type_id=='mer_reg'){
		...
		postdata.area_id=$("input[name='area_id']").val();
		postdata.circle_id=$("input[name='circle_id']").val();
		postdata.city_id=$("input[name='city_id']").val();
		postdata.province_id=$("input[name='province_id']").val();
