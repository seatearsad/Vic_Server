...
<body>
	...
	<div class="container container-fill" style='padding-top:50px'>
		...
		<form class="pigcms-form" method="post" action="{pigcms{:U('Index/meal_add')}">
			...
			<div class="pigcms-container">
				...
				<input class="pigcms-input-block" size="20" name="price" id="price" type="text" value="{pigcms{$now_meal.price|floatval}"/>
			</div>
			<div class="pigcms-container">
				<p class='pigcms-form-title'>商品原价：</p>
				<input class="pigcms-input-block" size="20" name="old_price" id="old_price" type="text" value="{pigcms{$now_meal.old_price|floatval}"/>
			</div>
			...
			<div class="pigcms-container">
			 ...
			 <if condition="!empty($stores) AND empty($now_meal) AND !($goods_id gt 0)">
			<div class="pigcms-container">
			...
			<elseif condition="!empty($now_meal) AND $goods_id gt 0" />
	...
	<script type="text/javascript">
		var picarr = [];
		var attachurl = "{pigcms{$site_URl}", localIds, pic_detail = [], Img_Classify = 'foodshop_goods', upload_url = "{pigcms{:U('Index/img_uplode')}";
			//$("input[name='pic_detail']").val(pic_detail);
		var sortid = '{pigcms{$now_meal["sort_id"]}';
		...
		function GetMealSort(store_id){
			...
			if (typeof(meal_sortArr[store_id]) != 'undefined') {
				...
				sortHTML += '<option value="">无分类</option></select>';
			}
			$('#meal_sort_p').html(sortHTML);
		}
		GetMealSort({pigcms{$store_id});
