...
function appendItemHtml(has_more, item_list,type) {
	...
	if(type=='group'){
	   ...
	   detailurl="/index.php?g=WapMerchant&c=Index&a=sdetail";
	}
	for (var i = 0; i < item_list.length; i++) {
		...
		var content = "<a class='order-list-container' href='"+detailurl+"&order_id="+item.order_id+"'>"+(type=='meal' || type=='shop'? "<p class='shop-name'>"+item.storename+"<span style='float:right'>订单编号:"+item.order_id+"</span></p>":'<p class="shop-name">订单编号:'+item.order_id+'</p>')+
			"<div class='order-list-detail'>"+
				...
				"<p class='price'>合计：<span class='strong'>"+item.final_price+"</span>元" + (item.num > 0 ? "<span class='count'>("+item.num+")</span></p>" : "</p>") +
