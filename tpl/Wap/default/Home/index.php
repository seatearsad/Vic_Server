	...
	<body id="index">
        ...
        <div id="container">
			...
			<if condition="$activity_list">
				<dl class="list qianggou activity_list">
					<dd>
						<dl>
							<dd class="dd-padding" style="padding:.28rem .2rem .28rem .08rem;">
								<div>
									<strong><a href="{pigcms{:U('Wapactivity/index')}">限时活动</a></strong><space></space>
									<strong class="activity-right" style="float:right;color:#ee4339;">
										距结束：<span id="time_d">{pigcms{$time_array['d']}</span>天&nbsp;<span id="time_h">{pigcms{$time_array['h']}</span>:<span id="time_m">{pigcms{$time_array['m']}</span>:<span id="time_s">{pigcms{$time_array['s']}</span>
									</strong>
								</div>
							</dd>
							<dd>
								<volist name="activity_list" id="vo">
									<div class="qianggoucard">
										<a href="{pigcms{:U('Wapactivity/detail',array('id'=>$vo['pigcms_id']))}">
											<div class="img-container"><img src="{pigcms{$vo.list_pic}" alt="{pigcms{$vo.name}"/></div>
											<div class="brand">{pigcms{$vo.name}</div>
											<div class="campaign-price" style="color:#FF658E">已参与 {pigcms{$vo.part_count}</div>
										</a>
									</div>
								</volist>
							</dd>
					...
					<dd>
						<dl>
							<dd class="dd-padding" style="padding:.24rem .2rem .18rem .08rem;">
								<div>
									<a style="color:red;font-weight:bold;" href="{pigcms{:U('Classify/index')}">{pigcms{$config.classify_name}</a>
