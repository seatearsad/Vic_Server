...
<div class="car"> 
	...
	<div class="mod">
		...
		<div class="car_info_b">
			<if condition="$order['paid'] eq 0">
			<a href="{pigcms{:U('Meal/orderdel', array('mer_id' => $mer_id, 'store_id' => $store_id, 'orderid' => $order['order_id']))}">取消订单</a> 
			<a href="{pigcms{:U('Pay/check',array('order_id' => $order['order_id'], 'type'=>'meal'))}" class="zhifu">支付订单</a>  
			<elseif condition="$order['status'] eq 0"/>
			<a href="{pigcms{:U('My/meal_order_refund', array('mer_id' => $mer_id, 'store_id' => $store_id, 'orderid' => $order['order_id']))}">取消订单</a> 
			</if>
	    </div>
	</div>
	</volist>
