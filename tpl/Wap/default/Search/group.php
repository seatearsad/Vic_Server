...
<body>
        <div id="container">
	        <div class="search-wrapper">
				<div class="wrapper">
					<form id="search-form" action="{pigcms{:U('Search/group')}" method="post" group_action="{pigcms{:U('Search/group')}" meal_action="{pigcms{:U('Search/meal')}">
						<div class="search-dom">
							<select id="search-type">
								<option value="group" selected="selected">{pigcms{$config.group_alias_name}</option>
				...
				<if condition="$group_list">
					<div class="nav-bar" style="display:block;-webkit-transform-origin:0px 0px;opacity:1;-webkit-transform:scale(1, 1);">
						<ul class="nav">
							...
							<li class="dropdown-toggle caret sort <if condition="$now_sort eq 'price'">hover</if>" data-sort="price"><span class="nav-head-name">价格最低</span></li>
						</ul>
					</div>
				</if>
										...
										<a href="{pigcms{$vo.url}" class="react">
											<div class="dealcard" data-did="3859717">
												...
												<div class="dealcard-block-right">
													...
													<div class="title text-block">[{pigcms{$vo.prefix_title}]{pigcms{$vo.s_name}</div>
													<div class="price">
														...
														<span class="line-right">{pigcms{$vo.sale_txt}</span>
													</div>
												</div>
											</div>
										</a>
		...
		<script>
			$(function(){
				...
				$('.dropdown-toggle.caret').click(function(){
					window.location.href = "{pigcms{:U('Search/group',array('w'=>urlencode($keywords)))}&sort="+$(this).attr('data-sort');
				});
			});
		</script>
