...
window.shareData = {
		...
		"moduleID":"0",
		"imgUrl": "{pigcms{$invitation['store_image']}",
		"sendFriendLink": "{pigcms{$config.site_url}{pigcms{:U('Invitation/info', array('pigcms_id' => $invitation['pigcms_id']))}",
		"tTitle": "<if condition="$invitation['activity_type'] eq 0">约人吃饭<else/>约人看电影</if>",
		"tContent": "<if condition="$invitation['note']">{pigcms{$invitation['note']}<else />无约会说明</if>"
	};
</script>
{pigcms{$shareScript}
    </body>
