...
$(document).ready(function(){
	...
	if (getCookie('activity_type') != null) $('#activity_type').val(getCookie('activity_type'));
...
// 		$('#note_html').val(getCookie('note'));
	...
	$('#showcard').click(function(){
		...
		$.post("{pigcms{:U('Invitation/save_date')}", {'activity_type':$('#activity_type').val(), 'minute':$('#minute').val(), 'hour':$('#hour').val(), 'date':$('#date').val(), 'store_id':$('#store_id').val(), 'note':$('#note').val(), 'obj_sex':$('#obj_sex').val(), 'pay_type':$('#pay_type').val()}, function(data){
			if (data.error_code) {
				alert(data.msg);
			} else {
				delCookie('store_id');
