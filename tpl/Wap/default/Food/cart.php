...
<section data-role="body">
	<div class="main" >
		<div class="top">
			<span>
				<if condition="isset($level_off) AND !empty($level_off)">
				<div style="color:#FC6E05">{pigcms{$level_off['lname']}</div>
				<else />
				<div>我的订单</div>	
				</if>					
				<a href="{pigcms{:U('Food/menu', array('mer_id' => $mer_id, 'store_id' => $store_id, 'orid' => $orid))}" class="add">我还要</a>
						...
						<li id="dish_li{pigcms{$dditem['meal_id']}">
							<div class="licontent">
								...
								<div class="price_wrap">
									<strong>$<span class="unit_price">{pigcms{$dditem['price']}<input type="hidden" class="tureunit_price" <if condition="isset($dditem['vip_price']) AND $dditem['vip_price'] gt 0 AND false">value="{pigcms{$dditem['vip_price']}"<else/>value="{pigcms{$dditem['price']}"</if>></span></strong>
...
<footer data-role="footer">			
	<nav class="g_nav">
		<div>
			...
			<if condition="isset($level_off) AND !empty($level_off)">
			<span><span class="money"><del style="display: none">$<label id="allmoney">0</label></del>&nbsp;$<label id="levelallmoney">0</label></span>/<label id="menucount">0</label>份</span>
			<else />
