...
<body>
		...
		<dl class="list">
			<dd>
				<dl>
					...
					<if condition="!$now_activity['money']">
						<dd class="dd-padding kv-line-r">
							<h6>使用：</h6>
							<select name="score_type" id="score_type">
								<option value="1">商家{pigcms{$config['score_name']}</option>
								<option value="2">平台{pigcms{$config['score_name']}</option>
							</select>
						</dd>
					</if>
					<dd class="dd-padding kv-line-r">
						...
						<span class="kv-v" id="amount">
							<span class="J_total-price"><if condition="$now_activity['money']">{pigcms{$now_activity.money}元<else/>{pigcms{$now_activity.mer_score}分</if></span>
	...
	<script>
		var buyType = <if condition="$now_activity['money']">1<else/>2</if>,buyValue = <if condition="$now_activity['money']">{pigcms{$now_activity.money}<else/>{pigcms{$now_activity.mer_score}</if>,buySysScore = {pigcms{$now_activity['mer_score']/$config['activity_score_scale']};
	</script>
	<script>
		$(function(){
			...
			$('button.plus').click(function(){
				...
				if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(pigcms_now_quantity)){
					...
					if(buyType == 1){
						$('.J_total-price').html(buyValue*(pigcms_now_quantity+1)+'元');
					}else{
						if($('#score_type').val() == '1'){
							$('.J_total-price').html(buyValue*(pigcms_now_quantity+1)+'分');
						}else{
							$('.J_total-price').html(buySysScore*(pigcms_now_quantity+1)+'分');
						}
					}
			...
			quantity.blur(function(){
				...
				if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(pigcms_now_quantity)){
					...
					if(buyType == 1){
						$('.J_total-price').html(buyValue*(parseInt(quantity.val()))+'元');
					}else{
						if($('#score_type').val() == '1'){
							$('.J_total-price').html(buyValue*(parseInt(quantity.val()))+'分');
						}else{
							$('.J_total-price').html(buySysScore*(parseInt(quantity.val()))+'分');
						}
					}
				}
			});
			...
			$('#buy-form').submit(function(){
				...
				if(submit_now == false){
					...
					$.post("{pigcms{$config.site_url}/index.php?c=Activity&a=submit&id={pigcms{$now_activity.pigcms_id}",{q:quantity.val(),score_type:$('#score_type').val()},function(result){
						submit_now = false;
						$('.mj-submit').html('提交').prop('disabled',false);
						if(result.status == -3){
							alert(result.info);
							window.location.href = "{pigcms{:U('Login/index',array('referer'=>urlencode(U('Wapactivity/buy_coupon',array('id'=>$now_activity['pigcms_id'])))))}";
							...
							layer.open({
								...
								shadeClose: false,
								yes: function(){
									window.location.href = "{pigcms{:U('My/adress',array('referer'=>urlencode(U('Wapactivity/buy_coupon',array('id'=>$now_activity['pigcms_id'])))))}";
								}
							});
						}else{
							layer.open({
