...
<header>
	<nav id="nav_1" class="p_10">
		<ul class="box">
			<li><a href="wap.php?g=Wap&c=Card&a=coupon&token={pigcms{$token}&cardid={pigcms{$thisCard.id}&type={pigcms{$type}&is_use=0" class="<if condition="$is_use eq '0'">on</if>">未使用</a></li>
			<li><a href="wap.php?g=Wap&c=Card&a=coupon&token={pigcms{$token}&cardid={pigcms{$thisCard.id}&type={pigcms{$type}&is_use=1" class="<if condition="$is_use eq '1'">on</if>">已使用</a></li>
		</ul>
	</nav>
</header>
...
<div id="queren{pigcms{$item.id}" style="display:none;">
	<form action="/wap.php?g=Wap&c=Card&a=action_useCoupon&token={pigcms{$token}" method="post" id="payform{pigcms{$item.id}">
	<p style=" margin:10px 0;text-align:center">
	<input type="radio" name="paytype" onchange="radiochange(this,{pigcms{$item.id})" class="paytype{pigcms{$item.id}" value="0" checked/><label for="paytype{pigcms{$item.id}" style="font-size:12px;">线下支付</label> &nbsp; &nbsp; &nbsp; &nbsp;
	<input type="radio" name="paytype" onchange="radiochange(this,{pigcms{$item.id})" class="paytype{pigcms{$item.id}" value="1" /><label for="paytype2{pigcms{$item.id}" style="font-size:12px;">会员卡支付</label>
	</p>
	...
	<p style=" margin:10px 0 0 0" id="usetime_p">
		<input name="usetime" class="px" id="usetime{pigcms{$item.id}" value="" type="text" placeholder="请输入使用张数">
	</p>-->
...
<if condition="$item.is_use eq '0' AND false">
...
<li><if condition="$type eq 1">打折优惠券<else/>现金抵用券(每张抵用{pigcms{$item.price}元)</if></li>
...
function use(i){
	...
	jQ.post('/wap.php?g=Wap&c=Card&a=action_useCoupon&token={pigcms{$token}', submitData,
