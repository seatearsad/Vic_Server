...
<div id="mappContainer">
  <div class="inner root" style="height: 468px;">
    <div id="card" class="card" style="background:url('{pigcms{$card.diybg}') no-repeat 0 0;-webkit-background-size:267px 159px;background-size:267px 159px;">
		  ...
		  <strong class="pdo verify" style="">
				<span style="color:#847d64;text-shadow:0 1px #ebe9e0;">
					<em style="color:#847d64;text-shadow:0 1px #ebe9e0;">会员卡号</em>
					<span style="color:{pigcms{$card.numbercolor}">{pigcms{$mycard.number}</span>
				</span>
		  </strong>
	  </div>
	  <!--会员卡背面-->
    ...
    <p><span data-hidden-when-lost="使用时向服务员出示此卡">{pigcms{$card.msg}</span></p>
     ...
     <li class="intro">
		...
		<if condition="$info['logo'] neq ''">
			<span style="float:left;padding-righ:10px"><img src="{pigcms{$info.logo}"/></span>
		</if>
			{pigcms{$info.info}
	...
	<div style="height:40px;">&nbsp;</div>
  	...
  	<div class="footFix" id="footReturn"><a href="javascript:void(0)" onClick="location.href='{pigcms{:U('Card/vip',array('token'=> $token))}';"><span>返回会员卡页</span></a></div>
</div>
