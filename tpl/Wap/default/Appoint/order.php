...
.detail div.detail-right{ float: right; border-left: 1px dashed #80e1d5; margin-top: .5rem; width: 75px;}
	...
	<section id="main">
		<div class="yxc-body-bg index-section">
			<form action="" method="post" id="main_form">
						...
						<div class="comm-service">
							...
							<div class="con-service">
								<div class="con-service-inner" data-role="packageDescription">
									...
									<p style=" color:#32c8a2;">全价：{pigcms{$now_group['appoint_price']}</p>
								</div>
							</div>
						</div>
				</if>
				...
				<div class="yxc-attr-list detail">
					<div>
						<img src="/upload/appoint/{pigcms{$default_workers_info['avatar_path']}" class="detail-tou" />
					</div>
					<div class="detail-intro">
						<p>{pigcms{$default_workers_info['name']}</p>
				
						<p>共服务{pigcms{$default_workers_info['order_num']}次</p>
					</div>
				...
				<ul class="yxc-attr-list appoint-time" >
					<li data-role="chooseTime">
						...
						<p class="cover no-arrow">
							<input type="hidden" name="service_date" id="service_date" value="{pigcms{$_GET['now_date']}"/>
							<input type="hidden" name="service_time" id="service_time" value="{pigcms{$_GET['now_time']}"/>
							<input class="ipt-attr" type="text" id="serviceJobTime" placeholder="选择预约时间" readonly="readonly" <if condition="$_GET['now_date'] && $_GET['now_time']">value="{pigcms{$_GET['now_date']} {pigcms{$_GET['now_time']}"</if> />
						</p>
					</li>
				</ul>
				<div class="yxc-space"></div>
...
function showError(error){
	...
	<script type="text/javascript" src="{pigcms{$static_path}js/appoint_form.js?111"></script>
</body>
</html>