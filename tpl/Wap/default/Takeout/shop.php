...
<div class="container">
	<header class="nav">
		<div>
			<a href="{pigcms{:U('Takeout/menu', array('mer_id' => $mer_id, 'store_id' => $store_id))}">商品列表</a>
	...
	<section>
		...
		<div class="store_info">
			...
			<span><strong>{pigcms{:floatval($store['basic_price'])}</strong>起送价/元</span>
			<span><strong>{pigcms{:floatval($store['delivery_fee'])}</strong>配送费/元</span>
		</div>
		<ul class="box">
			<li>
			...
			<li>
				<a href="{pigcms{:U('Group/addressinfo',array('store_id'=>$store['store_id']))}">
		...
		<ul class="box">
			...
			<if condition="$store['delivery_area']">
	...
	<footer class="go_menu" style="height:70px;">
		<div class="fixed">
			<a href="{pigcms{:U('Takeout/menu', array('mer_id' => $mer_id, 'store_id' => $store_id))}">查看商品列表</a>
		</div>
	</footer>
</div>
<include file="kefu" />
