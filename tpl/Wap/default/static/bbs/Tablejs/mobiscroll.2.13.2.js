    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        function generateWheelItems(i) {
            ...
            $.each(values, function (j, v) {
                ...
                html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight + 'px;line-height:' + itemHeight + 'px;background-image:url('+cat_img_arr[j]+');background-size: 32px 28px; background-repeat: no-repeat; background-position: left center;padding-left: 32px;">' +
    ...
    $.mobiscroll.presets.scroller.select = function (inst) {
        ...
        function genValues(cont, keys, values) {
            ...
            $('option', cont).each(function () {
                ...
                cat_img_arr.push($(this).data("bgimg"));
                if (this.disabled) {
