...
$(document).ready(function(){
    ...
    var stars_send = $("#star_send > i");
    var stars_whole = $("#star_whole > i");
    var descriptionTemp;
    //$("#showb").css("width",0);

    stars_send.each(function(i){
        $(stars_send[i]).click(function(e){
            // var n = i+1;
            ...
            changeStar($(this).parent(),$(this).index());
        });
    });

    stars_whole.each(function(i){
        $(stars_whole[i]).click(function(e){
            changeStar($(this).parent(),$(this).index());
        });
    });
...
function changeStar(parent,index) {
	var star = index + 1;
	parent.children('i').each(function () {
		if(index < $(this).index()){
			$(this).addClass("level_solid_none");
		}else{
            $(this).removeClass("level_solid_none");
		}
    });
	if(parent.attr('id') == "star_send"){
		$("#score_send").val(star);
	}else {
        $("#score_whole").val(star);
	}
}
