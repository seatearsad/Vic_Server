...
var mapObjDom = null;
$(function(){
	if($('#store_id').val() && (merchant_workers_id == '' || merchant_workers_id == 0)){
		select_store()
	}
	$('#service_time').val();
	...
	$('#service-type .service-list li').click(function(){
		...
		$('#appoint_price').text($(this).data('payment-price'));
...
function select_store(){
	...
	$.post(ajaxWorkUrl,{'merchant_store_id':store_id,'appoint_id':appoint_id},function(data){
		...
		if(data.status){
			...
			//if(flag){
				str +='<div class="yxc-attr-list detail"><div><img src="' + data.worker_list[0]["avatar_path"] + '" class="detail-tou" /></div><div class="detail-intro"><p>'+data.worker_list[0]["name"]+'</p><p>'+data.worker_list[0]["desc"]+'</p><p>共服务'+data.worker_list[0]["order_num"]+'次</p></div><div class="detail-right"><a href="'+workerUrl+'"><img src="' + static_path +'images/detail_gh.png" /></a></div></div><div class="yxc-space"></div>';
			
				$('#work_select').after(str);
				getWorkerTime(data.worker_list[0]["merchant_worker_id"]);
			//}
			
		}else{
			$('.detail').hide();
			$('.detail').next('.yxc-space').hide();
			$('input[name="merchant_workers_id"]').val('');
			getAppointTime();
		}
	},'json')
}
...
if($('#merchant_workers_id').val()){
	getWorkerTime($('#merchant_workers_id'));
}
