	...
	<form id="buy-form" action="{pigcms{:U('Group/buy',array('group_id'=>$now_group['group_id']))}" method="POST" class="wrapper-list" autocomplete="off">
			...
			<dd>
				<dl>
					<dd class="dd-padding kv-line-r">
						<h6>单价<if condition="$_GET['type'] eq 3">(团长优惠)</if>：</h6>
						<p><del>{pigcms{$now_group['price']}元<if condition="$now_group['extra_pay_price'] gt 0 AND $config.open_extra_price eq 1 ">+{pigcms{$now_group.extra_pay_price|floatval}{pigcms{$config.extra_price_alias_name}</if></del></p>
					</dd>
					<if condition="!empty($leveloff) AND $finalprice gt 0">
						<dd class="dd-padding kv-line-r">
							...
							<span style="position: absolute;right: 8px;top: 15px;">优惠单价 <strong style="font-size:16px;color:#FF4907">{pigcms{$leveloff['price']}</strong>元<if condition="$now_group['extra_pay_price'] gt 0 AND $config.open_extra_price eq 1 ">+{pigcms{$now_group.extra_pay_price|floatval}{pigcms{$config.extra_price_alias_name}</if>
							</span>
						</dd>
					</if>
					...
					<dd class="dd-padding kv-line-r">
						...
						<span class="kv-v" id="amount">
							<!--
							<if condition="!empty($leveloff) AND $finalprice gt 0">
								<span class="J_total-price">{pigcms{$leveloff['price']*$now_group['once_min']}元<if condition="$now_group['extra_pay_price'] gt 0">+{pigcms{$now_group.extra_pay_price|floatval}{pigcms{$config.extra_price_alias_name}</if></span>
							<else />
								<span class="J_total-price">{pigcms{$now_group['price']*$now_group['once_min']}元<if condition="$now_group['extra_pay_price'] gt 0 AND $config.open_extra_price eq 1 ">+{pigcms{$now_group.extra_pay_price|floatval}{pigcms{$config.extra_price_alias_name}</if></span>
							</if>-->
							<span class="J_total-price">{pigcms{$real_price}</span>
		...
		<else/>
			...
			<if condition="$now_group['user_adress']['adress_id']">
				<dl class="list">
					<input type="hidden" name="pick_in_store" value="<php> if(empty($_GET['pick_addr_id'])){echo 0;}else{echo 1;}</php>"/>
					<dd class="dd-padding kv-line-r quantity">
						<h6>配送方式</h6>
						<a class="btn_express <php>if(empty($_GET['pick_addr_id'])){</php>post_package<php>}else{</php>post_package_click<php>}</php>" id="post_package">快递配送</a>&nbsp;&nbsp;<php>if($now_group['pick_in_store']){</php><a class="btn_express <php>if(!empty($_GET['pick_addr_id'])){</php>pick_in_store<php>}else{</php>pick_in_store_click<php>}</php>" id="pick_in_store">到店自提</a><php>}</php>
					</dd>
					<dd <php>if(empty($_GET['pick_addr_id'])||!empty($_GET['address_id'])){</php>style="display:block"<php>}else{</php>style="display:none"<php>}</php>>
					...
					<dd class="pick_address" <php>if(!empty($_GET['pick_addr_id'])){</php>style="display:block"<php>}else{</php>style="display:none"<php>}</php>>
						<a id="change_address" class="react" href="{pigcms{:U('My/pick_address',array('mer_id'=>$now_group['mer_id'],'group_id'=>$now_group['group_id'],'pick_addr_id'=>$pick_address['pick_addr_id']))}">
							<div class="more more-weak">
			...
			<dl class="list" id="mobile-show">
				<dd>
					<textarea name="remark" style="width:100%;height:60px;resize: none;"></textarea>
				</dd>
			</dl>
			...
			$('button.plus').click(function(){
				...
				if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(pigcms_now_quantity)){
					...
					if(extra_price>0&&open_extra_price==1){						
						...
						var s = price*(pigcms_now_quantity+1)/100 * 1.05 + ""
						$('.J_total-price').html(s.substring(0, s.indexOf(".") + 3));
					}
