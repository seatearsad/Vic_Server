...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><script async="" src="./首页_files/alog.mobile.min.js"></script><script type="text/javascript" src="./首页_files/api-list.js"></script><script type="text/javascript" src="./首页_files/api-list(1).js"></script>
...
<link type="text/css" rel="stylesheet" href="{pigcms{$static_path}tpl/1353/css/base.css">
    ...
    <section class="ap oh carouselBox" id="carouselBox" ontouchstart="touchStart(event)" ontouchmove="touchMove(event);" ontouchend="touchEnd(event);" style="overflow: hidden;">
        <ul class="oh ab carouselPics" id="carouselPics" style="width: 500%; position: relative; left: -640px;">
          <volist name="flash" id="so">   
             <li style="width: 220px;"><a href="{pigcms{$so.url}"><img src="{pigcms{$so.img}"></a></li>
          </volist>
        </ul>
...
<else />
    ...
    <div class="oh menu202" style="margin-top:8px">
        <ul class="clear">
          <volist name="info" id="vo">
            <if condition="$i gt 3">
              <li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>"><img src="{pigcms{$vo.img}"><span><h3>{pigcms{$vo.name}</h3></span></a></li>
            </if>
          </volist>
        </ul>
    </div>
