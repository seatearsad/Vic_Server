...
<link type="text/css" rel="stylesheet" href="{pigcms{$static_path}tpl/1343/css/base.css">
...
<body class="bg15 hAndroid hAndroid2x">
<!--music-->
    <if condition="$homeInfo['musicurl'] neq false">
      <include file="Index:music"/>
    </if>
<section id="container" style="min-height: 480px;">
<if condition="$homeInfo['switch'] eq 0">
  <section class="ap oh carouselBox" id="carouselBox" ontouchstart="touchStart(event)" ontouchmove="touchMove(event);" ontouchend="touchEnd(event);" style="overflow: hidden;">
        <ul class="oh ab carouselPics" id="carouselPics" style="width: 500%; position: relative; left: -390px;">
...
<else />
    ...
    <ul class="oh menu15">
      <volist name="info" id="vo">
          <li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>"><div><img src="{pigcms{$vo.img}"></div><span>{pigcms{$vo.name}</span></a></li>
