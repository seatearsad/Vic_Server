...
<link type="text/css" rel="stylesheet" href="{pigcms{$static_path}tpl/1335/css/base.css">
    ...
    <section class="ap oh carouselBox" id="carouselBox" ontouchstart="touchStart(event)" ontouchmove="touchMove(event);" ontouchend="touchEnd(event);" style="overflow: hidden;">
        <ul class="oh ab carouselPics" id="carouselPics" style="width: 500%; position: relative; left: -1280px;">
          <volist name="flash" id="so">   
            <li style="width: 640px;"><a href="{pigcms{$so.url}"><img src="{pigcms{$so.img}"></a></li>
          </volist>
        </ul>
...
<else />
   ...
   <ul class="oh menu351" style="margin-top:5px;">
      <volist name="info" id="vo"> 
          <li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>"><img src="{pigcms{$vo.img}"><span>{pigcms{$vo.name}</span><div class="arrow"></div></a></li>
      </volist>    
    </ul>
...
<script type="text/javascript" src="{pigcms{$static_path}tpl/1335/js/base.js"></script>
