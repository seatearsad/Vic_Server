...
<link type="text/css" rel="stylesheet" href="{pigcms{$static_path}tpl/1348/css/base.css">
...
<else />
    ...
    <ul class="oh menu381" id="menu381">
      <volist name="info" id="vo">  
          <li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>"><img src="{pigcms{$vo.img}"><div class="wz"><span><b>{pigcms{$vo.name}</b></span><p>{pigcms{$vo.info}</p></div></a></li>
      </volist>  
    </ul>
