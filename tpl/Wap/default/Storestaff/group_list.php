					...
					<dd class="dealcard dd-padding">
						<a href="{pigcms{:U('Storestaff/group_edit',array('order_id'=>$vo['order_id']))}">
							<ul class="dealcard-block-right">
								...
								<li class="detail"><span class="dth">订单信息：</span><span class="xth">数量: </span><span class="td">{pigcms{$vo.num}</span>&nbsp;&nbsp;<span class="xth">总价: </span><span class="td">{pigcms{$vo.total_money} 元<if condition="$config.open_extra_price eq 1 AND $vo.extra_price gt 0">+{pigcms{$vo.extra_price}{pigcms{$config.extra_price_alias_name}</if></span></li>
				...
				{{# if(d.list[i].paid == '1'){ }}
					...
					{{# }else if(d.list[i].status == '0'){ }}
						...
						{{# if(d.list[i].tuan_type != '2'){ }}
							...
							{{# }else{ }}
								<span ><a style="color:#FF658E" href="{pigcms{:U('Storestaff/group_pass_array')}&order_id={{ d.list[i].order_id }}" class="group_verify_btn">查看消费码详情</a></span>
							{{# } }}
						{{# }else{ }}
