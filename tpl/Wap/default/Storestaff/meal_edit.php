				...
				<tr>
					<td> 
						<if condition="$order['third_id'] eq '0' AND $order['pay_type'] eq 'offline'">
							总金额： ${pigcms{$order['total_price']}<br>
					 		平台余额支付 ：{pigcms{$order.balance_pay} <br>
					 		商家会员卡余额支付：{pigcms{$order.merchant_balance}<br>
							线下需向商家付金额：<font color="red">${pigcms{$order['total_price']-$order['merchant_balance']-$order['balance_pay']}元</font>
						<else/>
							平台余额支付：{pigcms{$order.balance_pay} <br>
					 		商家会员卡余额支付：{pigcms{$order.merchant_balance}<br>
					 		在线支付金额：{pigcms{$order.payment_money}<br> 
						</if>
					 </td>
				</tr>
