...
<body>
	<dl class="list" style="border-top:none;margin-top:0rem;">
				...
				<div class="find_type_div">
					<select name="find_type" id="find_type" class="col-sm-1">
						<optgroup label="{pigcms{$config.group_alias_name}">
							<option value="1">purchase password</option>
						</optgroup>
						<optgroup label="item">
	    ...
	    <div style="margin-top:.2rem;">
					...
					<dd class="dealcard dd-padding">
						<a href="{pigcms{:U('Storestaff/group_edit',array('order_id'=>$vo['order_id']))}">
							<ul class="dealcard-block-right">
								<li class="name"><span class="dth">{pigcms{$config.group_alias_name}name:</span>
								<span class="ttd">{pigcms{$vo.s_name}</span></li>
								<li class="detail"><span class="dth">order information:</span><span class="xth">number: </span><span class="td">{pigcms{$vo.num}</span>&nbsp;&nbsp;<span class="xth">Total price: </span><span class="td">{pigcms{$vo.total_money} CAD<if condition="$config.open_extra_price eq 1 AND $vo.extra_price gt 0">+{pigcms{$vo.extra_price}{pigcms{$config.extra_price_alias_name}</if></span></li>
								<li><span class="dth">Order Status:</span><span>
									...
									<elseif condition="$vo['paid']" />
										<if condition="$vo['third_id'] eq '0' AND $vo['pay_type'] eq 'offline' AND $vo['status'] eq '0'">
											<font color="red">Unpaid offline</font>
										<elseif condition="$vo['status'] eq 0" />
											<font color="green">Paid</font>&nbsp;
											<php>if($vo['tuan_type'] != 2){</php>
												<font color="red">Not consumed</font>
												<php>if(!$vo['pass_array']){</php>
													<span onclick="group_verify_btn({pigcms{$vo['order_id']},$(this));return false;" style="color:#FF658E">Verify consumption</span>
												<php>}</php>
											<php>}else{</php>
										...
										<elseif condition="$vo['status'] eq 1"/>
											<php>if($vo['tuan_type'] != 2){</php>
												<font color="green">Has been consumed</font>&nbsp;
											<php>}else{</php>
												<php>if($vo['is_pick_in_store']){</php>
													<font color="green">Has been picked up</font>
												<php>}else{</php>
			...
			<!--<div class="top-btn" style="display: block;left:10px;opacity: 0.8;"><a class="react" href="{pigcms{:U('Storestaff/group_list')}"><i class="text-icon">Refresh</i></a></div>
		...
		<dd class="dealcard dd-padding">
		 <a href="{pigcms{:U('Storestaff/group_edit')}&order_id={{ d.list[i].order_id }}">
		  <ul class="dealcard-block-right">
		   ...
		   <li  class="detail"><span class="dth">order information:</span><span class="xth">Quantity: </span><span class="td">{{ d.list[i].num }}</span>&nbsp;&nbsp;<span class="xth">Total price: </span><span class="td">{{ d.list[i].total_money }} CAD</span></li>
			<li><span class="dth">Order Status:</span><span>
				{{# if(d.list[i].paid == '1'){ }}
					{{# if(d.list[i].pay_type == 'offline' && d.list[i].third_id ==0){ }}
						<font color="red">Unpaid offline</font>&nbsp;

						{{# if(d.list[i].pass_array=='0'){ }}
						<span onclick="group_verify_btn({{ d.list[i].order_id }},$(this));return false;" style="color:#FF658E">Verify payment</span>
						{{# } }}
					{{# }else if(d.list[i].status == '0'){ }}
						...
						{{# if(d.list[i].tuan_type != '2'){ }}
							<font color="red">Not consumed</font>&nbsp;
							{{# if(d.list[i].num==1){ }}
								<span onclick="group_verify_btn({{ d.list[i].order_id }},$(this));return false;" style="color:#FF658E">Verify consumption</span>
							{{# }else{ }}
								<span ><a style="color:#FF658E" href="{pigcms{:U('Storestaff/group_pass_array')}&order_id={{ d.list[i].order_id }}" class="group_verify_btn">View the details of the consumer code</a></span>
							{{# } }}
						{{# }else{ }}
							
							{{# if(d.list[i]['is_pick_in_store']){ }}
								<font color="red">Not picked up</font>
							{{# }else{ }}
								<font color="red">Not shipped</font>
							{{# } }}
						{{# } }}
...
<script type="text/javascript">
	...
	$('#find_submit').click(function(){
		...
		if(find_value.val().length < 1){
			layer.open({title:['Unpaid','background-color:#FF658E;color:#fff;'],content:'请输入查找内容',btn: ['determine'],end:function(){find_value.focus();}});
			return false;
		}
		...
		$.post("{pigcms{:U('Storestaff/group_find')}",{find_type:post_type,find_value:post_value},function(result){
			...
			if(data.row_count > 0){
				...
				layer.open({title:['Error message:','background-color:#FF658E;color:#fff;'],content:'Not found content!',btn: ['determine'],end:function(){find_value.focus();}});
			}
		});
	...
	function group_verify_btn(oid,obj){
		...
		$.get("{pigcms{:U('Storestaff/group_verify')}&order_id="+oid,function(result){
			if(result.status == 1){
				layer.open({
					title:['Success Tip:','background-color:#FF658E;color:#fff;'],
					content:result.info,
					btn: ['determine'],
...
$('#qrcode_btn').click(function(){
	if(is_weixin()){
		wx.scanQRCode({
			...
			success:function (res){
				...
				if(result.indexOf('http://') !== 0 && result.indexOf('https://') !== 0){
					layer.open({title:['Error message:','background-color:#FF658E;color:#fff;'],content:'The content you scanned “ <font color="red">'+result+'</font> ” is not valid validation of two-dimensional code',btn: ['determine'],end:function(){}});
				}else{
					...
					if((ctype != 'group_qrcode' && ctype != 'meal_qrcode') || id== '' || c != 'Storestaff'){
						layer.open({title:['Error message:','background-color:#FF658E;color:#fff;'],content:'The content you scannedis not valid validation of two-dimensional code',btn: ['determine'],end:function(){}});
					}else{
						layer.open({
							title:['prompt:','background-color:#FF658E;color:#fff;'],
							content:'The initial inspection order belongs <font color="red">'+(ctype == 'group_qrcode' ? '{pigcms{$config.group_alias_name}' : '{pigcms{$config.meal_alias_name}')+'</font> order, you want toVerify consumptionor view orders?',
							btn: ['check order'],
							shadeClose: false,
							// yes: function(){
								// layer.open({
									// type: 2,
									// content: 'Verify consumption，Please wait'
								// });
		...
		layer.open({title:['Error message:','background-color:#FF658E;color:#fff;'],content:'You are not using WeChat browser, this feature can not be used! You can use the browser's own or other scanning two-dimensional code tool to scan.',btn: ['determine'],end:function(){}});
	}
	return false;
});
</script>
