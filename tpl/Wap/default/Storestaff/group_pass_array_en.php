...
<body id="index" data-com="pagecommon">
        ...
        <div class="wrapper-list" style="padding-bottom: 10px;">
			<h4 style="margin-top:.3rem;">{pigcms{$now_order.s_name} </h4><a class="btn" style="float:right;margin-right: 12px;margin-right: 15px;top:-.7rem;;position: relative;" href="{pigcms{:U('Storestaff/group_edit',array('order_id'=>$now_order['order_id']))}">Return</a>
			...
			<dl class="list coupons">
				<dd style="overflow:visible;">
					<dl>
						...
						<dd class="dd-padding coupons-code">
							{pigcms{$config.group_alias_name}commodity: <span><a href="{pigcms{:U('Group/detail',array('group_id'=>$now_order['group_id']))}" target="_blank">{pigcms{$now_order.s_name}</a></span>
						</dd>
						<dd class="dd-padding coupons-code">
							Order Type: <span><if condition="$now_order['tuan_type'] eq '0'">{pigcms{$config.group_alias_name}Voucher<elseif condition="$now_order['tuan_type'] eq '1'"/>Vouchers<else/>item</if></span>
						</dd>
						<dd class="dd-padding coupons-code">
							Consumption code <span style="float:right;margin-right:15px">operating <if condition="$now_order['status'] eq 0"><a style="color: blue;" href="javascript:void(0);" onclick="group_verify_btn({pigcms{$now_order.order_id},$(this));return false;" title="View the product Details">Validate All</a><elseif condition="($now_order.status eq 1 OR $now_order.status eq 2)"/><font color="green">Paid</font></if></span>
						</dd>
						<volist name="pass_array" id="vo">
							<dd class="dd-padding coupons-code">
							{pigcms{$vo.group_pass} <span style="float:right;margin-right:15px"><if condition="$vo.status eq 0" ><font color="red">Not Paid</font>&nbsp;<a href="javascript:void(0);" onclick="group_array_verify({pigcms{$now_order.order_id},'{pigcms{$vo.group_pass}',this);" title="View product details">Verify consumption</a><elseif condition="$vo.status eq 3" /><font color="red">Need to pay：{pigcms{$vo.need_pay} Dollars</font>&nbsp;<a href="javascript:void(0);" onclick="group_array_verify({pigcms{$now_order.order_id},'{pigcms{$vo.group_pass}',this);" title="View the product Details">Verify payment</a><elseif condition="$vo.status eq 2" /><font color="red">Refunded</font><elseif condition="$now_order.status eq 1 OR $now_order.status eq 2" /><font color="green">Paid</font><else /><font color="green">Paid</font></if></span>
							</dd>
						</volist>
					</dl>
				</dd>
