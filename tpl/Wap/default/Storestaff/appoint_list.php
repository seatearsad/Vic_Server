					...
					<dd class="dealcard dd-padding">
						<a href="{pigcms{:U('Storestaff/appoint_edit',array('order_id'=>$vo['order_id']))}">
							<ul class="dealcard-block-right">
								...
								<li class="name"><span class="dth">预约时间：</span><span class="ttd">{pigcms{$vo.appoint_date}&nbsp;{pigcms{$vo.appoint_time}</span></li>
								<li class="detail"><span class="dth">订单信息：</span><span class="xth">定金: </span><span class="td">{pigcms{:floatval($vo['payment_money'])}元<if condition="$config.open_extra_price eq 1 AND $vo.extra_price gt 0">+{pigcms{$vo.extra_price}{pigcms{$config.extra_price_alias_name}</if></span>&nbsp;&nbsp;<span class="xth">总价: </span><span class="td">{pigcms{:floatval($vo['appoint_price'])}元<if condition="$config.open_extra_price eq 1 AND $vo.extra_price gt 0">+{pigcms{$vo.extra_price}{pigcms{$config.extra_price_alias_name}</if></span></li>
								<li>
									...
									<span><if condition="$vo['paid'] == 0" >
										...
										<if condition="$vo['service_status'] == 0" >
											...
											<if condition='($vo["is_del"] eq 0) && ($vo["payment_status"] eq 0)'><span onclick="appoint_verify_btn({pigcms{$vo['order_id']},$(this));return false;" style="color:#428bca">验证服务</span></if>
									<if condition='($vo["is_del"] eq 0) && ($vo["paid"] eq 1)'><span onclick="appoint_verify_btn({pigcms{$vo['order_id']},$(this));return false;" style="color:#428bca">验证服务</span></if>
				...
				<dd class="dealcard dd-padding">
					<a href="{pigcms{:U('Storestaff/appoint_edit')}&order_id={{ d.list[i].order_id }}">
						<ul class="dealcard-block-right">
							...
							<li class="detail"><span class="dth">订单信息：</span><span class="xth">定金: </span><span class="td">{{ d.list[i].payment_money }}元</span>&nbsp;&nbsp;<span class="xth">总价: </span><span class="td">{{ d.list[i].appoint_price }}元</span></li>
							<li><span class="dth">订单状态：</span><span>
								{{# if(d.list[i].paid == '0'){ }}
									<font color="red">未支付</font>
									{{# if(d.list[i].service_status == '0'){ }}
										<font color="red">未服务</font>
										<span onclick="appoint_verify_btn({{ d.list[i].order_id }},$(this));return false;" style="color:#428bca">验证服务</span>
