...
<body>
    <div class="time_list">
        <ul>
            ...
            <input style="display: none" type="text" name="spec_id" value="{pigcms{$spec_id}" >
            <if condition="!$spec_id">
            <li>
                {pigcms{:L('_STORE_ADD_SPEC_')}
            </li>
            </if>
            <li>
                <input type="text" placeholder="{pigcms{:L('_STORE_PRO_NAME_')}({pigcms{:L('_STORE_REQUIRED_')})" name="name" value="{pigcms{$spec.name}">
            </li>
            <li>
            ...
            <li id="pro_val" style="margin-top: 10px">
                <if condition="$spec">
                    <volist name="spec['val']" id="vo">
                        <input type="text" placeholder="{pigcms{:L('_STORE_PRO_VALUE_')}" name="{pigcms{$vo['id']}" id="val_{pigcms{$i}" value="{pigcms{$vo['name']}"> <span data-id="{pigcms{$i}">X</span>
                    </volist>
                <else/>
...
<script>
    ...
    if(new_id != ''){
        var data = artDialog.open.origin.getNewSpecData(new_id);
        $('input[name=name]').val(data['name']);

        $('#pro_val').find('input').remove();

        val_num = data['val'].length;
        var input_html = '';
        for(var i=0;i<data['val'].length;i++){
            var id_num = i+1;
            input_html += '<input type="text" placeholder="{pigcms{:L(\'_STORE_PRO_VALUE_\')}" id="val_'+id_num+'" value="'+data['val'][i]+'"> <span data-id="'+id_num+'">X</span>';
        }
    ...
    function getVal(){
        ...
        $('#pro_val').find('input').each(function () {
            if(typeof($(this).attr('name')) == 'undefined'){
                var n_str = 0;
                if($(this).val() != ''){
                    var t_v = n_str + ':' + $(this).val();
                }
            }else{
                var n_str = $(this).attr('name');
                var t_v = n_str + ':' + $(this).val();
            }
    ...
    $('.add_c').click(function () {
        ...
        if(is_tip){
            ...
            if (spec_id != '') {//修改属性
                ...
                $.post("{pigcms{:U('Storestaff/goods_spec_edit')}", post_data, function (result) {
                    layer.open({
                        ...
                        time: 1,
                            ...
                            artDialog.open.origin.updateSpec(result.data);
                            art.dialog.close();
                        }
                    });
                });
                ...
                if(new_id != ''){
                    artDialog.open.origin.addNewSpecAndId(data,new_id);
                }else {
                    artDialog.open.origin.addNewSpec(data);
                }
                art.dialog.close();
            }
        }
