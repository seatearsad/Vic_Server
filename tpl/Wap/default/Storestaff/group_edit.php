...
<body id="index" data-com="pagecommon">
        ...
        <div class="wrapper-list" style="padding-bottom: 10px;">
			...
			<a class="btn" style="float:right;margin-right: 12px;margin-right: 15px;top:-.7rem;;position: relative;" href="{pigcms{:U('Storestaff/group_list')}">返 回</a>
			<if condition="$now_order['is_pick_in_store'] AND $now_order['status'] eq 0"><a class="btn" style="float:right;margin-right: 12px;margin-right: 15px;top:-.7rem;;position: relative;background-color:#0C8642;" href="javascript:void(0);" onclick="pick_click();">取货确认</a></if>
				...
				<dd style="overflow:visible;">
					<dl>
						...
						<dd class="dd-padding coupons-code">
							订单编号： <span>{pigcms{$now_order.real_orderid}</span>
						</dd>
						...
						<if condition="$now_order['pass_array']">
							<dd class="dd-padding coupons-code">
								操作：<span><a  href="{pigcms{:U('Storestaff/group_pass_array',array('order_id'=>$now_order['order_id']))}" class="group_verify_btn">查看消费码详情</a></span>
							</dd>
						<else />
							<dd class="dd-padding coupons-code">
								操作：<span><a  href="{pigcms{:U('Storestaff/group_verify',array('order_id'=>$now_order['order_id']))}" class="group_verify_btn">验证消费</a></span>
							</dd>
						</if>
						<dd class="dd-padding coupons-code">
						...
						<dd class="dd-padding coupons-code">
							单价： <span>{pigcms{$now_order.price}元<if condition="$config.open_extra_price eq 1 AND $vo.extra_price gt 0">+{pigcms{$now_order.extra_price}{pigcms{$config.extra_price_alias_name}</if></span>
						</dd>
						<dd class="dd-padding coupons-code">
					    ...
					    <if condition="$now_order['third_id'] eq '0' AND $now_order['pay_type'] eq 'offline'">
							 <dd class="dd-padding coupons-code">总金额： ${pigcms{$now_order['total_money']} </dd>
							 ...
							 <if condition="$now_order['wx_cheap'] neq '0.00'"><dd class="dd-padding coupons-code">	微信优惠 :${pigcms{$now_order['wx_cheap']} </dd></if>
							 <dd class="dd-padding coupons-code">线下需向商家付金额：<font color="red">${pigcms{$now_order['total_money']-$now_order['wx_cheap']-$now_order['merchant_balance']-$now_order['balance_pay']-$now_order['score_deducte']-$now_order['coupon_price']}元</font>
						<else/>
							 <dd class="dd-padding coupons-code">平台余额支付：{pigcms{$now_order.balance_pay} </dd>
							 <dd class="dd-padding coupons-code">商家会员卡余额支付：{pigcms{$now_order.merchant_balance} </dd>
							 <dd class="dd-padding coupons-code">在线支付金额：{pigcms{$now_order.payment_money} </dd>
						</if>

						<dd class="dd-padding coupons-code">
							买家留言： <span style="color:red"><b>{pigcms{$now_order.delivery_comment}</b></span>
						</dd>
					</dl>
				</dd>
			</dl>
			<if condition="$now_order['paid'] eq '1'">
				...
				<dl class="list">
					<dd>
						<dl>
							<dt><?php if(!$now_order['is_pick_in_store']){?>配送信息<?php }else{?>自提信息<?php } ?></dt>
							...
							<?php }?>
							<dd class="dd-padding coupons-code">
								<?php if(!$now_order['is_pick_in_store']){?>收货地址：<?php }else{?>取货地址：<?php } ?><span>{pigcms{$now_order.adress}</span>
							</dd>
