...
<body id="index">
        ...
        <form id="form" method="post" action="{pigcms{:U('My/recharge')}">
		    ...
		    <dl class="list">
		        <dd class="dd-padding">
		            <input id="money" placeholder="请填写充值金额" class="input-weak" type="text" name="money" value="{pigcms{$_GET.money}" <if condition="$_GET['label'] && $_GET['money']">readonly="readonly" onclick="$('#tips').html('订单充值时无法修改金额！').show();"</if>/>
		        </dd>
		    </dl>
