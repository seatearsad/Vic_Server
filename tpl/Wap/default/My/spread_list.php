        ...
        <dl class="list" style="margin-top:0px;">
		    <dd>
				<ul class="orderindex">
					...
					</li><li><a href="{pigcms{:U('My/spread_list',array('status'=>2))}" class="react <if condition="$_GET['status'] == '2'">hover</if>">
						<i class="text-icon">⌹</i>
						<span>已退款</span>
					</a>
					</li>
		    			...
		    			<volist name="spread_list" id="vo">
							<dd class="dealcard dd-padding">
			                    <div class="dealcard-block-right" style="margin-left:0px;">
			                        <div class="dealcard-brand">{pigcms{$vo.desc.txt} 《<a href="{pigcms{$vo.desc.url}" target="_blank" style="color:#F76120;">{pigcms{$vo.desc.info}</a>》</div>
			                        ...
			                        <weak style="color:black;">
										...
										<if condition="($vo.order_type eq 'store' OR $vo.order_type eq 'cash') AND $vo.status eq 0">
												<a class="btn btn-weak spread_check_btn" href="{pigcms{:U('My/spread_check',array('id'=>$vo['pigcms_id']))}">验证</a>
