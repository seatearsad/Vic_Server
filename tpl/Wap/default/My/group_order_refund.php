...
<body>
        ...
        <div class="wrapper-list">
			...
			<dl class="list">
			    <dd>
			        <dl>
						...
						<if condition="$unconsume_pass_num gt 0">
						<dd class="kv-line-r dd-padding">
							<h6>支付未消费数量：</h6><p><strong class="highlight-price">{pigcms{$unconsume_pass_num}</strong></p>
						</dd>
						</if>
						<dd class="kv-line-r dd-padding">
			    ...
			    <dd>
			        <dl>
							...
							<dd>
								<a class="react" href="javascript:;">
									<div class="more more-weak">
										<h6>使用商家优惠券：</h6>
										<span class="more-after">${pigcms{$now_order.card_price}</span>
									</div>
								</a>
							</dd>
						</if>
                        ...
                        <if condition="$now_order['score_deducte'] neq '0.00'">
                            <dd class="kv-line-r dd-padding">
                                <h6>使用{pigcms{$config.score_name}抵扣余额：</h6><p>{pigcms{$now_order.score_deducte}元</p>
                            </dd>
                        </if>
			...
			<if condition="$refund_money">
				<dl class="list">
					<dd>
						<dl>
							<dd class="kv-line-r dd-padding">
								<h6>应退金额：</h6><p><strong class="highlight-price">{pigcms{$refund_money}元</strong></p>
							</dd>
							<if condition="$refund_fee gt 0">
							<dd class="kv-line-r dd-padding">
								<h6>退款手续费：</h6><p><strong class="highlight-price" style="color:red;">{pigcms{$refund_fee}元</strong></p>
							</dd>
							</if>
						</dl>
					</dd>
			...
			$(function(){
				...
				$('#cancel').click(function(){
					if(cancel_){
						...
						window.location.href='{pigcms{:U('My/group_order_check_refund',array('order_id'=>$now_order['order_id']))}';
						cancel_ = false;
					}
				});
			});
