					...
					<dd>
						<dl>
							...
							<if condition="$score_deducte gt 0">
								<dd class="dd-padding">
									<label class="mt"><span class="pay-wrapper">本单可使用{pigcms{$config['score_name']}：{pigcms{$score_can_use_count}<?php if($_GET['type'] != 'gift'){ ?><br/>可抵扣金额：${pigcms{$score_deducte|floatval=###}<?php } ?><input type="checkbox" class="mt" value="1" id="use_score" name="use_score"><p style="display:block;float:right;">是否使用{pigcms{$config['score_name']}：</p></span></label>
								</dd>
							</if>
