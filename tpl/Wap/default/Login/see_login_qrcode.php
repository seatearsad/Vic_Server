...
<html>
	...
	<body>
		<p style="margin-top:20px;margin-bottom:20px;text-align:center;">{pigcms{:L('_B_D_LOGIN_WECHATSCANLOGIN_')}</p>
		...
		<script>
			...
			function ajax_weixin_login(){
				$.get("{pigcms{:U('Login/ajax_weixin_login')}",{qrcode_id:{pigcms{$id}},function(result){
					if(result == 'reg_user'){
						$('#login_status').html("{pigcms{:L('_B_D_LOGIN_HAVESCAN_')}").css('color','green').show();
						ajax_weixin_login();
					}else if(result == 'no_user'){
						$('#login_status').html("{pigcms{:L('_B_D_LOGIN_NOBODYSCANAGAIN_')}").css('color','red').show();
						window.location.href = redirect_url;
					}else if(result!=='true'){
						ajax_weixin_login();
					}else{
						$('#login_status').html("{pigcms{:L('_B_D_LOGIN_LOGINACCESSJUMP_')}").css('color','green').show();
