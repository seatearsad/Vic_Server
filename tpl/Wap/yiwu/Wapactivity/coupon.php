...
<body id="index">
		<div id="deal" class="deal">
			<div class="list">
			    ...
			    <dl class="list list-in">
					...
					<dd class="dd-padding buy-price" id="buy_box">
			            <div class="price">
			                <strong class="J_pricetag strong-color"><if condition="$now_activity['money']">{pigcms{$now_activity.money}<else/>{pigcms{$now_activity.mer_score}</if></strong>&nbsp;<span class="strong-color"><if condition="$now_activity['money']">元<else/>商家{pigcms{$config['score_name']}</if></span>
			                <space></space>
							<if condition="!$now_activity['money']"><del style="text-decoration:none;">或 {pigcms{$now_activity['mer_score']/$config['activity_score_scale']} 平台{pigcms{$config['score_name']}</del></if>
			            </div>
			        </dd>
					<ul class="campaign-tip dd-padding">
						<p class="w-goods-price">总数量：{pigcms{$now_activity.all_count}</p>
						<if condition="!$now_activity['is_finish']">
							<div class="w-progressBar">
								...
								<ul class="txt">
									<li class="txt-l"><p><b>{pigcms{$now_activity.part_count}</b>&nbsp;已发放</p></li>
						...
						<else/>
							<div class="m-detail-goods-result">
                                <p class="m-detail-goods-result-luckyCode" style="text-align:center;">已经发放完了</p>
                            </div>
						</if>
					</ul>
					<dd class="dd-padding m-detail-userCodes">
						<if condition="$user_part_list">
							<p><a href="{pigcms{:U('My/join_activity_detail',array('id'=>$now_activity['pigcms_id']))}">查看已领取的优惠券号码</a></p>
						<else/>
			...
			<dl class="list">
			    <dd>
			        <dl>
			            <dd class="dd-padding">
							<a href="{pigcms{:U('Wapactivity/intro',array('id'=>$now_activity['pigcms_id']))}" class="w-bar more">图文详情 <span class="w-bar-hint">( 建议在wifi下查看 )</span><span class="w-bar-ext"><b class="ico-next"></b></span></a>
			            </dd>
			        </dl>
			    </dd>
			</dl>
							...
							<volist name="vo" id="voo" key="j">
								<li>
									<if condition="$j eq 1">
										...
										<div class="text"><p class="f-breakword"><span class="txt-blue">{pigcms{$voo.nickname}</span>&nbsp;<span class="address">({pigcms{$voo.ip_txt} IP：{pigcms{$voo.ip})</span></p><p><span class="num">兑换了<span class="txt-red">{pigcms{$voo.part_count}</span>张</span>&nbsp;{pigcms{$voo.time|date='Y-m-d H:i:s',###}.{pigcms{$voo.msec}</p></div>
									<if condition="$j eq 1">
										</div>
									</if>
								</li>
		...
		<div class="m-simpleFooter m-detail-buy">
			<div class="m-simpleFooter-text" style="text-align:center;">
				<if condition="!$now_activity['is_finish']">
					<a id="quickBuy" class="w-button w-button-main" href="{pigcms{:U('Wapactivity/buy_coupon',array('id'=>$now_activity['pigcms_id']))}">立即兑换</a>
				<else/>
