...
<div class="qiandaobanner"><a href="javascript:history.go(-1);"><img src="{pigcms{$thisCard.payrecord}" ></a> </div>
<div class="jifen-box">
我的余额：{pigcms{$balance|default=0.00} 元 &nbsp; <!--button onclick="location.href='/wap.php?g=Wap&c=Card&a=topay&token={pigcms{$token}&cardid={pigcms{$_GET['cardid']|intval}'" style="float:right;color:red">充值</button-->
</div>
...
<tr>
<td>{pigcms{$re.createtime|date='m/d',###}</td>
<td>{pigcms{$re.ordername}</td>
<td><if condition="$re['type'] eq 1">+<else />-</if>{pigcms{$re.price}</td>
<td><if condition="$re['paid'] eq 1">成功<else /><a style="color:red" href="javascript:;">待付</a></if></td>
</tr>
</volist>
</tbody>
</table>
...
<a href="/wap.php?g=Wap&c=Card&a=expense&token={pigcms{$token}&cardid={pigcms{:intval($_GET['cardid'])}">
	<div class="jifen-box" style="text-align:center;margin-bottom:60px;font-size:12px;">
		{pigcms{$config['score_name']}和线下交易记录
	</div>
</a>
...
function dourl2(m){
	location.href= '/wap.php?g=Wap&c=Card&a=payRecord&token={pigcms{$token}&cardid={pigcms{$thisCard.id}&month='+m;
}
</script>
