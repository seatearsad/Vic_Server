...
<title>{pigcms{$thisCard.cardname}{pigcms{$config['score_name']}兑换</title>
...
<header>
	<nav id="nav_1" class="p_10">
		<ul class="box">
			<li><a href="wap.php?g=Wap&c=Card&a=integral&token={pigcms{$token}&cardid={pigcms{$thisCard.id}&type={pigcms{$type}&is_use=0" class="<if condition="$is_use eq '0'">on</if>">未使用</a></li>
			<li><a href="wap.php?g=Wap&c=Card&a=integral&token={pigcms{$token}&cardid={pigcms{$thisCard.id}&type={pigcms{$type}&is_use=1" class="<if condition="$is_use eq '1'">on</if>">已使用</a></li>
		</ul>
	</nav>
</header>
...
function use(i){
	...
	jQ.post('/wap.php?g=Wap&c=Card&a=action_useIntergral&token={pigcms{$token}', submitData,
	function(data) {
		if (data.success == 1) {
