...
<body id="index">
        ...
        <form id="form" method="post" action="{pigcms{:U('My/score_recharge')}">
		    ...
		    <dl class="list">
		        <dd class="dd-padding">
		            <input id="score" placeholder="请填写兑换积分数量" class="input-weak" type="text" name="score" value="{pigcms{$score_count}" <if condition="$_GET['label'] && $_GET['money']">readonly="readonly" onclick="$('#tips').html('订单兑换时无法修改金额！').show();"</if>/>
		        </dd>
		    </dl>
		    <p class="btn-wrapper">积分可兑换数为：{pigcms{$score_count} 个</p>
