			...
			<div id="scroller">
				<section class="shopDetail">
					...
					<if condition="$now_store['have_shop'] eq 1">
						<div class="linkInfo link-url" data-url="{pigcms{:U('Shop/index')}#shop-{pigcms{$now_store.store_id}">
							...
							<volist name="store_group_list" id="vo">
								<li  class="link-url" data-url="{pigcms{$vo.url}" <if condition="$i gt 2">style="display:none;"</if>>
									...
									<div class="dealcard-block-right">
										...
										<div class="price">
											<strong>{pigcms{$vo['price']}</strong><span class="strong-color">元<if condition="$vo.extra_pay_price gt 0">+{pigcms{$vo.extra_price}{pigcms{$config.extra_price_alias_name}</if></span><if condition="$vo['wx_cheap']"><span class="tag">微信再减{pigcms{$vo.wx_cheap}元</span></if><span class="line-right">已售{pigcms{$vo['sale_count']+$vo['virtual_num']}</span>
										</div>
									</div>
								</li>
							</volist>
							...
							<volist name="now_appoint" id="vo">
								<li  class="link-url" data-url="{pigcms{$vo.url}" <if condition="$i gt 2">style="display:none;"</if>>
									...
									<div class="dealcard-block-right">
										...
										<div class="price">
											<if condition='$vo["is_appoint_price"] neq 0'><strong>{pigcms{$vo['appoint_price']}</strong><span class="strong-color">元<php>if($vo['extra_pay_price']>0){</php>+{pigcms{$vo.extra_price}{pigcms{$config.extra_price_alias_name}<php>}</php></span><else /><strong>面议</strong></if><span class="line-right">已预约{pigcms{$vo['appoint_sum']}</span>
										</div>
									</div>
								</li>
							</volist>
