...
<form id="mpostForm" name="mpostForm" action="{pigcms{:U('Classify/classify_modify',array('cid'=>$cid))}" method="post"> 
	<if condition="$fabuset['isupimg'] eq 1">
		<div class="picNote">
			还可上传<em>8</em>张图片，已上传<em>0</em>张(非必填)
		</div>
				...
				<li class="align-top">
					<div class="item-content">
						<div class="item-inner">
							...
							<div class="item-input">
								<textarea id="Content" name="description" class="myborder"  placeholder="写上一些想要说明的内容" style="width: 90%;">
								{pigcms{$classify_userinput_info['description']}
								</textarea>
							</div>
						</div>
					</div>
     ...
     $(".imgGroup ul li.JSimg").find(".imgDelete").tap(function(){
	  ...
	  if ($(".upload_list").length) {
			var imgUpload = new ImgUpload({
				...
				countNum: "#uploadNum",
				url:"https://" + location.hostname + "/wap.php?g=Wap&c=Classify&a=ajaxImgUpload"
			})
		}
