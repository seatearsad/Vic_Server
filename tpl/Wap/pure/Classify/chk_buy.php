...
<body>
	...
	<form id="buy-form" method="POST" class="wrapper-list" autocomplete="off">
		<h4 style="margin-top:.4rem">{pigcms{$now_order.order_name}</h4>
		<dl class="list">
			<dd>
				<dl>
		...
		$(function(){
			...
			$('#buy-form').submit(function(){
				...
				if(submit_now == false){
					...
					$.post("{pigcms{$config.site_url}/wap.php?c=Classify&a=submit&order_id={pigcms{$_GET['order_id']}",{q:quantity.val()},function(result){
						
						submit_now = false;
						$('.mj-submit').html('提交').prop('disabled',false);
						if(result.status == -3){
							alert(result.info);
							window.location.href = "{pigcms{:U('Login/index',array('referer'=>urlencode(U('Classify/buy',array('id'=>$now_order['classify_userinput_id'])))))}";
							...
							layer.open({
								...
								yes: function(){
									...
									$('#label').val('wap_classify_'+{pigcms{$now_order.order_id}+'_'+quantity.val());
							...
							layer.open({
								...
								shadeClose: false,
								yes: function(){
									window.location.href = "{pigcms{:U('My/edit_adress',array('referer'=>urlencode(U('Gift/buy',array('id'=>$now_order['classify_userinput_id'])))))}";
								}
							});
						}else{
							layer.open({
						...
						if(result.status == 1){
							window.location.href = "{pigcms{:U('Classify/ShowDetail',array('id'=>$now_order['classify_userinput_id']))}";
						}else{
							if(result.status == -2){
