...
<html lang="zh-CN">
	<head>
		...
		<script type="text/javascript">
			var totalPage = {pigcms{$total},ajaxUrl = "{pigcms{:U('Appoint/ajaxFeedback',array('appoint_id' => $now_appoint['appoint_id']))}";
		</script>
	</head>
	<body>
		<if condition="!empty($list)">
			<section class="comment introList">
				<div class="titleDiv"><div class="title">评价<div class="rateInfo"><div class="starIconBg"><div class="starIcon" style="width:{pigcms{$now_appoint['score_mean']*20}%;"></div></div><div class="starText">{pigcms{$now_appoint.score_mean}</div></div><div class="right">{pigcms{$now_appoint.reply_count} 人评论</div></div></div>
			</section>
		</if>
		<div id="container">
			<div id="scroller">
		...
		window.shareData = {  
					...
					"moduleID":"0",
					"imgUrl": "", 
					"sendFriendLink": "{pigcms{$config.site_url}{pigcms{:U('Appoint/feedback', array('appoint_id' => $now_appoint['appoint_id']))}",
					"tTitle": "预约评价列表",
					"tContent": ""
		};
		</script>
