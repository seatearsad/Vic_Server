...
<head>
    <meta charset="utf-8"/>
	<title>{pigcms{:L('_PICK_UP_LIST_')}</title>
...
<section class="Navigation">
	<ul class="clr">
		<a href="{pigcms{:U('Deliver/pick')}"><li class="on" style="border-bottom:#25e28b 2px solid;color:#25e28b">{pigcms{:L('_CC_PICK_UP_')}</li></a>
	...
	<div class="Dgrab" id="Dgrab1">
		...
		<section class="robbed supply_{pigcms{$row['supply_id']}" data-id="{pigcms{$row.supply_id}">
			...
			<div class="sign_bottom">
				<a href="javascript:;" class="Pick" data-id="{pigcms{$row['supply_id']}">{pigcms{:L('_C_PICK_UP_')}</a>
			</div>
		</section>
		</volist>
		<else />
...
$(function(){
	...
	function grab(e) {
		...
		$.post(DeliverListUrl, "supply_id="+supply_id, function(json){
			mark = 0;
			if (json.status) {
				layer.open({title:['Reminder','background-color:#ffa52d;color:#fff;'],content:'Please start deliver',btn: ['Confirm'],end:function(){}});
			} else {
				layer.open({title:['Reminder','background-color:#ffa52d;color:#fff;'],content:json.info,btn: ['Confirm'],end:function(){}});
			}
