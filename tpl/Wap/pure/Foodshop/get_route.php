...
<body>
    ...
    <script>
        ...
        function realResult(result){
            ...
            var destination = encodeURIComponent('{pigcms{$now_store.area_name}{pigcms{$now_store.adress}');
            $('#frame_src').attr('src','http://api.map.baidu.com/direction?origin=latlng:'+result.result[0].y+','+result.result[0].x+'|name:'+origin+'&destination=latlng:{pigcms{$now_store.lat},{pigcms{$now_store.long}|name:'+destination+'&mode=driving&region=100010000&src=baidu|jsapi&output=html');
            motify.clearLog();
        }
    </script>
