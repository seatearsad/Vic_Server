					...
					<nav class="order_list">
						<volist name="order_list" id="vo">
							<section>
                                ...
                                <p><span>金额：</span>${pigcms{$vo.money|floatval=###}</p>
								<p><span>缴费时间：</span>{pigcms{$vo.time|date='Y-m-d H:i',###}</p>
								<if condition='$vo["order_type"] eq "property"'>
									<if condition='$vo["property_month_num"]'><p><span>物业费周期：</span>{pigcms{$vo['property_month_num']}个月</p></if>
									<if condition='$vo["presented_property_month_num"]'><p><span>物业费赠送周期：</span>{pigcms{$vo['presented_property_month_num']}个月</p></if>
									...
									<if condition='$vo["property_fee"]'><p><span>物业费：</span>{pigcms{$vo.property_fee}元/平方米/月</p></if>
...
<script type="text/javascript" language="javascript">
	$(function(){
		...
		var len=navL.length;
		var w=navL.outerWidth(true);
		scrollerIn.width(w*len+30);
		var myScroll;
			...
			$.get(link,function(data){
				if(data['status']){
					...
					for(var i in data['order_list']){
						...
						shtml +='</p><p><span>金额：</span>$'+data['order_list'][i]['money']+'</p><p><span>缴费时间：</span>'+data['order_list'][i]['time']+'</p>';
						if(data['order_list'][i]['order_type'] == 'property'){
							
							if(data['order_list'][i]['property_month_num']){
								shtml += '<p><span>物业费周期：</span>'+data['order_list'][i]['property_month_num']+'个月</p>';
							}
							
							if(data['order_list'][i]['presented_property_month_num']){
								shtml += '<p><span>物业费赠送周期：</span>'+data['order_list'][i]['presented_property_month_num']+'个月</p>';
							}
							...
							if(data['order_list'][i]['property_fee']){
								shtml += '<p><span>房屋面积：</span>'+data["order_list"][i]["property_fee"]+' 元/平方米/月</p>';
							}
