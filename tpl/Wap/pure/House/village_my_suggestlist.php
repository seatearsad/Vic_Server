...
<html lang="zh-CN">
	...
	<body>
    <if condition="!$is_app_browser">
        <header class="pageSliderHide"><div id="backBtn" onclick="location.href='{pigcms{:U('House/village_my', array('village_id' => $now_village['village_id']))}'"></div>投诉列表<div id="plus" onclick="location.href='{pigcms{:U('House/village_my_suggest',array('village_id'=>$now_village['village_id']))}'"><img src="{pigcms{$static_path}images/new_my/recharge.png" /></div></header>
    </if>
		<div id="container">
			<div id="scroller" class="village_my">
				<if condition="$repair_list">
					<nav class="order_list">
						<volist name="repair_list" id="vo">
							<section class="link-url" data-url="{pigcms{:U('House/village_my_suggest_detail',array('village_id'=>$vo['village_id'],'id'=>$vo['pigcms_id']))}">
				...
				<else/>
					<div class="noMoreDiv" style="margin-top:20px;background:#ebebeb;">您还没有使用投诉建议功能</div>
				</if>
                <if condition="!$is_app_browser">
