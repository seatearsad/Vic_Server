...
<html lang="zh-CN">
	<head>
		...
		var user_address='';var ajax_url_root = "{pigcms{$config.site_url}/wap.php?c=House&a=";var check_cart_url = "{pigcms{$config.site_url}/wap.php?c=Shop&a=confirm_order";var ajax_map_url = "{pigcms{$config.site_url}/index.php?g=Index&c=Map&a=suggestion&city_id={pigcms{$config.now_city}";var get_route_url = "{pigcms{:U('Group/get_route')}";var baiduToGcj02Url = "{pigcms{:U('Userlonglat/baiduToGcj02')}";var city_id="{pigcms{$config.now_city}";var cat_url="",sort_url="",type_url="";var noAnimate= true;var userOpenid="{pigcms{$_SESSION.openid}";var shopShareUrl = "{pigcms{$config.site_url}{pigcms{:U('Shop/index',array('openid'=>$_SESSION['openid']))}&shop-id=",shopReplyUrl = "{pigcms{$config.site_url}/index.php??g=Index&c=Reply&a=ajax_get_list&order_type=3&parent_id=";
		
		<if condition='$now_village["long"]'>var user_long = '{pigcms{$now_village["long"]}',user_lat = '{pigcms{$now_village["lat"]}'<else />var user_long = '0',user_lat = '0'</if></script>
		<script type="text/javascript" src="{pigcms{$static_path}js/house_shopBase.js?t={pigcms{$_SERVER.REQUEST_TIME}" charset="utf-8"></script>
		<style type="text/css">
		#pageCat #storeList{ margin-top:50px}
		</style>
	</head>
	<body>
		...
		<script type="text/javascript">
			window.shareData = {
				...
				"tTitle": "{pigcms{$config.shop_alias_name|default="周边快店"} - {pigcms{$config.site_name}",
