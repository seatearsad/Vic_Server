...
<html>
	...
	<body>
    <if condition="!$is_app_browser">
        <header class="pageSliderHide"><div id="backBtn" onclick="location.href='{pigcms{:U('House/village_my_repairlists')}'"></div>在线报修详情</header>
    </if>
		<div id="container">
			<div id="scroller">
				<section>
				...
				<elseif condition='$repair_detail["status"] eq 2' />
					...
					<if condition="$worker">
					<section>
						<p class="header">受理的维修员：<font color="green">{pigcms{$worker['name']}</font>，电话<a href="tel:{pigcms{$worker['phone']}">{pigcms{$worker['phone']}</font></p>
				...
				<elseif condition='$repair_detail["status"] eq 3' />
					...
					<if condition="$worker">
					<section>
						<p class="header">处理的维修员：<font color="green">{pigcms{$worker['name']}</font>，电话<a href="tel:{pigcms{$worker['phone']}">{pigcms{$worker['phone']}</font></p>
					...
					<a href="{pigcms{:U('House/village_comment',array('pigcms_id' => $repair_detail['pigcms_id'], 'type' => 'repair'))}" value="去评论"><div class="area_btn" style="font-size: 1.2rem;color: white;text-align: center;">去评论</div></a>
				<elseif condition='$repair_detail["status"] eq 4' />
					<if condition="$repair_detail['msg']">
					<section>
						<p class="header">接单时间：<font color="gray" style="font-size:12px;">{pigcms{$repair_detail.status_time_2|date='Y-m-d H:i',###}</font></p>
