...
<html lang="zh-CN">
	<head>
		...
		<style>
			...
			.header{margin-bottom:5px;font-size:16px;font-weight:bold}
	...
	<body>
    <if condition="!$is_app_browser">
        <header class="pageSliderHide"><div id="backBtn" onclick="location.href='{pigcms{:U('House/village_my_suggestlist')}'"></div>投诉列表详情</header>
    </if>
		<div id="container">
			<div id="scroller">
				<section>
					<p class="header">内容：</p>
					<p style="font-size:12px;color:#888888">{pigcms{$repair_detail.content}</p>
				</section>
				...
				<elseif condition='$repair_detail["status"] eq 3' />
					...
					<a href="{pigcms{:U('House/village_comment',array('pigcms_id' => $repair_detail['pigcms_id'], 'type' => 'suggest'))}" value="去评论"><div class="area_btn" style="font-size: 1.2rem;color: white;text-align: center;">去评论</div></a>
				<elseif condition='$repair_detail["status"] eq 4' />
						<if condition="$repair_detail['msg']">
						<section>
							<p class="header">接单时间：<font color="gray" style="font-size:12px;">{pigcms{$repair_detail.status_time_2|date='Y-m-d H:i',###}</font></p>
