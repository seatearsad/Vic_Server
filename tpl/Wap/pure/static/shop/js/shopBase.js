...
function searchAddress(address){
	...
	if(typeof(google) != "undefined"){
		...
		var input = (
			document.getElementById('pageAddressSearchTxt'));
...
//			$('#shopCatBar .title').html($(this).html());
	...
	if(nowShop.store && shopId == nowShop.store.id){
        ...
        if(nowShop.store.shop_remind != '' && nowShop.store.is_mult_class != 1){
            pageLoadHides();
...
console.log(cookieProductCart);
	if(cookieProductCart.length > 1){
...
console.log(cookieProductCartObj);
...
function getGeoconv(lng,lat){
	...
	getListGeoconvBack({result:[{x:lng, y:lat}]});
}
...
function getListGeocoder(){
	...
	$.getJSON("/index.php?g=Index&c=Map&a=geocoderGoogle&lng="+user_long+'&lat='+user_lat, function(d){
		//console.log(d);
		getListGeocoderBack(d)
	});
}
function getListGeocoderBack(obj){
	if(addressGeocoder == false){
		if(obj.result.length > 0){
			$('#locationText').html(obj.result[0].address);
			$.cookie('userLocationName',obj.result[0].address,{expires:700,path:'/'});
		}
		...
		var tmpName = obj.result.length > 0 ? obj.result[0].address : obj.result[0].address;
