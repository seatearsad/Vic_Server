...
$(function(){
	$('#car_form_add').click(function(){
		...
		$.post($(this).data('url'),{car_area:$('#car_area').val(),carnum:carnum,phone:phone},function(result){
			if(result.status == 1){
	...
	$('#car_form_edit').click(function(){
		...
		$('#loadingToast').fadeIn(100);
		$.post($(this).data('url'),{car_area:$('#car_area').val(),carnum:carnum,phone:phone,car_id:$(this).data('car_id')},function(result){
			$('#loadingToast').fadeOut(100);
			$('#iosDialog2 .weui-dialog__bd').html(result.info);
			$('#iosDialog2').show();
			return false;
