...
$(function(){
	...
	$('#input_6').scroller($.extend(opt['dateYMD'], opt['default']));
    
	$('#store_id').val('');
	$('#merchant_workers_id option:eq(0)').attr('selected',false);
	$('#service_time').val();
	...
	$('.bt-sub-order').click(function(){
			...
			if(tmpDom.data('role')){
				if(tmpDom.data('required')){
					if(tmpDom.data('role') == 'phone' && !/^0?1[3|4|5|7|8][0-9]\d{8}$/.test(slA[i].value)){
						...
						formError(tmpDom);
						return false;
					}else if(tmpDom.data('role') == 'position' && slA[i].value == ''){
						alert(tmpDom.data('long'));
						formError(tmpDom);
						return false;
		...
		$.post(window.location.href,$('#main_form').serialize(),function(result){
			if(result.status == 1){
				...
				//indow.location.href = 'history.go(-1)';
				setTimeout(function(){
					window.history.go(-1);
				},2000)
