...
//	$('#show_format_price').text(t_price);
	...
	if (this_index != null) {
		...
		$.cookie(cookie_buy_index, JSON.stringify([goodsCart[this_index]]), {expires:700,path:'/'});
	} else {
		this_num = num;
		if (stock != -1 && this_num > stock) {
			motify.log('库存不足，不能购买！');
	...
	$.cookie(cookie_buy_index, JSON.stringify([{
		'store_id':store_id,
		'productId':goods_id,
		'count':this_num,
		'productName':name,
