$(function () {
    ...
    $(".scUl li").click(function () {
		...
		$.post(ajax_get_presented_property_month_url,{'id':package},function(result){
			...
			if (result['status'] > 0) {
				...
				result['total_price'] && $('#totalmoney').text(result['total_price']);
			}
		},'json')
    });
	...
	$('#diy_propertyt_month_num').keyup(function(){
		...
		$.post(ajax_diy_get_presented_property_month_url,{'diy_propertyt_month_num':$(this).val(),'village_id':village_id},function(result){
			...
			if(result['status']){
				...
				$('#totalmoney').text(result['total_price']);
			}
		},'json')
	});
    ...
    $('#confirm').click(function () {
		layer.open({title:['是否确认提交？','background-color:#06c1ae;color:#fff;'],content:'金额：' +  $('#totalmoney').text(),shadeClose:false,btn: ['确定','取消'],yes:function(){
		layer.closeAll();
		...
		$.post(window.location.href,{'txt':'','money':money,'property_month_num':property_month_num,'presented_property_month_num':presented_property_month_num,'house_village_property_id':house_village_property_id},function(result){
			layer.closeAll();
