...
var pickerEvent = {
    Init: function (elemid) {
        ...
        html += htmlObj.right;
        html += "</div></div>";
        html += '<div style="clear: both;" class="rlzzc"></div>';
        elemId=elemid;
        ...
        document.getElementById("calendar_choose").style.left = 0+"px";
        document.getElementById("calendar_choose").style.top  = 0+"px";
        document.getElementById("calendar_choose").style.zIndex = 10000000;
        ...
        for (var i = 0; i < tds.length; i++) {
            if (tds[i].getAttribute("date") != null && tds[i].getAttribute("date") != "" && tds[i].getAttribute("price") != "-1") {
				if(tds[i].getAttribute("choose") == "yes"){
    ...
    setPriceArr: function (arr,setPriceTmp) {
    	setPriceDefault=setPriceTmp;
        obj.priceArr = arr;
    },
    remove: function () {
        var p = document.getElementById("calendar_choose");
...
var pickerHtml = {
    ...
    getLeft: function () {
    	if(obj.year == new Date().getFullYear() && obj.month == new Date().getMonth()+1){
			var left = '<div class="calendar_left pkg_double_month"><p class="date_text">' + obj.year + '年' + obj.month + '月</p><a href="javascript:void(0);" title="上一月" id="picker_last">上一月</a><a href="javascript:void(0);" title="下一月" id="picker_next" class="pkg_circle_bottom">下一月</a></div>';
    	}else{
    ...
    getRight: function () {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
				...
				if(obj.year < new Date().getFullYear() || obj.month < new Date().getMonth()+1 || (obj.month == new Date().getMonth()+1 && i-c < new Date().getDate())){
					chooseCan = "no";
					classStyle = "class='on no'";
				}else{
					chooseCan = "yes";
				}
				//判断今天
				if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
					html += '<td  ' + classStyle + ' choose="'+chooseCan+'" date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '"><a><span class="date basefix" style="color:#06c1ae">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}else if(obj.year <= new Date().getFullYear() && obj.month <= new Date().getMonth()+1 && i-c <= new Date().getDate()){
					html += '<td  ' + classStyle + ' choose="'+chooseCan+'" date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01" style="color:#999;"></span></a></td>';
				}else{
					html += '<td  ' + classStyle + ' choose="'+chooseCan+'" date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '"><a><span class="date basefix" style="color:#06c1ae">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
                if (index == 6) {
                    html += '</tr>';
                    index = -1;
...
//            $("#total_price").html(price*num);
            var checkbox_c5	=	$("#checkbox_c5").attr('checked');
			var checkbox_c4	=	$("#checkbox_c4").attr('checked');
			var nn=$(".num").val();
			var em=$(".Sanhe_right em").text();	//一张票的价格
			if(checkbox_c4 == undefined){
				var park=0;	//车位的价格
			}else{
				var park=$("#park").text()*park_number;	//车位的价格
			}
			if(checkbox_c5 == undefined){
				var guide_price=0;	//导游的价格
			}else{
				var guide_price=$("#guide_price").text()*1;	//导游的价格
			}
			$(".Advance i").text(nn*em+park+guide_price);
        }
    }
}
