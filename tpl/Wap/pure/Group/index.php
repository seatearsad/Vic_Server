...
<head>
    <meta charset="utf-8" />
    <title>{pigcms{:L('_VIC_NAME_')} - {pigcms{:L('_LUNCH_TXT_')}</title>
...
<div class="searchBox">
    <form id="search-form" action="{pigcms{:U('Search/group')}" method="post">
        <input type="search" id="keyword" name="w" placeholder="{pigcms{:L('_PUSH_SEARCH_KEY_')}" autocomplete="off"/>
    </form>
</div>
...
<section class="navBox pageSliderHide">
    <ul>
        <li class="dropdown-toggle caret category" data-nav="category">
            <span class="nav-head-name"><if condition="$now_category">{pigcms{$now_category.cat_name}<else/>{pigcms{:L('_ALL_CLASSIF_')}</if></span>
        </li>
        <li class="dropdown-toggle caret biz subway" data-nav="biz">
            <span class="nav-head-name"><if condition="$now_area">{pigcms{$now_area.area_name}<else/>{pigcms{:L('_ALL_CITY_')}</if></span>
        </li>
                        ...
                        <ul>
                            <li class="category-wrapper">
                                <ul class="dropdown-list">
                                    <li data-category-id="-1" <if condition="empty($top_category)">class="active"</if> onclick="list_location($(this));return false;"><span data-name="{pigcms{:L('_ALL_CLASSIF_')}">{pigcms{:L('_ALL_CLASSIF_')}</span></li>
                                        ...
                                        <div class="sub_cat hide" style="display:none;">
                                            <if condition="$vo['cat_count'] gt 1">
                                                <ul class="dropdown-list sub-list">
                                                    <li data-category-id="{pigcms{$vo.cat_url}" onclick="list_location($(this));return false;"><div><span class="sub-name" data-name="{pigcms{$vo.cat_name}">{pigcms{:L('_ALL_TXT_')}</span></div></li>
                            ...
                            <if condition="$all_area_list">
                                <li class="biz-wrapper">
                                    <ul class="dropdown-list">
                                        <li data-area-id="-1" <if condition="empty($now_area_url)">class="active"</if> onclick="list_location($(this));return false;"><span data-name="{pigcms{:L('_ALL_CITY_')}">{pigcms{:L('_ALL_CITY_')}</span></li>
                                        <volist name="all_area_list" id="vo">
                                            ...
                                            <div class="sub_cat hide" style="display:none;">
                                                <if condition="$vo['area_count'] gt 0">
                                                    <ul class="dropdown-list sub-list">
                                                        <li data-area-id="{pigcms{$vo.area_url}" onclick="list_location($(this));return false;"><div><span class="sub-name" data-name="{pigcms{$vo.area_name}">{pigcms{:L('_ALL_TXT_')}</span></div></li>
...
<div id="container">
    <div id="scroller">
                ...
                <ul class="goodList">
                    ...
                    <li class="link-url" data-url="{{ d.store_list[i].group_list[j].url }}" {{# if(j > 1){ }}style="display:none;"{{# } }}>
                        <div class="dealcard-img imgbox ">
                            ...
                            {{# if(d.store_list[i].group_list[j].is_start == 0){ }}<i class="store_state">{pigcms{:L('_NOT_BEGIN_')}</i>{{# } }}
                        </div>
                        <div class="dealcard-block-right">
                            <div class="title">{{ d.store_list[i].group_list[j].group_name }}</div>
                            <div class="price">
                                <strong>{{ d.store_list[i].group_list[j].price }}</strong><span class="strong-color">元{{# if( d.store_list[i].group_list[j].extra_pay_price!=''&&open_extra_price){ }}+{{  d.store_list[i].group_list[j].extra_pay_price }}{{  extra_price_name }}{{# } }}</span><div class="line_m">{{ d.store_list[i].group_list[j].old_price }}元{{# if( d.store_list[i].group_list[j].extra_pay_price){ }}{{  d.store_list[i].group_list[j].extra_pay_price }}{{# } }}</div> {{# if(d.store_list[i].group_list[j].wx_cheap){ }}<span class="tag">微信再减{{ d.store_list[i].group_list[j].wx_cheap }}元</span>{{# } }}<span class="line-right">
        ...
        <script id="groupListBoxTpl" type="text/html">
            ...
            <dd class="link-url" data-url="{{ d.group_list[i].url }}">
                ...
                <div class="dealcard-img imgbox">
                    ...
                    {{# if(d.group_list[i].is_start == 0){ }}<i class="store_state">{pigcms{:L('_NOT_BEGIN_')}</i>{{# } }}
                </div>
                <div class="dealcard-block-right">
                    ...
                    <div class="price">
                        <strong>{{ d.group_list[i].price }}</strong><span class="strong-color">元{{# if( d.group_list[i].extra_pay_price!=''){ }}{{  d.group_list[i].extra_pay_price }}{{# } }}</span>{{# if(d.group_list[i].wx_cheap){ }}<span class="tag">微信再减{{ d.group_list[i].wx_cheap }}元</span>{{# }else{ }}<del>{{ d.group_list[i].old_price }}</del>{{# } }} {{# if( d.group_list[i].extra_pay_price!=''){ }}{{  d.group_list[i].extra_pay_price }}{{# } }}<span class="line-right">{{ d.group_list[i].sale_txt }}</span>
                    </div>
                </div>
            </dd>
            {{# } }}
        ...
        <section class="storeListBox listBox">
            ...
            <div class="no-deals hide">{pigcms{:L('_NO_SUCH_OTHER_')}</div>
        </section>
        <div id="pullUp">
