...
<body id="index">
        ...
        <form id="form" method="post" action="{pigcms{:U('My/withdraw')}">
		    ...
		    <dl class="list">
		        <dd class="dd-padding">
				 <input id="truename" placeholder="填写真实姓名，否则无法提现" class="input-weak" type="text" name="truename" value="{pigcms{$user_info['truename']}"/>
				 </dd>
		        <dd class="dd-padding">
		            <input id="money" placeholder="请填写提现金额" class="input-weak" type="text" name="money" value="{pigcms{$_GET.money}"/>
		        </dd>
				<if condition="$config.company_pay_user_percent gt 0">
				<dd class="dd-padding" id="percent">
		          应收服务费:0 元
		        </dd>
				</if>
		    </dl>
		    <p class="btn-wrapper">金额最多仅支持两位小数,只支持微信提现</p>
		    <p class="btn-wrapper">余额：{pigcms{$user_info['now_money']}元,<if condition="$now_user.frozen_money gt 0">冻结金额：{pigcms{$now_user.frozen_money|floatval}元,</if>可提现：{pigcms{$user_info.can_withdraw_money}元<if condition="$config.company_pay_user_percent gt 0">,提现服务费率{pigcms{$config.company_pay_user_percent}%</if></p>
		...
		<script>
		var percent = Number('{pigcms{$config.company_pay_user_percent}')
			$(function(){
				$('#money').change(function(){
					var percent_money = ($(this).val()*percent/100).toFixed(2);
					$('#percent').html('应收服务费:'+percent_money+'元')
				});
			})
