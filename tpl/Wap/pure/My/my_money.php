...
<body>
		...
		<dl id="levelUpdate" class="title" <php>if($config['level_onoff']==0){ echo 'style="display:none"'; }</php>>
			<div class="pb10">
				...
				<div style="padding-top:12px;width:80%;">{pigcms{:L('_LEVEL_MANAGE_')}
					...
					<else/>
					<div style="position:absolute;color:#fff;background-color:#ffb80f;padding:0px 8px;right:1.8em;top:301px;border-radius:10px;">VIP<span style="font-size:17px;">{pigcms{$now_user.level}</span></div>
					</if>
				</div>
				<img class="title_img" src="{pigcms{$static_path}images/new_my/tubiao2_11.png"></img>
			</div>
	...
	<if condition="$config['score_recharge'] AND $config.open_extra_price eq 0">
	<dl id="score_recharge" class="title">
		...
		<script>
			...
			$('#withdraw').on('click',function(){
				location.href =	"{pigcms{:U('My/withdraw')}";
			});
