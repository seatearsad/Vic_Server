						...
						<div class="dealcard" <if condition='!$now_order["appoint_id"]'>style=" height:1.8rem"</if> >
							...
							<div class="dealcard-block-right">
								...
								<div class="title text-block">
									<if condition='$now_order["payment_status"] eq 1'>定金：${pigcms{$now_order.payment_money}元</if><br/>
									服务类型：
										<if condition='!$now_order["appoint_id"]'>
											<span style="color:green">自营</span>
										<else />
											<if condition="$now_order['appoint_type'] eq 0"><span>到店</span>
												<elseif condition="$now_order['appoint_type'] eq 1" /><span>上门</span>
											</if>
										</if>
				...
				<dd>
					<dl>
						...
						<ul class="ul">
							...
							<if condition="$now_order['product_card_give_money'] neq 0"><li>商家会员卡赠送余额：{pigcms{$now_order['product_card_give_money']}元</li></if>
							<if condition="$now_order['product_merchant_balance'] neq 0"><li>平台余额抵扣：{pigcms{$now_order['product_merchant_balance']}元</li></if>
							<if condition="$now_order['product_card_price'] gt 0"><li>商家优惠券：{pigcms{$now_order['product_card_price']}元</li></if>
							<if condition="$now_order['product_coupon_price'] gt 0"><li>平台优惠券：{pigcms{$now_order['product_coupon_price']} 元</li></if>
				
							<if condition="$now_order['product_score_deducte'] gt 0"><li>{pigcms{$config.score_name}抵扣：{pigcms{$now_order['product_score_deducte']}元</li></if>
							...
							<elseif condition="($now_order['payment_money'] gt 0) AND ($now_order['is_initiative'] eq 1)" /><li>定金支付金额：{pigcms{$now_order['payment_money']}元</li>
							...
							<li>余款支付方式：<if condition="!empty($now_order['product_pay_type_txt'])">{pigcms{$now_order['product_pay_type_txt']}<else />余额支付</if></li>
							
							<if condition="$now_order['user_pay_time'] gt 0"><li>余额支付时间：{pigcms{$now_order['user_pay_time']|date='Y-m-d H:i:s',###}</li></if>
							</if>
