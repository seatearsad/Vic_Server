...
<body>
	...
	<form id="buy-form" method="POST" class="wrapper-list" autocomplete="off">
		<h4 style="margin-top:.4rem">{pigcms{$now_order.appoint_name}</h4>
		<dl class="list">
			<dd>
				<dl>
					...
					<dd class="dd-padding kv-line-r quantity">
						...
						<div class="kv-v">
							<div class="stepper" data-com="stepper">
								&nbsp;<input class="mt number" type="tel" name="quantity" min="1" max="1" disabled="disabled" value="1"/>
							</div>
						</div>
					</dd>
					...
					<dd class="dd-padding kv-line-r">
						...
						<span class="kv-v" id="amount">
							<span class="J_total-price">{pigcms{$now_order['appoint_price'] - $now_order['payment_money']}元</span>
		...
		$(function(){
			...
			$('#buy-form').submit(function(){
				...
				if(submit_now == false){
					...
					$.post("{pigcms{$config.site_url}/wap.php?c=My&a=submit&order_id={pigcms{$_GET['order_id']}",{q:quantity.val()},function(result){
						submit_now = false;
						$('.mj-submit').html('提交').prop('disabled',false);
						if(result.status == -3){
							window.location.href = "{pigcms{:U('Login/index',array('referer'=>urlencode(U('My/buy',array('id'=>$now_order['order_id'],'type'=>$_GET['type'])))))}";
							...
							layer.open({
								...
								yes: function(){
									...
									$('#label').val('wap_appoint_'+{pigcms{$now_order.order_id});
							...
							layer.open({
								...
								shadeClose: false,
								yes: function(){
									window.location.href = "{pigcms{:U('My/edit_adress',array('referer'=>urlencode(U('My/buy',array('id'=>$now_order['order_id'],'type'=>$_GET['type'])))))}";
								}
							});
						}else{
							layer.open({
						...
						if(result.status == 1){
							window.location.href = "{pigcms{:U('My/appoint_order',array('order_id'=>$now_order['order_id']))}";
						}else{
							if(result.status == -2){
