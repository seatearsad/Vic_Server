...
<body id="index">
        ...
        <div class="wrapper-list">
			...
			<dl class="list coupons" id="share-url" <php>if($now_order['single_buy']==1||$is_wexin_browser||$is_app_browser||$my_group_join['status']!=0||($now_order['status']==4||$now_order['status']==3||$now_order['status']==6)||$effective_time < 0){</php>style="display:none"<php>}</php>>
				<dd>
					<dl>
						<dd class="dd-padding coupons-code">
			...
			<elseif condition="$now_order['status'] eq 0 AND $now_order['total_money'] neq 0"/>
				<div class="btn-wrapper">
					<if condition="$effective_time gt 0 AND $my_group_join['status'] eq 0 AND $now_order['single_buy'] eq 0"><span class="order-cancel" id="share_group" style="float:left;"  onclick="<php>if($is_wexin_browser){</php>_system._guide(true)<php>}else{</php>copy()<php>}</php>">分享团购</span></if>
						
					<if condition="$now_group['group_refund_fee'] eq 100"><span class="order-cancel" style="background-color:#BBB9B5;">该团购不能退款</span><elseif condition="$now_order.is_head gt 0 AND $effective_time gt 0 AND $my_group_join['status'] lt 2"/><span class="order-cancel" style="background-color:#BBB9B5;">团长订单不能取消</span>
    ...
    <script src="{pigcms{:C('JQUERY_FILE')}"></script>
							...
							if(user_arr){
								$.each(user_arr, function(index, val) {
									...
									if($('#uid-'+val.uid).length==0){
										...
										$('#user-list').append('<div id="uid-'+val.uid+'" data-id="'+val.uid+'"><img class="img_list" src="'+val.avatar+'" /><span >'+val.nickname+'</span></div>');
									}
		...
		<script type="text/javascript">
			...
			window.shareData = {  
				...
				"tContent": "{pigcms{$user_session['nickname']}邀请你参加【{pigcms{$now_group['name']}】团购，享受优惠"
			};
