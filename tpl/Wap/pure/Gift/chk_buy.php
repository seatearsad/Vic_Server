...
<body>
	...
	<form id="buy-form" method="POST" class="wrapper-list" autocomplete="off">
		<h4 style="margin-top:.4rem">{pigcms{$now_order.gift_name}</h4>
		<dl class="list">
			<dd>
				<dl>
					<dd class="dd-padding kv-line-r">
						<h6>库存剩余：</h6>
						<p>{pigcms{$now_order['sku'] - $now_order['sale_count']}次</p>
					...
					<dd class="dd-padding kv-line-r quantity">
						...
						<div class="kv-v">
							<div class="stepper" data-com="stepper">
								&nbsp;<input class="mt number" type="tel" name="quantity" min="{pigcms{$now_order['num']}" max="{pigcms{$now_order['num']}" disabled="disabled" value="{pigcms{$_GET['num']}"/>
							</div>
						</div>
					</dd>
					...
					<dd class="dd-padding kv-line-r">
						...
						<span class="kv-v" id="amount">
							<span class="J_total-price">{pigcms{$now_order['total_price']}元</span>
		...
		$(function(){
			...
			$('#buy-form').submit(function(){
				...
				if(submit_now == false){
					...
					$.post("{pigcms{$config.site_url}/wap.php?c=Gift&a=submit&order_id={pigcms{$_GET['order_id']}&source=wap",{q:quantity.val()},function(result){
						submit_now = false;
						$('.mj-submit').html('提交').prop('disabled',false);
						if(result.status == -3){
							alert(result.info);
							window.location.href = "{pigcms{:U('Login/index',array('referer'=>urlencode(U('Gift/buy',array('id'=>$now_order['gift_id'])))))}";
							...
							layer.open({
								...
								yes: function(){
									...
									$('#label').val('wap_gift_'+{pigcms{$now_order.order_id}+'_'+quantity.val());
							...
							layer.open({
								...
								shadeClose: false,
								yes: function(){
									window.location.href = "{pigcms{:U('My/edit_adress',array('referer'=>urlencode(U('Gift/buy',array('id'=>$now_order['gift_id'])))))}";
								}
							});
							...
							layer.open({
								...
								shadeClose: false,
								yes: function(){
									window.location.href = "{pigcms{:U('success_order',array('order_id'=>$_GET['order_id']))}";
								}
							});
						}else{
							layer.open({
						...
						if(result.status == 1){
							window.location.href = "{pigcms{:U('Gift/detail',array('id'=>$now_order['gift_id']))}";
						}else{
							if(result.status == -2){
