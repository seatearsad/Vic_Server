...
$(function(){
	...
	$('.nav .form_sec div').live('click',function(){
		...
		if($(this).hasClass('meal')){
			search_form.attr('action',search_form.attr('meal_action')).find('.form_input .input').attr('placeholder','请输入店铺名称');
		}
		if($(this).hasClass('group')){
			search_form.attr('action',search_form.attr('group_action')).find('.form_input .input').attr('placeholder','请输入商品名称、地址等');
		}
...
function levelToupdate(currentscore,needscore,obj){
	...
	if(currentscore>0 && needscore>0){
	   ...
	   if(confirm("升级会扣除您"+needscore+score_name+"\n您确认要升级吗？")){
