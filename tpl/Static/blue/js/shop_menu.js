...
$(document).ready(function(){
    ...
    position.lng = parseFloat($.cookie('shop_select_lng'));
	position.lat = parseFloat($.cookie('shop_select_lat'));

	var pyrmont = {lat: parseFloat(store_lat), lng: parseFloat(store_long)};
	var map = null;
	//console.log(pyrmont);
	...
	var marker = new google.maps.Marker({
		...
		//draggable: true,
        map: map,
        icon:"/tpl/Wap/default/static/images/map/my_pos.png"
    });
    ...
    var marker2 = new google.maps.Marker({
		...
		//draggable: true,
        map: map,
        icon:"/tpl/Wap/default/static/images/map/store_pos.png"
	});
	...
	$(document).on('click', '.carmiddle ul a', function(){
		...
		cartFunction(name, price, goods_id, goodsCartKey, $(this).attr('class'), '', packing_charge, stock,extra_price);
	});
	...
	$(document).on('click', '.purchase', function(){
		...
		if (!cartFunction(name, price, goods_id, goodsCartKey, 'jia', productParam, packing_charge, stock,extra_price)) {
			return false;
		}
	    ...
	    flyer.fly({
	        ...
	        onEnd:function(){
//	            cartFunction(name, price, goods_id, goodsCartKey, 'jia', productParam, packing_charge, stock);
	            flyer.remove();
	        }
	    });
	});
	...
	$(document).on('click', '.Popup .plus a', function(){
		...
		if ($(this).attr('class') == 'jia') {
			...
			if (!cartFunction(name, price, goods_id, goodsCartKey, 'jia', '', packing_charge, stock,extra_price)) {
				return false;
			}
		    ...
		    flyer.fly({
		        ...
		        onEnd:function(){
//		            cartFunction(name, price, goods_id, goodsCartKey, 'jia', '', packing_charge, stock);
		            flyer.remove();
		        }
		    });
		} else {
			cartFunction(name, price, goods_id, goodsCartKey, 'jian', '', packing_charge, stock,extra_price);
		}
	});
	...
	$(document).on('click', '.click', function(event){
		if ($(this).data('has_format')) {
			...
			if (!cartFunction(name, price, goods_id, goodsCartKey, 'jia', '', packing_charge, stock,extra_price,extra_price)) {
				return false;
			}
...
function popup_html(data, has_format, goodsCartKey)
{
	...
	if (this_index != null) {
		...
		html += '<a href="javascript:void(0)" class="jia" data-price="' + goodsCart[this_index].productPrice + '" data-stock="' + data.stock_num + '" data-packing_charge="' + data.packing_charge + '" data-goods_id="' + data.goods_id + '" data-index="' + goodsCartKey + '" data-name="' + data.name + '" data-extra_pay_price="' + data.extra_pay_price + '">+</a>';
		html += '</div>';
	} else {
		...
		html += '<a href="javascript:void(0)" class="jia" data-price="' + data.price + '" data-stock="' + data.stock_num + '" data-packing_charge="' + data.packing_charge + '" data-goods_id="' + data.goods_id + '" data-index="' + data.goods_id + '" data-name="' + data.name + '" data-extra_pay_price="' + data.extra_pay_price + '">+</a>';
		html += '</div>';
	}
...
//格式化购物车的html
function format_cart_html(num, name, price, goods_id, index, packing_charge, stock,extra_price)
{
	...
	div += '<a href="javascript:void(0)" class="jian" data-price="' + price + '" data-stock="' + stock + '" data-packing_charge="' + packing_charge + '" data-goods_id="' + goods_id + '" data-index="' + index + '" data-name="' + name + '" data-extra_pay_price="' + extra_price + '">-</a>';
	div += '<input type="text" value="' + num + '" readonly="readonly">';
	div += '<a href="javascript:void(0)" class="jia" data-price="' + price + '" data-stock="' + stock + '" data-packing_charge="' + packing_charge + '" data-goods_id="' + goods_id + '" data-index="' + index + '" data-name="' + name + '" data-extra_pay_price="' + extra_price + '">+</a>';
	div += '</div>';
...
function cartFunction(name, price, goods_id, index, type, productParam, productPackCharge, stock,extra_price)
{
...
//	}
	...
	if (this_index != null) {
		if (type == 'jia') {
			...
			goodsCartPackCharge += productPackCharge;
		} else {
			this_num = goodsCart[this_index].count - 1;
			if (stock != -1 && this_num > stock) {
				layer.msg('库存不足，不能购买！');
...
//	console.log('total===============' + goodsCartPackCharge);
	var div = format_cart_html(this_num, name, price, goods_id, index, productPackCharge, stock,extra_price);
...
//	var nowShopCart = $.parseJSON($.cookie(cookie_index));
	...
	for (var i in nowShopCart) {
		if (nowShopCart[i] != null && nowShopCart[i].count > 0) {
			...
			cart_goods_html += format_cart_html(nowShopCart[i].count, nowShopCart[i].productName, nowShopCart[i].productPrice, nowShopCart[i].productId, goodsCartKey, nowShopCart[i].productPackCharge, nowShopCart[i].productStock,nowShopCart[i].productExtraPrice);
