	...
	$.getScript("http://api.map.baidu.com/getscript?v=2.0&ak=4c1bb2055e24296bbaef36574877b4e2",function(){
			...
			onSearchComplete:function(results){
				...
				if(search_count > 0){
					...
					for(var i=0;i<search_count;i++){
						...
						result_panel += '<li class="result-item" data-lng="'+now_poi.point.lng+'" data-lat="'+now_poi.point.lat+'" data-title="'+now_poi.title+'" data-content="'+now_poi.address+'"><span class="icon icon-'+i+'"></span><a class="J-show-around-deals btn-selected" href="javascript:;">查看附近'+shop_alias_name+'</a><h3>'+now_poi.title+'</h3><p class="desc">地址：'+now_poi.address+'</p></li>';
		...
		$('#result-panel .result-item').live('mouseover mouseout click',function(e){
			if(e.type == 'mouseover'){
				...
				(new BMap.Geocoder()).getLocation(pt,function(rs){
					if(rs.surroundingPois && rs.surroundingPois[0] && rs.surroundingPois[0].address){
						if(rs.surroundingPois[0].title){
							var tmpAddress = rs.surroundingPois[0].title;
						}else{
							var tmpAddress = rs.surroundingPois[0].address;
						}
					}else{
						addComp = rs.addressComponents;
						var tmpAddress = addComp.city + addComp.district + addComp.street;
					}
					infoWindow = new BMap.InfoWindow('<div class="infowin-box">位置：<em class="poi-address" style="height:40px;display:block;">'+ tmpAddress +'</em><p style="text-align:center;"><a href="javascript:setSelect(\''+ tmpAddress +'\',\''+pt.lat+'\',\''+pt.lng+'\');" class="J-show-around-deals btn btn-normal btn-small" hidefocus="true" style="color:black;">查看附近'+shop_alias_name+'</a></p></div>',info_opts);
...
function setSelect(adress,lat,lng){
	...
	document.cookie = "userLocationName=" + encodeURIComponent(adress) + ";path=/;expires=" + exp.toGMTString(); 
	document.cookie = "userLocationLat=" + lat + ";path=/;expires=" + exp.toGMTString(); 
	document.cookie = "userLocationLong=" + lng + ";path=/;expires=" + exp.toGMTString(); 
	document.cookie = "userLocation=" + lng + "," + lat + ";path=/;expires=" + exp.toGMTString(); 
