$(function(){
	...
	$('button').click(function(){
		...
		finalprice=finalprice > 0 ? finalprice : group_price;
				...
				if(input_dom.attr('data-max') != '0' && input_Val >= parseInt(input_dom.attr('data-max'))){
					...
					if(extra_price>0){						
						...
						if(open_extra_price==1&& extra_price>0){
							alert('');
							$('#J-deal-buy-total').html(((input_Val+1)*(group_price*10000))/10000+'+'+extra_price*(input_Val+1)+extra_price_name);
							var s = (((input_Val+1)*(finalprice*10000))/10000+'+'+extra_price*(input_Val+1)+extra_price_name) * 1.05 + "";
							$('#deal-buy-total-t,#levelofftotal').html(s.substring(0, s.indexOf(".") + 3));
						}else{
							$('#J-deal-buy-total').html(((input_Val+1)*(group_price*10000))/10000);
							var s = ((input_Val+1)*(finalprice*10000))/10000 * 1.05 + "";
							$('#deal-buy-total-t,#levelofftotal').html(s.substring(0, s.indexOf(".") + 3));
		...
		if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test($(this).val())){
			...
			if(input_dom.attr('data-max') != '0' && input_Val >= parseInt(input_dom.attr('data-max'))){
				...
				if(open_extra_price && extra_price >0){
					...
					$('#J-deal-buy-total,#deal-buy-total-t').html(((input_Val)*(group_price*10000)/10000+'+'+extra_price*(input_Val)+extra_price_name) * 1.05);
				}else{
					
					$('#J-deal-buy-total,#deal-buy-total-t').html((input_Val)*(group_price*10000)/10000 * 1.05);
				}
			}
		}
	});
