...
.expand{margin-left:130px;}
 ...
 <div id="site-mast" class="site-mast"><include file="topbar"/></div>
	 ...
	 <a href="{pigcms{$siteUrl}/classify/" target="_blank"><img src="{pigcms{$config.classify_logo}" alt="{pigcms{$config.classify_name}" title="{pigcms{$config.classify_name}" width="180" height="58" /></a>
	 </span>
	 </header>
	 <else/>
	 <header id="postheader" style="height:58px">
	 <span id="logo" style="top:12px;">
	 <a href="/" target="_blank"><img src="{pigcms{$config.site_logo}" alt="{pigcms{$config.classify_name}" title="{pigcms{$config.classify_name}" width="160" height="45" /></a>
				...
				if(response.error == '0'){
					//if(response.data){
					  $.each(response.data,function(kk,vv){
					     $('.loading-'+response.id).replaceWith('<li class="J-pic-thumbnail pic-item"><input type="hidden" name="inputimg[]" value="'+response.imgurl+'"><img src="'+response.urlpath+'" width="120" height="110"/><a href="javascript:void(0);" class="pic-item__remove" uploader_id="'+file.id+'" hidden="hidden" style="display:none;">X&nbsp;</a></li>');
					  });
					//}
				}else{
