				...
				<volist name="index_top_adver" id="vo">
					<li class="common-banner__sheet cf mt-slider-sheet mt-slider-current-sheet" style="<if condition='$i eq 1'>opacity:1;<else/>opacity:0;display:none;</if>">
						<a class="common-banner__link" target="_blank" href="{pigcms{$vo.url}">
							<img onload="delayImg(this);" src="{pigcms{$static_public}images/empty.png" data-original="{pigcms{$vo.pic}" width="980" height="60" alt="{pigcms{$vo.name}"/>
						</a>
					</li>
				</volist>
			</ul>
	...
	<div class="component-search-box">
		<div class="J-search-box search-box ">
			<form action="{pigcms{:U('Meal/Search/index')}" class="search-box__form J-search-form" name="searchForm" method="post" group_action="{pigcms{:U('Group/Search/index')}" meal_action="{pigcms{:U('Meal/Search/index')}">
				...
				<input tabindex="1" type="text" name="w" autocomplete="off" class="s-text search-box__input J-search-box__input" value="{pigcms{$keywords}" placeholder="请输入店铺名称"/>
				<input type="submit" class="s-submit search-box__button" hidefocus="true" value="搜&nbsp;&nbsp;索"  data-mod="sr"/>
			</form>
			<ul class="search-suggest J-search-suggest" style="display:none;"></ul>
		...
		<div class="site-mast__site-nav-inner">
			<div class="component-cate-nav">
				...
				<div class="cate-nav J-nav__list J-nav__list--delayed log-mod-viewed" style="display:none;">
					<volist name="all_category_list" id="vo" key="k" offset="0" length="10">
			...
			<nav>
				<ul class="navbar cf">
					<volist name="web_index_slider" id="vo" offset="0" length="9">
