							...
							<volist name="order_list" id="vo">
								<div class="J-order-w">
									<div class="order-title">订单编号：
										<a href="{pigcms{:U('Index/appoint_order_view',array('order_id'=>$vo['order_id']))}" target="_blank">{pigcms{$vo.order_id}</a>
										<if condition="($vo['paid'] == '0') && ($vo['is_del'] == '0')">
									...
									<div class="order-row">
										<div class="order-cell order-op order-cell--right">
											...
											<elseif condition="$vo['paid'] eq 1 && $vo['payment_status'] eq 1"/>
												<a href="#">已付款</a>
											<elseif condition="$vo['paid'] eq 0 && $vo['payment_status'] eq 1 && $vo['is_del'] eq 0" />
												<a class="btn-hot btn-mini" href="{pigcms{:U('Index/Pay/check',array('type'=>'appoint','order_id'=>$vo['order_id']))}">付款</a>
										...
										<div class="order-row--sub order-row--last">
											<div class="order-cell order-info">
												<div class="deal-info cf">
													...
													<div class="info-detail">
														...
														<if condition="$vo['appoint_date']"><p>预约时间：<?php echo date('Y-m-d', strtotime($vo['appoint_date'])) ?>&nbsp;&nbsp;{pigcms{$vo.appoint_time}</p>
														<else />
															<if condition='$type eq 1'>
																<p style="color:red">平台分配订单</p>
															<else />
																<p style="color:red">商家分配订单</p>
															</if>
														</if>
											...
											<div><a target="_blank" href="{pigcms{:U('Index/appoint_order_view',array('order_id'=>$vo['order_id']))}">订单详情</a></div>
											<div><if condition='$vo["is_del"]'><span style=" color:red"><switch name='vo["is_del"]'>
