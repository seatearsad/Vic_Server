									...
									<div class="order-row">
										<div class="order-cell order-op order-cell--right">
											...
											<elseif condition="$vo['status'] eq 0" />											
												<a class="btn-hot btn-mini" href="javascript:void(0)" onclick="meal_order_cancel({pigcms{$vo['order_id']})">取消</a>
											<elseif condition="$vo['status'] == 1"/>
												<a class="btn-hot btn-mini" href="{pigcms{:U('Rates/meal')}">评价</a>									
											</if>
										</div>
										<div class="order-row--sub order-row--last">
											...
											<div><a target="_blank" href="{pigcms{:U('Index/meal_order_view',array('order_id'=>$vo['order_id']))}">订单详情</a></div></div>
										</div>
									</div>
								</div>
							</volist>
