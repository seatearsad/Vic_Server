...
<body>
 ...
 <div class="body pg-buy-process"> 
							...
							<dt>
								...
								<em class="info-section--text">
									<if condition="$now_order['status'] eq 3"  >
										已取消并退款
									<elseif condition="$now_order['status'] == '4'"/>
										已取消
									<elseif condition="empty($now_order['paid']) AND ($now_order['status'] eq 0)" />
										未付款
							...
							<dd class="last">
							  ...
							  <if condition="$now_order['status'] eq '3'" >
								...
								<elseif condition="($now_order['status'] eq '0') AND ($now_order['paid'] eq '1')" />
									<div class="operation">
										<a class="btn btn-mini" href="javascript:void(0)" onclick="meal_order_cancel({pigcms{$now_order['order_id']})">取消订单</a>
									</div>
	...
	<script type="text/javascript">
		...
		function meal_order_cancel(order_id){
			if(confirm('确认取消该订单？')){
				var cancelUrl = "{pigcms{:U('meal_order_check_refund')}";
