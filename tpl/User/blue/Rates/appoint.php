...
<title>待评价预约订单 | {pigcms{$config.site_name}</title>
				...
				<div id="content" class="coupons-box">
					<div class="mainbox mine">
						<select class="J-orders-filter orders-filter dropdown--small">
							...
							<option value="{pigcms{:U('Rates/shop')}">{pigcms{$config.shop_alias_name}</option>
							<option value="{pigcms{:U('Rates/appoint')}" selected="selected">预约</option>
						</select>
										...
										<volist name="order_list" id="vo">
											<div class="rate-item <if condition="$i eq count($order_list)">rate-item--last</if>">
												<div class="rate-item__title">
													<a href="{pigcms{$vo.url}" target="_blank" title="{pigcms{$vo.appoint_name}">
												...
												<div class="J-rate-content rate-item__content">
													<h3 class="J-deal-title">
														<a href="{pigcms{$vo.url}" title="{pigcms{$vo.appoint_name}" target="_blank">{pigcms{$vo.appoint_name}</a>
													</h3>
													<p class="reminding color-weaken">你为本次{pigcms{$vo.appoint_name}打几分？</p>
													...
													<form action="{pigcms{:U('Rates/reply_to',array('order_id'=>$vo['order_id']))}" order_id="{pigcms{$vo.order_id}" method="post" class="J-edit-wrapper" style="display:none">
														...
														<p class="area-tips">（最少评论15字，您的评价将是其他用户的重要参考）亲，您觉得本{pigcms{$vo.appoint_name}怎么样？赶快告诉其他用户吧</p>
						...
						<else/>
							<div class="notice">您没有待评价的预约订单</div>
						</if>
                    </div>
				</div>
			</div> <!-- bd end -->
		...
		$(function(){
			...
			$('.widget-rating span').mouseover(function(){
				...
				if(!form_wrap.hasClass('is_show')){
					...
					$.post("{pigcms{:U('Rates/del_invalid_pic')}",{order_id:form_wrap.attr('order_id'),order_type:2});
				}
			});
			...
			$('.J-edit-wrapper').submit(function(){
				...
				$.post($(this).attr('action'),$(this).serialize()+'&order_type=2',function(result){
					alert(result.info);
					if(result.status == 1){
