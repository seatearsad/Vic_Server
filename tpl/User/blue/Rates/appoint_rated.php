...
<title>已评价预约订单 | {pigcms{$config.site_name}</title>
				...
				<div id="content" class="coupons-box">
					<div class="mainbox mine">
						<select class="J-orders-filter orders-filter dropdown--small">
							...
							<option value="{pigcms{:U('Rates/shop_rated')}">{pigcms{$config.shop_alias_name}</option>
							<option value="{pigcms{:U('Rates/appoint_rated')}" selected="selected">预约</option>
						</select>
		...
		$(function(){
			$.each($('.J-rate-wrapper'),function(i,item){
				if($(item).attr('pic_ids') != ''){
					...
					$.post("{pigcms{:U('Rates/ajax_get_pic')}",{pic_ids:$(item).attr('pic_ids'),order_type:2},function(result){
						if(result == '0'){
