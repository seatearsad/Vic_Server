...
<title>待评价{pigcms{$config.shop_alias_name}订单 | {pigcms{$config.site_name}</title>
				...
				<div id="content" class="coupons-box">
					<div class="mainbox mine">
						<select class="J-orders-filter orders-filter dropdown--small">
							...
							<option value="{pigcms{:U('Rates/shop')}" selected="selected">{pigcms{$config.shop_alias_name}</option>
							<option value="{pigcms{:U('Rates/appoint')}">预约</option>
						</select>
										...
										<volist name="order_list" id="vo">
											<div class="rate-item <if condition="$i eq count($order_list)">rate-item--last</if>">
												...
												<div class="J-rate-content rate-item__content">
													...
													<p class="reminding color-weaken">你为本次{pigcms{$config.shop_alias_name}打几分？</p>
													...
													<form action="{pigcms{:U('Rates/reply_to',array('order_id'=>$vo['order_id']))}" order_id="{pigcms{$vo.order_id}" method="post" class="J-edit-wrapper" style="display:none">
														...
														<p class="area-tips">（最少评论15字，您的评价将是其他用户的重要参考）亲，您觉得本{pigcms{$config.shop_alias_name}怎么样？赶快告诉其他用户吧</p>
									                    ...
									                    <div class="add-list">
									                        ...
									                        <volist name="vo['info']" id="vv">
									                        	<span data-goods-id="{pigcms{$vv.goods_id}">{pigcms{$vv.name}<if condition="$vv['spec']">({pigcms{$vv['spec']})</if></span>
									                        </volist>
									                        </div>
						...
						<else/>
							<div class="notice">您没有待评价的{pigcms{$config.shop_alias_name}订单</div>
						</if>
                    </div>
				</div>
			</div> <!-- bd end -->
	...
	<script>
		$(function(){
			...
			$('.J-edit-wrapper').submit(function(){
				...
				$.post($(this).attr('action'),$(this).serialize()+'&order_type=3',function(result){
					alert(result.info);
					if(result.status == 1){
		    ...
		    $(".add-list span").click(function(){
		        ...
		        var obj = $(this), goods_ids = '', pre = '';
		        obj.parents('.add-list-span').children('.select').each(function(){
		        	goods_ids += pre + $(this).attr('data-goods-id');
		        	pre = ',';
				});
				obj.parents('.add-list').find('input').val(goods_ids);
		    });
		});
	</script>
</body>
