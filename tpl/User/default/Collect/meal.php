...
<head>
    ...
    <title>{pigcms{$config.meal_alias_name}收藏 | {pigcms{$config.site_name}</title>
    <!--[if IE 6]>
			...
			<div id="bd" class="cf">
				...
				<div class="component-order-nav mt-component--booted">
					<div class="side-nav J-order-nav">
						...
						<dl class="side-nav__list">
							...
							<dd class="last">
								<ul class="item-list">
									<li><a href="{pigcms{:U('Point/index')}">我的{pigcms{$config['score_name']}</a></li>
				...
				<div id="content" class="coupons-box">
					<div class="mainbox mine">
						<ul class="filter cf">
							<li><a href="{pigcms{:U('Collect/index')}">{pigcms{$config.group_alias_name}收藏</a></li>
							<li class="current"><a href="{pigcms{:U('Collect/meal')}">{pigcms{$config.meal_alias_name}收藏</a></li>
						</ul>
						<if condition="$meal_list">
							<div class="table-section">
								<table id="collection-list" cellspacing="0" cellpadding="0" border="0">
									<tbody>
										<tr>
											<th width="auto" class="item-info">餐厅</th>
											<th width="120">电话</th>
											<th width="112">操作</th>
										</tr>
												...
												<td class="deal">
													<table class="deal-info">
														<tr>
															...
															<td class="text">
																<a class="deal-title" href="{pigcms{$vo.url}" title="{pigcms{$vo.name}" target="_blank">{pigcms{$vo.name}</a>
															</td>
														</tr>
													</table>
												</td>
												<td>{pigcms{$vo.phone}</td>
												<td class="op"><a class="btn btn-mini" href="{pigcms{$vo.url}" target="_blank">购买</a><a class="inline-link remove-collection" href="javascript:void(0);" fav-id="{pigcms{$vo.store_id}">删除</a></td>
											</tr>
										</volist>
									</tbody>
								</table>
						...
						<else/>
							<div class="notice">您还没有收藏过呢，在餐厅详情页直接收藏啦！</div>
						</if>
						{pigcms{$pagebar}
					</div>
				</div>
	...
	<script>
		$(function(){
			$('.remove-collection').click(function(){
				var now_dom = $(this);
				if(confirm('确定删除该收藏？')){
					$.post("{pigcms{:U('Index/Collect/collect')}",{action:'del',type:'meal_detail',id:$(this).attr('fav-id')},function(result){
						if(result.status == '1'){
