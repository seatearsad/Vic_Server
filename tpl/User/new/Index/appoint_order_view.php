							...
							<dd class="bunch-section__content">
								...
								<ul class="flow-list">
									...
									<if condition="($now_order['pay_type'] eq 'offline' AND $now_order['paid'] AND !empty($now_order['third_id'])) OR ($now_order['pay_type'] neq 'offline' AND $now_order['paid'])">
										...
										<li style="margin:30px 0;width:auto;"><b>支付详情：</b>实际支付金额 ${pigcms{$now_order['pay_money']}  商家会员卡余额支付 ${pigcms{:floatval($now_order['merchant_balance'])}  平台余额支付 ${pigcms{:floatval($now_order['balance_pay'])} 
										</li>
									</if>
								</ul>
								...
								<table cellspacing="0" cellpadding="0" border="0" class="info-table">
									<tbody>
										...
										<tr>
											...
											<td></td>
											<td class="total"><if condition='$now_order["appoint_date"] && $now_order["appoint_time"]'><?php echo date('Y-m-d', strtotime($now_order['appoint_date'])); ?>&nbsp;&nbsp;&nbsp;{pigcms{$now_order.appoint_time}</if></td>
										</tr>
									</tbody>
								</table>
							</dd>
