					...
					<div class="mainbox mine">
						...
						<dl class="info-section primary-info J-primary-info">
							<dt>
								...
								<em class="info-section--text"><if condition="empty($now_order['paid'])">未付款<elseif condition="$now_order['third_id'] eq '0' AND $now_order['pay_type'] eq 'offline'"/>未消费 (<font color="red">线下未付款</font>)<elseif condition="empty($now_order['status'])"/>未消费<elseif condition="$now_order['status'] == '1'"/>已使用<elseif condition="$now_order['status'] == '2'"/>已完成</if></em>
						...
						<dl class="bunch-section J-coupon">
							...
							<dd class="bunch-section__content">
								<ul class="flow-list">
									...
									<br><li style="margin:30px 0;width:auto;"><b>支付详情：</b>在线支付金额 ${pigcms{$now_order['payment_money']}  商家会员卡余额支付 ${pigcms{:floatval($now_order['merchant_balance'])}  平台余额支付 ${pigcms{:floatval($now_order['balance_pay'])} <if condition="$now_order['score_deducte'] gt 0">{pigcms{$config['score_name']}抵扣金额 ${pigcms{$now_order.score_deducte} {pigcms{$config['score_name']}使用数量 {pigcms{$now_order.score_used_count}</if>
										</li>
								</ul>
