					...
					<div class="mainbox mine">
						...
						<dl class="info-section primary-info J-primary-info">
							<dt>
								...
								<em class="info-section--text"><if condition="empty($now_order['paid'])">未付款<elseif condition="$now_order['third_id'] eq '0' AND $now_order['pay_type'] eq 'offline'"/>未消费 (<font color="red">线下未付款</font>)<elseif condition="empty($now_order['status'])"/><if condition="$now_order['tuan_type'] neq 2">未消费<else/>未发货</if><elseif condition="$now_order['status'] == '1'"/><if condition="$now_order['tuan_type'] neq 2">已使用<else/>已发货</if><elseif condition="$now_order['status'] == '2'"/>已完成</if></em>
