...
<div class="main-content">
	...
	<div class="breadcrumbs" id="breadcrumbs">
		<ul class="breadcrumb">
			...
			<li class="active">添加{pigcms{$config.meal_alias_name}</li>
		</ul>
	</div>
	<!-- 内容头部 -->
						...
						<div class="tab-content">
							<div id="basicinfo" class="tab-pane active">
								...
								<div class="form-group">
									<label class="col-sm-1"><label for="title">{pigcms{$config.meal_alias_name}网址</label></label>
									...
									<span style='color:red;display:none;' class='js-show-red'>暂无匹配的{pigcms{$config.meal_alias_name}信息</span>
								</div>
								<div class="form-group js-show-red-no">
									<label class="col-sm-1"><label for="title">{pigcms{$config.meal_alias_name}名称</label></label>
									<span id="group_name"></span>
									&nbsp;&nbsp;
								</div>
								<div class="form-group js-show-red-no">
									<label class="col-sm-1"><label for="title">{pigcms{$config.meal_alias_name}所属商家</label></label>
...
$(function(){
	$('.js-show-red').html('暂无匹配的{pigcms{$config.meal_alias_name}信息');
	hide();
})
function checkGroup(){
	var check_url = $('#group_url').val();
	if(check_url.length <1){
		alert('请输入{pigcms{$config.meal_alias_name}地址，才能做匹配');
	}
	 
	if(check_url.indexOf('wap.php')>0){
		var a = parseURL(check_url);
		var group_id = a.params.store_id;
	}else{
	...
	$.post('{pigcms{:U("Index/check_meal")}', {store_id:group_id}, function(result){
		
		if(result.error == 0){
			$('.js-show-red').html('暂无匹配的{pigcms{$config.meal_alias_name}信息');
			hideRed();
			$('#group_name').html(result.store_name);
...
function doSub(){
	...
	$.post('{pigcms{:U("Index/active_meal")}', {store_id:groupId,sort:sort,url:check_url}, function(result){ 
		if(result){
			if(result.error == 0){
				window.location.href = "{pigcms{:U('Index/active_meal_list')}";
			}else if(result.error == 1){
				alert(result.msg);
			}else{
				alert('保存失败，请稍后重试');
