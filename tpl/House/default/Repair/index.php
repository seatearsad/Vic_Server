    ...
    <div class="breadcrumbs" id="breadcrumbs">
        <ul class="breadcrumb">
            <li>
                ...
                <a href="{pigcms{:U('Repair/index')}">在线报修列表</a>
            </li>
        </ul>
    </div>
    <!-- 内容头部 -->
			...
			<div style="border:1px solid #c5d0dc;padding:10px;" class="form-group">
				<form method="get" action="{pigcms{:U('Repair/index')}">
					...
					<select style="margin-right:10px;height:42px; width:120px" class="col-sm-1" id="find_type" name="find_type">
						...
						<option value="3" <if condition="$_GET['find_type'] eq 3">selected="selected"</if>>报修地址</option>
					</select>
					...
					<select style="margin-right:10px;height:42px; width:100px" class="col-sm-1" id="status" name="status">
						...
						<option value="1" <if condition="$_GET['status'] eq 1">selected="selected"</if>>未指派</option>
						<option value="2" <if condition="$_GET['status'] eq 2">selected="selected"</if>>已指派</option>
						<option value="3" <if condition="$_GET['status'] eq 3">selected="selected"</if>>已受理</option>
						<option value="4" <if condition="$_GET['status'] eq 4">selected="selected"</if>>已处理</option>
					...
					<a onclick="location.href='{pigcms{:U('Repair/index')}'" class="btn btn-success">重置</a>
					<a onclick="location.href='{pigcms{:U('repair_export',array_merge($_GET,array('type'=>1)))}'" class="btn btn-success fr">EXCEL导出</a>
				</form>
			</div>
            <div class="row">
                <div class="col-xs-12">
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            <thead>
                                <tr>
									...
									<th width="10%"><a style="color:blue;cursor: pointer" href="{pigcms{:U('Repair/index',array_merge($_GET,array('time'=>asc)))}">报修时间↓</a></th>
									<elseif condition='$_GET["time"] eq "asc"' />
									<th width="10%"><a style="color:blue;cursor: pointer" href="{pigcms{:U('Repair/index',array_merge($_GET,array('time'=>desc)))}">报修时间↑</a></th>
									<else />
									<th width="10%"><a style="color:blue;cursor: pointer" href="{pigcms{:U('Repair/index',array_merge($_GET,array('time'=>desc)))}">报修时间</a></th>
									</if>
                                   	<th width="10%">报修地址</th>
