						...
						<div class="tab-content">
							<div id="basicinfo" class="tab-pane active">
								<div class="form-group">
									...
									<input class="col-sm-2" size="20" name="title" id="title" type="text" value=""/>
								</div>
								...
								<div class="form-group">
									...
									<textarea id="content" name="content"  placeholder="写上一些想要发布的内容"></textarea> 
								</div>
								
								<div class="form-group">
									<label class="col-sm-1">活动时间：</label>
									<input class="col-sm-2 Wdate" type="text" readonly style="height:30px;" onfocus="WdatePicker({minDate:'{pigcms{:date('Y年m月d日 ',$_SERVER['REQUEST_TIME'])}',isShowClear:false,readOnly:true,dateFmt:'yyyy年MM月dd日',startDate:'{pigcms{:date('Y-m-d',$_SERVER['REQUEST_TIME'])}',vel:'activity_start_time'})" value="{pigcms{:date('Y年m月d日',$_SERVER['REQUEST_TIME'])}"/>
									<input name="activity_start_time" id="activity_start_time" type="hidden" value="{pigcms{:date('Y-m-d',$_SERVER['REQUEST_TIME'])}"/>
									<label class="col-sm-1" style=" text-align:center; width:40px">至</label>
									<input class="col-sm-2 Wdate" type="text" readonly style="height:30px;" onfocus="WdatePicker({minDate:'{pigcms{:date('Y年m月d日 ',$_SERVER['REQUEST_TIME'])}',isShowClear:false,readOnly:true,dateFmt:'yyyy年MM月dd日',startDate:'{pigcms{:date('Y-m-d',strtotime('+30 day'))}',vel:'activity_end_time'})" value="{pigcms{:date('Y年m月d日',strtotime('+30 day'))}"/>
									<input name="activity_end_time" id="activity_end_time" type="hidden" value="{pigcms{:date('Y-m-d',strtotime('+30 day'))}"/>
								</div>
								
								<div class="form-group">
									<label class="col-sm-1">报名截止时间：</label>
									<input class="col-sm-2 Wdate" type="text" readonly style="height:30px;" onfocus="WdatePicker({minDate:'{pigcms{:date('Y年m月d日 ',$_SERVER['REQUEST_TIME'])}',isShowClear:false,readOnly:true,dateFmt:'yyyy年MM月dd日',startDate:'{pigcms{:date('Y-m-d',$_SERVER['REQUEST_TIME'])}',vel:'stop_apply_time'})" value="{pigcms{:date('Y年m月d日',$_SERVER['REQUEST_TIME'])}"/>
									<input name="stop_apply_time" id="stop_apply_time" type="hidden" value="{pigcms{:date('Y-m-d',$_SERVER['REQUEST_TIME'])}"/>
								</div>
								
								<div class="form-group">
									<label class="col-sm-1"><label for="apply_limit_num">活动报名人数</label></label>
									<input class="col-sm-2" size="20" name="apply_limit_num" id="apply_limit_num" type="text" value="0" />
									<span class="form_tips">0 为不限制</span>
								</div>
								<div class="form-group">
									<label class="col-sm-1"><label for="sort">排序</label></label>
									<input class="col-sm-2" size="20" name="sort" id="sort" type="text" value="0" />
									<span class="form_tips">值越大，越靠前显示。</span>
								</div>
								
								<div class="form-group">
									<label class="col-sm-1"><label for="apply_fee">报名费用</label></label>
									<input class="col-sm-2" size="20" name="apply_fee" id="apply_fee" type="text" value="0" />
								</div>
                                <div class="form-group">
