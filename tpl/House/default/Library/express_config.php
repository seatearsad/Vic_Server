		...
		<table>
			...
			<tr>
				...
				<td width="85%">
					 ...
					 <label>
						<input class="col-sm-2 Wdate" type="text" readonly style="height:30px;" placeholder="开始时间" onfocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'HH:mm',vel:'start_time'})" value="{pigcms{$express_config.start_time|date='H:i',###}"/>
						<input name="start_time" id="start_time" type="hidden" value="{pigcms{$express_config.start_time|date='H:i',###}"/>
&nbsp;&nbsp;至&nbsp;&nbsp;
						<input class="col-sm-2 Wdate" placeholder="结束时间" type="text" readonly style="height:30px;" onfocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'H:mm',vel:'end_time'})" value="{pigcms{$express_config.end_time|date='H:i',###}"/>
			...
			<tr>
			 ...
			 <tr>
				<td colspan="4"><button class="chk_express" style=" margin:0 auto; display:block;float:right">保存</button></td>
			</tr>
